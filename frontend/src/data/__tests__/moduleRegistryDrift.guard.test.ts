 ...
 *
 * Они не совпадают. Так было с DevHub: продавался и был открыт на /devhub,
 * а в каталоге его не было вовсе — вписан 14.09.2026, счётчики подняты вслед.
 * IP Bureau и вовсе живёт под двумя
 ...
 * поимённый, и большинство осознанно не вынесено на витрину (внутренняя
 * механика, материалы). Сторожим только рост числа: если завтра забудут вынести
 * ещё один модуль, счётчик это покажет.
 *
 * 13.09.2026: 19 -> 20, добавлен `qspace` (3D-модельер помещений). Он внесён в
 * каталог, но на витрину НЕ вынесен намеренно: цена и имя модуля ещё не
 * назначены основателем, а витрина — это про купить. Поднимать этот порог
 * можно только так: назвать модуль и причину. Молча увеличенный храповик
 * означает, что сторож перестал сторожить.
 */
const CATALOGUED_NOT_SHOWN_MAX = 20;
