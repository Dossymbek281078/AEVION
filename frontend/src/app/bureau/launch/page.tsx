...
// 13.09.2026: функция, а не константа. Страница рендерится на каждый запрос
// (замер прода: cache-control no-store), но объект `metadata` вычисляется при
// загрузке модуля и живёт, пока жив экземпляр функции. Заголовок с датой,
// посчитанный один раз, пережил бы день запуска на тёплом экземпляре.
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", display: "flex", flexDirection: "column", gap: "clamp(14px, 4vw, 28px)" }}>
        <header>
          ...
          <p style={{ color: MUTED, fontSize: 15.5, lineHeight: 1.5, margin: "10px 0 0" }}>
            ...
            {left > 0
              ? ` Открываем ${left === 1 ? "завтра" : `через ${left} дн.`} — ${PLATFORM_LAUNCH_HUMAN}.`
              : left === 0
                ? ` Открываем сегодня — ${PLATFORM_LAUNCH_HUMAN}.`
                : ` Обещали ${PLATFORM_LAUNCH_HUMAN} — напишем, как только откроем.`}
          </p>
        </header>


        <WaitlistCapture
          // Язык НЕ задан жёстко (было lang="ru", снято 06.09.2026, живой
          // замер): доводчик AutoTranslate переводит EN-визитёру весь текст
          // посадочной, но НЕ атрибуты — оставался русский «вы@почта.рф»
          // ровно в поле контакта. Форма идёт за языком посетителя;
          // русскому — русская, как раньше. Образец: /go, /devhub/launch.
          source={source}
          tone="light"
