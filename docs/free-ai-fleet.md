...
## Порядок действий, когда ключ на руках

1. `RAILWAY_TOKEN=<токен> ./scripts/set-provider-key-railway.sh <провайдер> <ключ>` —
   запишет, передеплоит и дождётся, пока живой прод покажет `configured: true`.
2. `node scripts/free-fleet-live-check.js` — спросит нового участника одним словом:
   отвечает или нет, за сколько, с какой ошибкой.
3. Открыть `/api/qcoreai/providers` и убедиться, что счётчик бесплатных вырос.
4. Через сутки посмотреть `/api/qcoreai/smart/savings` — там видно, сколько прогонов
   ушло дёшево. Внимание: на 27.07 в этом реестре было ВСЕГО ТРИ прогона за всё время,
   то есть цифра «сэкономлено» пока ничего не измеряет. Она станет осмысленной, когда
   через `smart` пойдёт реальный поток.
5. Если провайдер отвечает 429 в первый же день — это не поломка, а его бесплатный
   тариф. Смотрите таблицу лимитов ниже и подключайте следующего.
