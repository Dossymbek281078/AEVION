...
describe("платный доступ не закрывает сбор адресов", () => {
  ...
  test("справочник нотариусов открыт только на чтение", async () => {
    // Страницы /bureau/notaries серверные и токена не несут: закрой их стеной —
    // справочник пропадёт у всех. Открываем GET и ничего больше.
    const src = await import("node:fs").then((fs) =>
      stripComments(fs.readFileSync(new URL("../src/lib/planGate.ts", import.meta.url), "utf8")));
    expect(src, "справочник нотариусов не открыт — после закрытия бюро 402 любому").toContain('p === "/notaries"');
    expect(src, "карточка нотариуса не открыта").toContain('p.startsWith("/notaries/")');
    expect(src, "исключение нотариусов не ограничено чтением — откроется и запись")
      .toMatch(/req\.method === "GET" && \(p === "\/notaries"/);
  });
