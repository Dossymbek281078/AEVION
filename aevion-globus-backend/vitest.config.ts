...
export default defineConfig({
  ...
  test: {
    ...
    globals: false,
    // 30 с, а не 10. Мера взята из `launch/2026-08-30` (диагноз 11–12.08.2026),
    // а не придумана заново. Причина там разобрана точно: многие тесты
    // делают `await import("../src/routes/…")` ВНУТРИ тела (чтобы задать env
    // до импорта), и время импорта тяжёлого роутера засчитывается в таймаут
    // ТЕСТА. Под чужими сборками один импорт легко перерастает десять секунд.
    //
    // Почему ОБЕ меры, а не одна: последовательный прогон (ниже) снимает
    // гонку МЕЖДУ файлами, но не ускоряет импорт на загруженной машине.
    // Замер 19.08 при 121 процессе node и четырёх чужих сборках: только с
    // последовательностью набор был зелёным 2 раза из 3, а третий раз упёрся
    // именно в 10 секунд на `await import`.
    testTimeout: 30_000,
    hookTimeout: 30_000,
    env: {
      // ЖИВАЯ СЕТЬ В UNIT-НАБОРЕ — источник случайной красноты.
      //
      // `opentimestamps.anchor.test.ts` ходит на настоящий сервер календаря.
      // Выключатель в нём предусмотрен автором, но НИГДЕ не был задан, то
      // есть сеть дёргалась каждым прогоном. Замер 19.08: этот файл упал
      // по 30-секундному таймауту в одном прогоне из трёх — причина внешняя,
      // логика теста при этом верна.
      //
      // Покрытие НЕ теряется: тот же путь проверяют отдельные смоуки —
      // `scripts/ots-smoke.ts`, `scripts/trust-anchor-smoke.ts` и якорь в
      // `scripts/bureau-prod-smoke.js`. Там сеть уместна, здесь нет.
      OT_SKIP_NETWORK: "1",
    },
    // ЧТО ИЗ ИХ КОНФИГА НЕ ВЗЯТО И ПОЧЕМУ: там ещё стоит ограничение
    // воркеров (`maxWorkers`). При `fileParallelism: false` файлы идут
    // последовательно, и это ограничение почти не работает — а добавлять
    // настройку, польза которой не измерена, значит расходиться с их веткой
    // без причины. Если после мержа обеих мер флейк вернётся, брать надо
    // именно его, и тогда уже с замером.
