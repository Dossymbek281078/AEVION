...
function CancelInner() {
   ...
   */
  /*
   * Каждый источник проходит проверку по списку, а не только память.
   *
   * Значение из адреса раньше шло СЫРЫМ и попадало в три места: в адрес кнопки
   * `/pricing/<тариф>`, в её подпись и в событие учёта. Страницы тарифов живут
   * на динамическом маршруте, поэтому любое значение что-то отрисует — и по
   * подобранной ссылке кнопка возврата вела бы в никуда, а на нашем экране
   * печатался бы чужой текст.
   */
  const первыйИзвестный = (...значения: Array<string | null | undefined>): string | null =>
    значения.find((з) => тарифИзвестен(з)) ?? null;
