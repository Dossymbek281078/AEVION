...
const HEDGE =
  /(^|[^\p{L}])(не могу|не уверен|не берусь|возможно|вероятно|скорее всего|может быть|недостаточно данных|затрудняюсь|сложно сказать|сложно судить|трудно сказать|оценить сложно|однозначного ответа нет|многое зависит|всё зависит|все зависит|нужны дополнительные данные|требуется уточнение|не стал бы утверждать|бы не стал утверждать|точно сказать нельзя|can't|cannot|not sure|unable to|as an AI|not able|it depends|hard to say|difficult to say|insufficient data|cannot be certain)([^\p{L}]|$)/iu;
