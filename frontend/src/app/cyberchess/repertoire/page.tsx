...
function ModeSwitcher({ mode, setMode }: { mode: Mode; setMode: (m: Mode) => void }) {
  ...
  return (
    <div
      style={{
        display: "flex",
        // Перенос — чтобы четыре кнопки помещались на узком телефоне. Замер
        // 28.08.2026 на ширине 320: ряд «Управление · Поиск · Сравнить ·
        // Печать» получался шириной 348 при окне 305 и утаскивал страницу
        // вбок на 49 пикселей. Сторож указывал на кнопку «Печать», но она
        // лишь последняя в ряду — переполнял сам ряд.
        flexWrap: "wrap",
        gap: 4,
        padding: 4,
