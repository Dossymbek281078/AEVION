 ...
 */
const ИЗВЕСТНЫЕ = [
  // lib/pricingI18n.ts отсюда УБРАН 17.09.2026: основатель решил, что пробный
  // период не нужен, кнопка «Попробовать 14 дней бесплатно» снята со страницы
  // цен вместе с текстом. Появится обещание снова — сторож покраснеет.
  //
...
describe("обещание пробного периода не расходится с расчётом цены", () => {
  ...
  it("прибор видит предмет: исходники найдены, расчёт цены прочитан", () => {
    ...
    // С 17.09 словарь страницы цен обещания больше не содержит, поэтому контроль
    // берёт оставшееся известное место — историческую запись журнала изменений.
    const журнал = readFileSync(join(SRC, "lib/i18n-lang/ru.ts"), "utf8");
    expect(журнал).toContain("14 дней бесплатно");
  });
