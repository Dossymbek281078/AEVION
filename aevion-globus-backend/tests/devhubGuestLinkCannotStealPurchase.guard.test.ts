...
//
// Он читает ИСХОДНИК и проверяет, что нужные строки в нём есть. Такой
// признак ошибается в обе стороны: совпадает с упоминанием в комментарии
// и проходит, если сравнение стоит ПОСЛЕ записи или под условием, которое
// никогда не истинно.
//
// Поэтому ПОВЕДЕНИЕ теперь проверяется отдельно, вызовом и запросом:
//   devhubGuestLinkIsBoundToOneBrowser — чужой браузер с ВЕРНЫМ токеном
//     получает отказ, и связь при этом не записывается;
//   devhubGuestLinkRouteContract — «письмо ушло» и «покупки нет»
//     неразличимы снаружи, отказы не выдаются за успех;
//   devhubGuestLinkIsRateLimited — открытая ручка под боевым пределом.
//
// Этот файл оставлен намеренно: он стережёт ФОРМУ (токен хешируется, а не
// хранится; ответ нейтрален по тексту), которую поведенческие проверки не
// видят. Если будете его править — не дублируйте сюда поведение, иначе два
// сторожа начнут утверждать одно и разойдутся молча.
