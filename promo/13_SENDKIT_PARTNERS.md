# Partner send-kit — PIF · Mubadala · ADQ

> Три финальных письма под Innovation Partnership — партнёрство, не выкуп: партнёр входит
> в основном ресурсами + небольшой возвратный аванс $10M, 49% upside партнёру, фаундер
> остаётся Chief Idea Officer. Универсал одним письмом — `21_UNIVERSAL_LETTER.md`.
> Структура каждого: subject + body + 3 reply-шаблона + send-чеклист.
> Язык — английский для всех трёх (международные партнёры).
> **Читай своими глазами перед каждой отправкой. Это черновики, не финал.**
>
> ⚠️ **Правило безопасности идей:** никогда не отправлять в государственные
> или квазигосударственные фонды KZ (Baiterek, QazTech, AIFC) — высокий риск
> копирования идей без партнёрства. Только международные частные и суверенные
> фонды, для которых IP-protection является стандартом практики.

---

## ПИСЬМО 1 — PIF / Sanabil Investments (Саудовская Аравия)

**Subject:** `AEVION Partnership — bring your team, the founder stays (post-quantum IP, MENA-ready)`

**To:** `info@sanabil.com` (официальный contact Sanabil) / через форму на sanabil.com/contact
**CC (если есть warm intro):** через LinkedIn поиск «Sanabil Investments investment team»
**Язык:** английский

```
Sanabil Investment Team,

Eid Mubarak!

I'm not a programmer. I'm a Financial Director and construction business
owner in Kazakhstan who spent a decade watching millions disappear into
fraudulent permits, stolen design documents, and untraceable payments.
A chess candidate master who thinks in patterns and plays 10 moves ahead.

Using AI, I built 30+ working solutions to these problems in 6 months.
Not because I write fast code — because I understood the problem for 10 years.
One of them — QSign — is the only post-quantum digital signature product
in Central Asia (FIPS 204, GA).

This is not a pitch for capital to continue building. It's a partnership
offer where you bring the execution team and I bring the thinking that
built 30 products nobody asked me to build — because I couldn't stop seeing
the problems.

How the partnership works:
  This is a partnership, not a buyout. The founder stays — full-time on
  ideas, as Chief Idea Officer. You enter mostly with resources: compute,
  a team of 50-100 engineers, distribution, brand — plus a small $10M
  returnable advance whose only purpose is to free the founder to work
  full-time on the pipeline. The advance is repaid from the founder's
  share as the projects grow.

Financial case:
  Income split: 51% founder / 49% partner (starting frame) — 49% of the
  upside goes to you.
  Year 3: $130M ARR → $1.0-1.6B company valuation
  Year 5: $490M ARR → $3.9-5.9B company valuation
  Your 49% of the upside scales with the company, not with a fixed entry
  price — you grow as the pipeline grows.

Why this fits Vision 2030:
  1. QSign (FIPS 204 ML-DSA-65, GA) — post-quantum e-signature ready for
     national infrastructure in KSA, UAE, Kazakhstan. Government contract
     potential: $20-100M/year per jurisdiction.
  2. AEVION Bank — DIFC-licenseable digital bank with Shariah-compliant
     structure available. $200M+ ARR by year 5.
  3. QRight (Patent Bureau) — IP attestation for AI-generated content.
     National patent office integration = recurring government revenue.
  4. DevHub — 15 SaaS tabs → 1 workspace (9 integrations live). Zapier
     and Make are unavailable or unreliable in MENA. First mover available.

Founder has 5+ unrevealed product concepts disclosed under NDA at LOI stage.

Full brief + live health-board: https://aevion.app/partner
30-min call this week?

— Dosymbek Zhakiya, Founder & Chief Idea Officer @ AEVION
yahiin1978@gmail.com
```

### Reply — POSITIVE

```
Thank you for the quick reply.

Available for a 30-minute call:
  • [Day, time GST / Riyadh time]
  • [Day, time GST / Riyadh time]
  • [Day, time GST / Riyadh time]

Zoom / Teams — your choice.

Before the call, two things worth reviewing:
  1. https://aevion.app/partner — full brief, printable
  2. https://aevion.app/transparency — live health-board

Looking forward.

— Dosymbek
yahiin1978@gmail.com
```

### Reply — PUSHBACK («not right fit / not now»)

```
Appreciate the honest reply.

Three ways to stay in touch with low time commitment:

  1. 90-day pilot — Trust $50K / Dev $75K / Financial $100K.
     Full pilot fee credited against future partnership.
     https://aevion.app/pilot

  2. Quarterly update — 5-line email at the start of each quarter.
     Reply only if something becomes relevant.

  3. Introduce us to a more relevant partner within your network —
     your read on who would be a better fit is genuinely valuable.

Either way, thank you for the response.

— Dosymbek
```

### Reply — SILENCE 30 days

**Subject:** `Re: AEVION Partnership — Q3 2026 checkpoint`

```
Following up once on the AEVION partnership brief from May 27, 2026.

Two verified updates since then:

  1. [UPDATE 1 — e.g. "QSign SDK downloaded by 300+ teams on npm"] ← fill in real number
  2. [UPDATE 2 — e.g. "AEVION Bank prototype completed a full test payment cycle"] ← fill in

If timing remains wrong — no reply needed.
Brief still at: https://aevion.app/partner?ref=pif-fu1

— Dosymbek
yahiin1978@gmail.com
```

---

## ПИСЬМО 2 — Mubadala Investment Company (Абу-Даби, ОАЭ)

**Subject:** `AEVION Partnership — DIFC digital bank + post-quantum IP (you bring the team, founder stays)`

**To:** через форму mubadala.ae → «Investment Enquiry» / или `communications@mubadala.ae`
**Alt contact:** DIFC FinTech Hive (они напрямую коннектят с Mubadala) → fintechhive.ae/contact
**Язык:** английский

```
Mubadala Investment Company,

Eid Mubarak!

I'm not a programmer. I'm a Financial Director and construction business
owner in Kazakhstan — the industry where fraudulent permits, stolen design
documents, and opaque payment chains cost billions annually. A chess
candidate master who spent 10 years watching this problem from the inside.

Using AI, I turned that decade of frustration into 30+ working prototypes
in 6 months: post-quantum signatures, digital banking, IP registry,
developer platform. One person. No engineering team. Zero VC funding.

AEVION is now seeking an execution partner. You bring the team.
I bring the thinking that never stops generating — and I stay.

Why Mubadala specifically — three direct synergies:

  1. DIFC digital banking license
     AEVION Bank has a completed technical foundation. With Mubadala's
     DIFC relationships, time-to-license drops from 36 to 12-18 months.
     UAE digital banking market: $500M+ ARR opportunity by 2030.

  2. QSign as UAE critical infrastructure
     Post-quantum signatures (ML-DSA-65 FIPS 204) for CBUAE compliance.
     AEVION is production-ready; UAE regulators are actively evaluating
     quantum-resistant cryptography. First-mover position available now.

  3. MENA developer platform
     AEVION DevHub (9 integrations live: GitHub, Vercel, Cloudflare,
     Stripe, ElevenLabs, Brevo, DALL-E, Drive, Railway) — the first
     full-stack developer workspace that works in MENA compliance environments.

How it works: a partnership, not a buyout. The founder stays as Chief
Idea Officer, full-time on the pipeline. Mubadala enters mostly with
resources — the engineering team, DIFC relationships, distribution,
brand — plus a small $10M returnable advance to free the founder for
full-time work; the advance is repaid from the founder's share as the
projects grow. Income split 51% founder / 49% partner — 49% of the
upside to you.

Year 5 scenario: $490M ARR, $3.9-5.9B valuation. Your 49% of the upside
scales with the company as the pipeline grows.

Brief + live modules: https://aevion.app/partner
30-min call this week or next?

— Dosymbek Zhakiya, Founder & Chief Idea Officer @ AEVION
yahiin1978@gmail.com
```

### Reply — POSITIVE

```
Thank you for the response.

Available for a 30-minute call:
  • [Day, time UAE time (GST)]
  • [Day, time UAE time (GST)]
  • [Day, time UAE time (GST)]

Zoom / Teams / in-person DIFC if preferred.

Preparation: https://aevion.app/partner (full brief, printable PDF)
Live modules: https://aevion.app/transparency

— Dosymbek
```

### Reply — PUSHBACK

```
Understood, and thank you for the candid reply.

If the timing or structure needs adjustment, three alternatives:

  1. DIFC FinTech Hive pilot programme — AEVION Bank module is a natural
     candidate. I can apply directly; your introduction would accelerate.

  2. Quarterly update — brief checkpoint, no calls required.

  3. Warm intro within Mubadala portfolio to a company that could
     benefit from QSign or DevHub — even a referral is valuable.

Thank you either way.

— Dosymbek
```

### Reply — SILENCE 30 days

**Subject:** `Re: AEVION Partnership — Q3 checkpoint`

```
Following up once on the AEVION brief from May 27, 2026.

Two things changed since then:

  1. [UPDATE 1 — verifiable fact]
  2. [UPDATE 2 — verifiable fact]

Brief: https://aevion.app/partner?ref=mubadala-fu1

— Dosymbek
```

---

## ПИСЬМО 3 — ADQ / Abu Dhabi Developmental Holding (ОАЭ)

> ADQ — суверенный фонд Абу-Даби ($110B+ AUM), фокус на tech-инфраструктуре
> и diversification. Работает параллельно с Mubadala, отдельный LP.
> Прямая синергия: ADQ активно строит digital economy в MENA.

**Subject:** `AEVION Partnership — digital infrastructure play for MENA (you bring the team, founder stays)`

**To:** через форму adq.ae → «Investment Enquiry» / `invest@adq.ae`
**Alt:** через ADGM (Abu Dhabi Global Market) ecosystem — они коннектят с ADQ tech team

```
ADQ Investment Team,

Eid Mubarak!

A question first: can your team build something like QSign?
Yes — in 3-6 months. But while they're building QSign, who builds
the next one? And the one after that?

That's the offer on the table.

I'm Dosymbek Zhakiya — Financial Director and construction business owner
in Kazakhstan. A chess candidate master. A person who for 10 years worked
inside one of the world's most opaque industries and saw exactly what
trust infrastructure should look like — but didn't exist.

With AI tools, I built 30+ working prototypes in 6 months. Not development
speed — idea speed. The bottleneck is execution. That's where ADQ comes in.

Partnership structure — a partnership, not a buyout:
  ADQ brings the execution — engineers, compute, distribution, brand —
  and the founder stays full-time as Chief Idea Officer. The money detail
  is at the bottom; the headline is the team and the pipeline.

Why ADQ / MENA fit:

  1. Digital banking infrastructure
     AEVION Bank: DIFC-licenseable digital bank, Shariah-compliant
     structure ready. ADQ's financial services portfolio = natural fit.
     ARR potential: $200M+ by year 5.

  2. Post-quantum signatures as MENA standard
     QSign (ML-DSA-65 FIPS 204 GA) — ready now, competition is 12-18
     months behind. First-mover for UAE / KSA government e-signature
     infrastructure. National contract potential: $20-100M/year.

  3. Developer platform for the region
     AEVION DevHub — 9 integrations live, zero viable MENA-compliant
     competitors to Zapier/Make. SMB developer market in UAE/KSA
     is underserved. ARR potential: $50-150M/year.

  4. AI API for government sector
     QCoreAI (5+ AI providers, 490 tests passing) — last-mile AI
     for governments where Anthropic/OpenAI have compliance gaps.

The money, at the bottom: ADQ enters mostly with resources plus a small
$10M returnable advance — its only purpose is to free the founder for
full-time work on ideas, repaid from the founder's share as the projects
grow. Income split 51% founder / 49% partner — 49% of the upside to ADQ.

Year 5 scenario: $490M ARR, $3.9-5.9B valuation. Your 49% of the upside
scales with the company as the pipeline grows.

Full brief + live health-board: https://aevion.app/partner?ref=adq
30-min call this week?

— Dosymbek Zhakiya, Founder & Chief Idea Officer @ AEVION
yahiin1978@gmail.com
```

### Reply — POSITIVE

```
Thank you for the quick reply.

Available for a 30-minute call:
  • [Day, time UAE (GST)]
  • [Day, time UAE (GST)]
  • [Day, time UAE (GST)]

Zoom / Teams / in-person Abu Dhabi if preferred.

Before the call:
  1. https://aevion.app/partner — full brief, printable
  2. https://aevion.app/transparency — live health-board

— Dosymbek
yahiin1978@gmail.com
```

### Reply — PUSHBACK

```
Appreciate the honest reply.

Three ways to keep the conversation low-commitment:

  1. 90-day pilot on one pillar (Trust $50K / Dev $75K / Financial $100K).
     Full fee credited against future partnership.
     https://aevion.app/pilot

  2. Quarterly update — 5-line email, no calls required.
     Reply only if something becomes relevant.

  3. Introduction to a more relevant team within ADQ's portfolio —
     even a referral is genuinely valuable.

Thank you either way.

— Dosymbek
```

### Reply — SILENCE 30 days

**Subject:** `Re: AEVION Partnership — Q3 checkpoint`

```
Following up once on the AEVION brief from May 27, 2026.

Two verified updates since then:

  1. [UPDATE 1 — verifiable fact, e.g. "QSign SDK downloaded by 400+ teams"]
  2. [UPDATE 2 — verifiable fact]

Brief: https://aevion.app/partner?ref=adq-fu1

— Dosymbek
```

---

## Send-чеклист (единый для всех трёх)

- [ ] Прочитал письмо вслух — нет запинок
- [ ] Дата отправки: **вторник, среда или четверг**
- [ ] Время: 8:00-10:00 по времени получателя (Эр-Рияд +3, Абу-Даби +4)
- [ ] Plain-text mode в Gmail — никакого HTML, никаких подписей с лого
- [ ] Нет attachments — всё по ссылке aevion.app/partner
- [ ] Ссылка `aevion.app/partner` работает (проверить перед отправкой)
- [ ] Записал дату отправки в таблицу
- [ ] Три временных слота готовы для reply-шаблона POSITIVE
- [ ] ⚠️ Не отправлять в KZ госструктуры (Baiterek, QazTech, AIFC) — риск кражи идей

---

## 30-дневный календарь

| День | Действие |
|------|---------|
| **D-0 (сегодня)** | Отправить **Mubadala** — DIFC прямая синергия с Banking + QSign |
| **D+3** | Отправить **ADQ** — второй UAE sovereign fund, digital infra focus |
| **D+7** | Отправить **PIF/Sanabil** — Vision 2030 angle (самый крупный) |
| **D+7-14** | Если Mubadala ответил → фокус на нём, остальные пауза |
| **D+28-30** | Follow-up по молчащим (шаблон Silence, по одному) |
| **D+45** | Если ноль ответов — пересмотреть subject / попробовать SoftBank Vision Fund |

**Почему Mubadala первым:**
- DIFC jurisdiction = их home turf (это плюс, а не риск)
- AEVION Bank + DIFC license = прямой ответ на их портфельную стратегию
- UAE финтех регулятор (CBUAE) активно смотрит на post-quantum cryptography
- После positive reply — ADQ и PIF видят, что уже есть интерес → создаёт urgency

**Почему НЕ KZ государственные структуры:**
В Казахстане существует задокументированный риск того, что государственные или
квазигосударственные структуры используют pitch-материалы как roadmap для
собственной разработки без партнёрства с автором. Идеи не защищены до
подписания NDA + term sheet с эскроу-условиями. С международными фондами
(PIF, Mubadala, ADQ) этот риск минимален — они профессиональные инвесторы,
их reputation зависит от честности сделок.

---

— редакция 2026-05-27, AEVION
