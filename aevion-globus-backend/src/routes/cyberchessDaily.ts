...
router.get('/puzzle', async (_req: Request, res: Response) => {
  ...
  return res.json({
    ...
    // Машинный признак рядом с человеческой подписью. Раньше клиент решал по
    // ПОДСТРОКЕ в source — и «резервный пул…: банк не ответил» содержит слово
    // «банк», поэтому страница уверенно писала «задача из общего банка» ровно
    // тогда, когда банк НЕ ответил. Признак для машины отдельно от текста для
    // человека: одна строка не может служить обоим.
    fromBank: !!fromBank,
    source: fromBank
