...
describe("новых мёртвых адресов не появляется", () => {
  ...
  it("ни одной новой страницы с адресом на домене без почты", () => {
    ...
    expect(
      новые,
      "ДОБАВИЛОСЬ. Письмо на @aevion.app не доходит никуда: у домена нет MX. " +
  ...
  it("список не протухает: починенное убирается из него", () => {
    ...
    expect(
      исчезнувшие,
      "ПОЧИНЕНО, УБЕРИТЕ ИЗ СПИСКА. Адрес со страницы ушёл — удалите её строку " +
        "из набора ИЗВЕСТНЫЕ выше, иначе список " +
        "будет обещать беду, которой уже нет. Это НЕ значит, что починка не помогла",
    ).toEqual([]);
  });
});
