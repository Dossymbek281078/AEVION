...
// Каждая проверка делает `await import()` роутера, и время трансформации этого
// модуля идёт в бюджет самого теста. В одиночку укладывается, при полном
// параллельном прогоне — нет: замер 12.08.2026 дал 10983 мс и КРАСНЫЙ тест при
// полностью исправном коде (те же 246 проверок в одиночку зелёные). Ложная
// краснота хуже отсутствия проверки: к ней привыкают и перестают смотреть.
// Тяжёлому файлу — свой явный таймаут; на скорость одиночного прогона не влияет.
