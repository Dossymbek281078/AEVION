...
/**
 * Экран остатка обязан показывать ВСЕ возможности, которые списываются.
 *
 * 02.09.2026 я завёл две новые (речь, перевод) и чуть не оставил список в
 * `/studio/credits` литеральным. Списание работало бы, а показателя на экране
 * не было — человек тратит квоту, которой не видит, и упирается в предел без
 * предупреждения. Ни один прежний тест этого не ловил: оба места по
 * отдельности исправны, расходятся они только вместе.
 */
describe("экран остатка не отстаёт от таблицы тарифов", () => {
  test("остаток показывает ВСЕ возможности из таблицы, а не список рядом с ней", async () => {
    const app = makeApp();
    const r = await request(app).get("/api/devhub/studio/credits");
    expect(r.status, "ручка остатка не ответила").toBe(200);

    const изТаблицы = Object.keys(r.body.tierInfo?.free ?? {}).sort();
    const наЭкране = Object.keys(r.body.usage ?? {}).sort();

    // Контроль прибора: пустой разбор дал бы «списки совпали» на двух пустых.
    expect(изТаблицы.length, "таблица тарифов не доехала в ответ").toBeGreaterThan(4);

    expect(
      наЭкране,
      "Возможность списывается, но её нет на экране остатка — человек тратит " +
        "квоту, которой не видит. Список возможностей в /studio/credits должен " +
        "выводиться ИЗ ТАБЛИЦЫ (Object.keys), а не писаться рядом с ней.",
    ).toEqual(изТаблицы);
  });
});
