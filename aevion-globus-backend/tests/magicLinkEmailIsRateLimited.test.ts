...
const ВХОДЫ = ["affiliate", "partners"] as const;

// КОНТРОЛЬ ОХВАТА. Список выше написан рукой, а ручек входа со временем
// становится больше: третья добавилась бы МОЛЧА и осталась без предела
// на отправку писем — то есть без защиты ровно там, где выжигается
// суточная квота почтового провайдера.
//
// Читаем исходник, а не роутер: поднимать приложение ради одного
// подсчёта дороже, а имя маршрута в исходнике и есть источник правды.
test("КОНТРОЛЬ ОХВАТА: перебор покрывает все ручки magic-link", async () => {
  const fs = await import("node:fs");
  const url = new URL("../src/routes/pricing.ts", import.meta.url);
  const текст = fs.readFileSync(url, "utf8");
  const найдено = [...текст.matchAll(/"\/([a-z0-9-]+)\/magic-link"/g)].map((m) => m[1]);
  expect(найдено.length, "в исходнике не нашлось ни одной ручки — проверять нечего")
    .toBeGreaterThan(0);
  const вне = найдено.filter((имя) => !(ВХОДЫ as readonly string[]).includes(имя));
  expect(вне, "ручка magic-link есть в коде, но этот сторож её не проверяет").toEqual([]);
});
