...
//
// ⚠️ ПЕРЕСЕЧЕНИЕ, СВЕСТИ ПРИ МЕРЖЕ. Параллельная сессия в тот же день
// написала `scripts/build-contract-check.mjs` (ветка
// `docs/qbuild-master-plan-refresh`), и он **сильнее** этого храповика:
// ловит тот же голый `/api/...`, но вдобавок сверяет каждый вызов фронтенда
// с реально смонтированными маршрутами Express — то есть видит и обратный
// дефект, когда ручку переименовали, а вызов остался. Плюс он уже поставлен
// гейтом на PR.
//
// Здесь остаётся одно, чего у него нет: планка на весь репозиторий разом
// (BASELINE), которая не даёт числу расти, пока долг не выплачен; он же
// ходит по одному модулю за раз. Когда обе ветки сольются — правильнее
// перенести идею планки в его скрипт и удалить этот файл, а не держать две
// проверки одного класса.
//
