...
const Cell=React.memo(function Cell({sq,pieceType,pieceColor,bg,cursor,iS,iV,iCk,iPM,iPS,iL,isShadow,isAnimDest,isDragOrigin,pmIdx,coordFile,coordRank}:CellProps){
  ...
  return <div data-sq={sq}
    ...
    {hasPiece&&<div key={snapNonce} className={snapClass} style={{width:"88%",height:"88%",transform:isLifted?"scale(1.12)":"none",filter:isLifted?"drop-shadow(0 6px 10px rgba(0,0,0,0.45))":(isShadow?"drop-shadow(0 2px 3px rgba(0,0,0,0.25))":"drop-shadow(0 2px 3px rgba(0,0,0,0.35))"),opacity:isDragOrigin||isAnimDest?0:(isShadow?0.55:1),transition:"transform 120ms cubic-bezier(0.4,0,0.2,1), opacity 100ms ease-out, filter 120ms ease-out",animation:iCk?"cc-pulse-glow 1.2s ease-in-out infinite":undefined,borderRadius:iCk?"50%":undefined,pointerEvents:"none"}}><Piece type={pieceType} color={pieceColor}/></div>}
