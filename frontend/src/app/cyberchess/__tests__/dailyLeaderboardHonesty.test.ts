...
describe("таблица лидеров берётся с сервера", () => {
  ...
  test("имена живых гроссмейстеров не зашиты в страницу", () => {
    ...
    const s = src();
    // Проверяем ЛЮБОЙ способ записи, а не только одинарные кавычки: имя может
    // приехать в двойных, в шаблонной строке или прямо текстом в разметке.
    // Комментарии отбрасываем — объяснение запрета в шапке файла упоминает эти
    // имена, и сторож краснел бы на собственном объяснении.
    const kod = s
      .split("\n")
      .filter((l) => !/^\s*(\/\/|\*|\/\*)/.test(l))
      .join("\n");
    for (const name of ["Magnus", "Hikaru", "Nakamura", "Carlsen"]) {
      expect(kod, `имя живого гроссмейстера «${name}» попало в страницу задачи дня`).not.toContain(name);
    }
  });
