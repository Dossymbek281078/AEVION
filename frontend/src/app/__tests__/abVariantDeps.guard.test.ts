...
/**
 * Возвращает и найденное, и число реально прочитанных файлов.
 *
 * Второе — не диагностика, а часть контракта. Проверки `files.length` снаружи
 * недостаточно: она свидетельствует об обходе ДИСКА, но не о том, что цикл
 * ниже дошёл до конца. Ранний `return` внутри него вернул бы пустой список
 * нарушений, неотличимый от честного «всё чисто», — ровно та ошибка, которую
 * 28.07 пришлось ловить мутацией у соседнего сторожа scaleClaims. Со
 * `scanned` такой обрыв становится маленьким числом и падает снаружи.
 */
export function findVariantDepViolations(
  files: string[],
...
describe("A/B-вариант не стоит в зависимостях эффекта", () => {
  ...
  it("набор исходников непустой (сам сторож не должен молча проверять ноль файлов)", () => {
    ...
    // `continue` — проверка осталась зелёной, просмотрев один файл. Порог здесь
    // поднят до реального масштаба дерева (1800+ файлов), а не до символических 50.
    //
    // Эта проверка отвечает ровно за обход ДИСКА. За то, что цикл чтения файлов
    // дошёл до конца, отвечает `scanned` из findVariantDepViolations — размер
    // набора о содержимом цикла ничего не говорит.
  ...
  it("ни одна страница не ставит вариант в зависимости", () => {
    const { violations, scanned } = findVariantDepViolations(files);
    // Цикл внутри функции обязан дойти до конца: пустой список нарушений при
    // оборванном обходе выглядит точно так же, как чистый код.
    expect(scanned, "обход оборвался внутри функции — прочитано слишком мало файлов").toBeGreaterThan(500);
