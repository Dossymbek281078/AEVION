...
describe("тема обращения", () => {
  ...
  it("на экран не уходит сырое сообщение движка", () => {
    /*
     * Класс найден соседним окном на странице цен: покупателю показывали
     * внутренний адрес API и инструкцию запустить бэкенд. Здесь место хуже —
     * эта форма единственный работающий канал поддержки (у домена нет записи
     * MX), и пишет в неё тот, у кого уже что-то пошло не так. Технический текст
     * читается как «сломалось совсем» и отваливает человека окончательно.
     */
    expect(страница, "сообщение движка уходит на экран").not.toContain(
      "setError(e instanceof Error ? e.message",
    );
    expect(страница, "нет человеческой фразы").toContain("errorGeneric");
    expect(страница, "подробность потеряна вместо журнала").toContain("console.warn(");
  });
