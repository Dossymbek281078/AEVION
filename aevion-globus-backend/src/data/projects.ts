...
export const projects: GlobusProject[] = [
  ...
  },
  {
    ...
    description:
      "3D-модельер помещений из 2D-плана: DXF из AutoCAD -> 3D-модель в браузере с тремя слоями. Черновая отделка: разводка электрики по типовым нормам (розетки 0.3 м, выключатели 0.9 м у дверей, магистраль под потолком, щиток у входа) и труб (ХВС/ГВС, канализация с уклоном 2 см/м к стояку) — помечена черновиком для обсуждения с прорабом, не проектная документация. Чистовая: краска стен (6 цветов), пол (паркет/ламинат/плитка, процедурные текстуры), окна и двери в проёмах, светильники сеткой по габариту. Декор: каталог 21 предмета (гостиная, спальня, кухня, санузел, сплит-система и радиатор), перетаскивание мышью, поворот и сдвиг кнопками, экспорт кадра в PNG. Демо-квартира 8x6 м открыта сразу, без загрузки файла. Разбор DXF: LINE/LWPOLYLINE, единицы из $INSUNITS или по габариту — выбранная единица показывается человеку, а не угадывается молча. Честно: дуги/блоки DXF и растровые JPEG/PDF не разбираются (этап 2, страница говорит это прямо), проёмы из DXF не распознаются, авто-разводка — ориентир, а не проект. MVP 2026-09-07 — страница /qspace, бэкенда не требует.",
    kind: "product",
    status: "mvp",
    priority: 3,
