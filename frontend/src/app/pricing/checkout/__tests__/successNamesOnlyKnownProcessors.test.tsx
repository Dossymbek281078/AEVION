...
describe("сумма из адреса не показывается как принятый платёж", () => {
  test("контроль: экран отрисован", () => {
    query = "provider=paybox&total=5000";
    expect(текст().toLowerCase()).toContain("payment");
  });

  test("без подтверждения сумма на экран не выводится", () => {
    // `?total=5000` рисовало «Сумма: $50» рядом с заголовком «оплата
    // принята». Вместе это читается как «мы получили от вас 50 долларов» —
    // утверждение о нашей записи, которого сервер не подтверждал.
    query = "provider=paybox&total=5000";
    expect(
      текст(),
      "сумма из адреса показана как принятый платёж",
    ).not.toContain("$50");
  });
});
