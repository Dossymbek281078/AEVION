 ...
 *
 * Замеры 2026-07-27, Beginner при temperature 95, по 60 партий — ДВА прогона подряд:
 *   15% мусорных (e4×17, d4×14, Nf3×5, b4×4, g3×3, f4×3, Nc3×3, a4×2)
 *   20% мусорных (e4×17, d4×9,  Nf3×5, h4×4, Nc3×4, b4×3, f4×3, g3×3)
 * При temperature 40 прошлый замер давал 8%. Разброс между прогонами ~5 пунктов — на 60
 * партиях это порядка одной стандартной ошибки, поэтому в комментарии оба числа, а не
 * удобное. Во втором прогоне вернулся 1.h4 — тот самый ход, ради ухода от которого
 * температуру когда-то охлаждали.
 *
 * Хочешь убрать — поднимать `bookChance` (сейчас 0.80): книга даёт здоровые ходы, доля
 * мусора упадёт пропорционально. Но это поднимет и силу, поэтому после правки обязателен
 * ladder-замер.
 */
