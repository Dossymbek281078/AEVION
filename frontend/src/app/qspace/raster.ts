 ...
 *
 * 🔴 ГРАНИЦА ПО НАКЛОНУ, измеренная 13.09.2026 на повёрнутом прямоугольнике:
 *
 *     наклон 0-2°   4 стены из 4 — верно
 *     наклон 5°     8 отрезков вместо 4: каждая стена разваливается надвое,
 *                   а «толщина» раздувается с 3 до 13 точек (лестница из
 *                   коротких прогонов читается как толстая линия)
 *     наклон 10°    линий не найдено, и об этом сказано прямым текстом
 *
 * Опасен СРЕДНИЙ случай: при 5° результат неверен и ничем себя не выдаёт.
 * Детектор наклона намеренно НЕ написан: единственный доступный здесь признак —
 * раздутая толщина, а он неотличим от чертежа с настоящими толстыми стенами.
 * Сторож, кричащий на исправном чертеже, будет отключён в первый же день, и
 * защиты не останется вовсе. Поэтому граница названа человеку словами на
 * странице и здесь, а результат в любом случае проходит экран ручной правки:
 * человек видит найденные линии поверх своей картинки и убирает лишние.
