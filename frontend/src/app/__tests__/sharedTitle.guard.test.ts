...
function nearestTitle(pageDir: string): string {
  ...
  while (dir.startsWith(APP_DIR)) {
    ...
    if (existsSync(layout)) {
      ...
      // Только настоящий строковый литерал, закрытый запятой. `title: t("ключ")`
      // и шаблонные строки — вычисляемый заголовок: разбирать их регуляркой
      // значит выдать кусок кода за заголовок. Этот сторож так и сделал
      // 28.07.2026, сообщив про «один заголовок t( на 9 страниц».
      const literal = src.match(/title:\s*"([^"\\]+)"\s*,/);
      if (literal) return literal[1];
      if (/title:\s*(?:t\(|`)/.test(src)) return `__computed__:${relative(APP_DIR, dir)}`;
...
const KNOWN_DEBT: Record<string, number> = {
  ...
  // 9, а не 8: число выросло не потому, что раздел вырос, а потому что
  // разбор заголовков починен 28.07.2026 и стал видеть страницу, которую
  // раньше относил к вычисляемым. Это единственный случай, когда цифру
  // здесь допустимо поднять.
  "Сметный тренажёр · AEVION": 9,
};
