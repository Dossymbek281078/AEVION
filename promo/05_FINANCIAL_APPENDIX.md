# Financial Appendix — для CFO партнёра

> Приложение к `00_MASTER_PITCH.md`. Не полная финансовая модель — она строится после открытия дата-комнаты.
> Этот документ объясняет **экономику единицы (AEV)**, **источники выручки по столпам** и **базовые predicates** рыночного потенциала.
>
> **Структура сделки (единое предложение):** финансирование = **$10M возвратным авансом** + ресурсы партнёра (compute, инженеры, дистрибуция, бренд); доход проекта делится **51% основатель / 49% партнёр**. Это **партнёрство, не выкуп** — здесь нет оценки за процент и нет цены входа. Апсайд основателя = его **51% дохода растущего проекта**; апсайд партнёра = его 49%. Полные условия — `02_DEAL_TERMS.md`.
...
### Что это значит для экономики проекта
...
**$1.6B mid-case ARR к 2030.** При мультипликаторе 6-12× (SaaS-norm для high-growth, vertical-defensible) этот run-rate соответствует масштабу категории в $9.6B-$19.2B на 2030.

**Что это значит для партнёрства:** при таком потенциале доход проекта, делимый 51/49, становится трансформирующим для обеих сторон — но он приходит **по факту роста**, а вход партнёра — это ресурсы + возвратный аванс $10M, а не покупка доли по фиксированной цене.
...
Для понимания масштаба категории (НЕ для назначения цены входа — в партнёрстве цены за долю нет):
...
**Сложение по столпам (lower bound):** $250M + $1.5B + $1.6B + $1.6B = **$5B** — масштаб категории, в которой работает проект.

**Вывод:** потенциал категории — миллиарды. Именно поэтому модель сделки — **партнёрство с делением дохода 51/49**, а не разовый выкуп: обе стороны разделяют апсайд от роста, а вход партнёра — ресурсы + возвратный аванс $10M, без фиксации цены за долю.
