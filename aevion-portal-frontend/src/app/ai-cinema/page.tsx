...
export default function AiCinemaPage() {
  ...
  const load = async () => {
    try { setLoading(true);
      const res = await fetch(`${API_BASE}/api/media/submissions?type=cinema`, { headers: getAuth() });
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/media/submissions`, {
        ...
        body: JSON.stringify({ type: "cinema", title, description, fileDataUrl, mimeType: fileMimeType }),
      });
      const data = await res.json().catch(() => null);
  ...
  return (
    <div style={{ minHeight: "calc(100vh - 49px)", background: "linear-gradient(180deg, #fdf2f8 0%, #fce7f3 100%)" }}>
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "32px 20px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
          <div>
            <h1 style={{ fontSize: 32, fontWeight: 800, color: "#db2777", marginBottom: 4 }}>🎬 AI Cinema</h1>
            <p style={{ color: "#64748b", fontSize: 15, margin: 0 }}>Загрузка AI-фильмов и всемирное голосование</p>
          </div>
          <button onClick={() => setShowUpload(!showUpload)} style={{
            padding: "10px 20px", borderRadius: 10, border: "none", fontSize: 14, fontWeight: 600, cursor: "pointer",
            background: showUpload ? "#e2e8f0" : "linear-gradient(135deg, #db2777, #be185d)", color: showUpload ? "#475569" : "#fff",
          }}>
        ...
        {!token && (
          <div style={{ padding: "12px 16px", borderRadius: 12, background: "#fdf2f8", border: "1px solid #fbcfe8", color: "#9d174d", fontSize: 14, marginBottom: 20 }}>
            <Link href="/auth" style={{ color: "#db2777", fontWeight: 600 }}>Войдите</Link> чтобы загружать фильмы и голосовать
          </div>
        )}
        ...
        {showUpload && (
          <div style={{ background: "#fff", borderRadius: 16, padding: 24, border: "1px solid #e2e8f0", marginBottom: 24 }}>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: "#1e293b", marginBottom: 16 }}>Загрузить фильм</h2>
            <form onSubmit={submit} style={{ display: "grid", gap: 14 }}>
              <div>
                <label style={{ display: "block", fontSize: 13, fontWeight: 500, color: "#475569", marginBottom: 6 }}>Название</label>
                <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Название фильма" style={inputStyle} />
              </div>
              <div>
                <label style={{ display: "block", fontSize: 13, fontWeight: 500, color: "#475569", marginBottom: 6 }}>Описание</label>
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Синопсис / описание / жанр" rows={4} style={{ ...inputStyle, resize: "vertical" }} />
              </div>
              <div>
                <label style={{ display: "block", fontSize: 13, fontWeight: 500, color: "#475569", marginBottom: 6 }}>Видеофайл</label>
                <input type="file" accept="video/*" onChange={onFile} style={{ fontSize: 14 }} />
              ...
              <button type="submit" disabled={!canUpload || submitting} style={{
                ...
                background: canUpload ? "linear-gradient(135deg, #db2777, #be185d)" : "#e2e8f0", color: canUpload ? "#fff" : "#94a3b8",
              }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: 60 }}>
            ...
            <p style={{ color: "#94a3b8" }}>Пока нет загруженных фильмов. Будьте первым!</p>
          </div>
        ) : (
          <div style={{ display: "grid", gap: 12 }}>
            {items.map((it) => (
              <div key={it.id} style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid #e2e8f0" }}>
                ...
                {it.fileUrl && it.mimeType?.startsWith("video") && (
                  <video controls src={`${API_BASE}${it.fileUrl}`} style={{ width: "100%", marginTop: 12, borderRadius: 10, maxHeight: 400 }} />
                )}
                    ...
                    <div style={{ display: "flex", gap: 4 }}>
                      {[1, 2, 3, 4, 5].map((r) => (
                        <button key={r} onClick={() => vote(it.id, r)} disabled={votingId === it.id} style={{
                          width: 32, height: 32, borderRadius: 8, border: "1px solid #fbcfe8", background: "#fdf2f8", cursor: "pointer", fontSize: 14,
                        }}>
                          {r}
                        </button>
                      ))}
