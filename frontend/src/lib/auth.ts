...
//
// Остаётся `app/cyberchess/page.tsx`: он читает псевдонимы, но ПОСЛЕ
// канонического ключа, то есть работает и без них. Файл огромный и его
// правят несколько веток одновременно — трогать его ради уборки дороже,
// чем оставить.
//
// Условие снятия: `authTokenKey.baseline.json` не содержит cyberchess.
// Тогда удалить TOKEN_ALIASES, зеркалирование в setAuthToken,
// syncTokenAliases и его вызов в ClientProviders. Очистку в
// clearAuthToken оставить: у людей, входивших в это окно, псевдонимы
// лежат в браузере, и выход обязан их стирать.
