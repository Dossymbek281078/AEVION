...
const OWN_GATE = new Set([
  ...
  // ⚠️ 22.09.2026: у шахмат СВОЕГО механизма доступа НЕТ — в отличие от
  // остальных строк этого списка. Замер: по 12 файлам src/routes/cyberchess*.ts
  // ноль обращений к resolveUserPlan / isModuleEntitled / appSubscriptionState
  // (контроль: у devhub, который ведёт доступ своим тарифом, их 30; слово
  // "Tier" в cyberchessOpening.ts — про уровни данных дебюта, не про права).
  // То есть модуль ПРОДАЁТСЯ (база $24/мес, пять ступеней лестницы), а платит
  // человек за то, что и так открыто. Это не техническая недоделка, которую
  // можно закрыть правкой: решение «что именно даёт платная ступень шахмат»
  // принимает основатель. Строка остаётся исключением, чтобы сторож не был
  // вечно красным, но теперь она названа долгом, а не «устроено иначе».
  "cyberchess",
  "cyberchess-addon",
  "smeta-trainer",
  "qbuild",
