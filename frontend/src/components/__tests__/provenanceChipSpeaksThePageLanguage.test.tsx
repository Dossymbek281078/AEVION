...
describe("чип провенанса говорит на языке страницы", () => {
  ...
  it("подсказка ДЕЙСТВИТЕЛЬНО открыта к моменту сбора — контроль прибора", () => {
    // Соседнее окно предупредило о второй яме: click отрабатывает синхронно, а
    // перерисовка может произойти ПОСЛЕ — тогда сборщик честно читает дерево,
    // только не то, и тест зелен и на исправном, и на сломанном коде.
    //
    // Проверяем утверждением, а не рассуждением о том, что fireEvent обёрнут в
    // act: если однажды это перестанет быть правдой, здесь покраснеет.
    const { container } = render(
      <I18nProvider><DataProvenanceChip dataQuality={данные} /></I18nProvider>,
    );
    открытьПодсказки(container);
    expect(
      container.querySelector('[role="tooltip"]'),
      "подсказка не открылась — значит сбор идёт по ЗАКРЫТОМУ дереву и ничего не доказывает",
    ).not.toBeNull();
  });

  it("предмет проверки вообще на странице — иначе сторож зелен на пустоте", () => {
    const { container } = render(
      <I18nProvider><DataProvenanceChip dataQuality={данные} /></I18nProvider>,
    );
    // Признак берём ИЗ ФИКСТУРЫ, а не по памяти: 96 — это measuredPct, он обязан
    // оказаться на экране. Первым я взял значок 📊 и контроль справедливо
    // покраснел: значка в этом варианте чипа нет, он у соседнего компонента.
    // Контроль поймал не продукт, а меня — ровно за этим он и стоит.
    expect(container.textContent ?? "", "чипа на странице нет").toContain("96");
  });
