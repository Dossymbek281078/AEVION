...
function computeQRightSignature(contentHash: string) {
  const SIGN_SECRET =
    process.env.QSIGN_SECRET || "dev-qsign-secret-change-me-in-prod";
  return crypto
...
cyberchessRouter.post("/games", async (req, res) => {
  try {
    ...
    if (!wp || !bp) return res.status(400).json({ error: "players required" });
    if (wp === bp) {
    ...
    if (!white || !black) return res.status(404).json({ error: "Player not found" });
...
cyberchessRouter.get("/games/:id", async (req, res) => {
  try {
    ...
    if (!game) return res.status(404).json({ error: "Game not found", id });
...
cyberchessRouter.post("/games/:id/moves", async (req, res) => {
  try {
    ...
    let move: any = null;
    let uciToStore: string | null = null;
    ...
    if (chess.isCheckmate && chess.isCheckmate()) status = "checkmate";
    else if (chess.isDraw && chess.isDraw()) status = "draw";
    else if (chess.isStalemate && chess.isStalemate()) status = "stalemate";
    else if (chess.isGameOver && chess.isGameOver()) status = "game_over";
