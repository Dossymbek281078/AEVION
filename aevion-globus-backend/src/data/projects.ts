...
export const projects: GlobusProject[] = [
  ...
  },
  {
    ...
    description:
      "Навигационный слой городского неба для аэротакси: провайдер-независимые 3D-аэрокоридоры поверх реального цифрового двойника города — три города (Астана, Нью-Йорк, Токио), реальные здания из OpenStreetMap → поле высот 20 м. Движок A* с раскладкой по высотным полосам (нижняя кромка выше застройки + запас, растёт при низкой уверенности в высоте) + обход полигональных запретных зон + послойная ветровая модель (влияет на ETA) + Ed25519-подпись цифрового двойника. UI показывает скоринг пригодности вертипортов и живой рынок 4D-слотов прав QRight (capacity-гейт + SHA-256 receipt), слоты опционально персистятся в Postgres (in-memory fallback). Ограничения берутся у самих регуляторов: Нью-Йорк — сетка потолков FAA UAS Facility Map (загружена как есть, подписана Ed25519, привязана к Bitcoin через OpenTimestamps, сверяется с живым фидом каждые 12 ч); Токио — режим разрешений MLIT/JCAB (полёт над плотно населённым районом требует разрешения министра, 100% твина под режимом); Астана — открытого фида не найдено, и модуль честно говорит об этом, а не выдумывает данные. Наземный ветер — реальный METAR ближайшего аэропорта. Любой маршрут выдаётся подписанным документом-обоснованием для подачи регулятору. Честно: доказательство концепции, не сертифицированное авиационное ПО; UASFM — сетка допусков Part 107 для малых БВС, а не сертификация аэротакси; точечные запретные зоны и рост ветра с высотой остаются иллюстративными. MVP 2026-07-11 — /api/qskyway.",
    kind: "product",
    status: "mvp",
    priority: 3,
