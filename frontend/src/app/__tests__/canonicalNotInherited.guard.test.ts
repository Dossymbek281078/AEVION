 ...
 *
 * ⚠️ Искать этот образец списком по ГЛАГОЛУ в адресе (notarize, upgrade, log,
 * export, print, verify) можно, но список даёт КАНДИДАТОВ, а не находки: из
 * шести открытых настоящими оказались три, и одна из оставшихся —
 * qcoreai/upgrade — ПРОДАЮЩАЯ страница с тарифами и выбором «месяц/год».
 * Закрыть её от поиска значило бы спрятать то, ради чего платят. Два verify —
 * публичные проверялки чека и сертификата: на них приходят ИЗВНЕ, чтобы
 * убедиться, что бумага настоящая, и им показ нужен.
 *
 * Отличать по одному вопросу: страница ТРЕБУЕТ ВХОДА (Bearer, «нужен вход») —
 * закрывать, роботу видна пустая оболочка. Работает без входа и что-то
 * рассказывает — оставлять.
 *
