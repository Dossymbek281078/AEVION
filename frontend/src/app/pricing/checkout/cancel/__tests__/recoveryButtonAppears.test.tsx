...
describe("возврат к тарифу после отказа", () => {
  ...
  it("после отказа от покупки приложения кнопка тарифа планеты не появляется", async () => {
    // С 15.09.2026 приложение покупается на сроке, и в памяти лежит срок «lite».
    // Но /pricing/lite продаёт всю планету — кнопка повела бы не к той покупке.
    запомнитьНамерение("lite", "cyberchess");
    const ссылки = await ссылкиПри("paybox=1");
    expect(ссылки, "срок приложения выдан за тариф планеты").not.toContain("/pricing/lite");
  }, 60000);
