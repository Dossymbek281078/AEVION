 ...
 *  - появился НОВЫЙ такой обработчик → тест краснеет. Это твёрдое правило.
 *  - починили старый → тест ЗЕЛЁНЫЙ, но печатает, что пора убрать из `KNOWN`.
 *    Красным он тут быть не может: список заморожен по состоянию main, а
 *    починки приезжают отдельными ветками — иначе сразу после мержа main
 *    становился бы красным, что запрещено. Уборка `KNOWN` идёт следующим
 *    коммитом после мержа, по напечатанному списку.
 *
...
/**
 * Замороженный список на 28.07.2026 — состояние `main` на момент написания.
 * Каждая строка — обработчик, который ещё не разобран. Список обязан только
 * СОКРАЩАТЬСЯ.
 *
 * ЧТО БУДЕТ ПРИ МЕРЖЕ ВЕТОК С ИСПРАВЛЕНИЯМИ. Семь строк из списка перестанут
 * находиться, и тест «список не устарел» покраснеет, назвав их поимённо. Это
 * задумано: он требует вычеркнуть исправленное, иначе список превратится в
 * вечную свалку. Правка на семь строк, тест сам говорит, какие.
 *
 * Слепок специально сделан по `main`, а не «как будет после мержа»: иначе эта
 * ветка была бы красной в одиночку, а в main пускаем только зелёное.
 */
const KNOWN: string[] = [
...
describe("публичные ручки не отдают строку из базы целиком", () => {
  ...
  it("напоминает убрать из списка то, что уже починено", () => {
    const fixed = KNOWN.filter((k) => !found.includes(k));
    if (fixed.length > 0) {
      // Намеренно не падаем — см. пояснение в шапке файла.
      console.warn(
        `\n[сторож] починено ${fixed.length} — уберите из KNOWN, иначе список ` +
          `превратится в свалку:\n  ${fixed.join("\n  ")}\n`,
      );
    }
    // Проверяем то, что обязано выполняться всегда: список не пуст и состоит из
    // живых записей. Пустой KNOWN означал бы, что разбор сломался и сторож
    // молча перестал что-либо ловить.
    expect(KNOWN.length).toBeGreaterThan(0);
    expect(KNOWN.filter((k) => !/^[A-Za-z-]+\.ts /.test(k))).toEqual([]);
  });
});
