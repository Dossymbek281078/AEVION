...
//
// Замер 08.09.2026: копий репозитория со скриптом на машине одиннадцать, а
// реестр отправленных в git НЕ попадает (и не должен — там адреса людей).
// При process.cwd() запуск из другой копии не видел бы прежний реестр: если
// часть писем не влезла в суточный потолок Brevo (300) и на следующий день
// продолжили из соседней папки, получившие получили бы письмо ВТОРОЙ раз, а
// потолок сгорел бы впустую. Теперь реестр всегда один и тот же, откуда бы
// скрипт ни звали.
