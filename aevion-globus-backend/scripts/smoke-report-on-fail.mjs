...
// Имя ФАЙЛА и заголовок — по фактическому прогону, а не всегда «qskyway».
// 19.08.2026 обёртку впервые позвали с all-smokes, и отчёт вышел с шапкой
// «Локальный смоук QSkyway», хотя внутри стояло «Прогон: all-smokes».
// Инструмент, написанный под один случай и применённый ко всем, врёт именно
// в заголовке — в единственной строке, которую человек читает точно.
const slug = script.replace(/[^a-z0-9-]/gi, "-");
const REPORT = join(OUT_DIR, slug + "-FAILED.md");
...
child.on("exit", (code) => {
  ...
  writeFileSync(
    REPORT,
    `# Локальный смоук ${script}: ${code === 2 ? "не состоялся" : "падение"}\n\n` +
      `Время: ${now}\nПрогон: ${script}\n\n${what}\n\n` +
      "Эти проверки идут в режиме ЗАПИСИ — на проде их запускать нельзя,\n" +
      "значит другой проверки у покрытых ими путей нет.\n\n" +
      "## Хвост лога\n\n```\n" + tail + "\n```\n\n" +
