...
export function parseDxf(text: string): DxfResult {
  ...
  // Координата с ЗАПЯТОЙ вместо точки: parseFloat читает «8,5» как 8, то есть
  // стена молча становится короче на полметра, а смета — меньше на те же
  // проценты. По стандарту DXF разделитель всегда точка, поэтому чинить разбор
  // мы не станем: принимать запятую значило бы толковать чужой файл по догадке.
  // Но и молчать нельзя — это ровно тот случай, когда число выглядит
  // правдоподобным и является неверным.
  const КООРДИНАТЫ = new Set([10, 20, 11, 21, 30, 31]);
  const сЗапятой = ps.filter(([c, v]) => КООРДИНАТЫ.has(c) && v.includes(",")).length;
  if (сЗапятой > 0) {
    warnings.push(
      `В ${сЗапятой} координат(ах) десятичный разделитель — ЗАПЯТАЯ, а не точка. `
      + "Дробная часть таких чисел потеряна: «8,5» прочиталось как 8. "
      + "Сохраните чертёж заново с точкой в качестве разделителя, иначе размеры "
      + "и смета будут занижены.",
    );
  }
  ...
  } else if (wallSegs.length > 0) {
    // Слой НАЙДЕН, но отрезков на нём меньше четырёх — для замкнутой комнаты
    // этого мало, и брать только их опаснее, чем взять всё. Но сказать при
    // этом «слой не найден» было бы неправдой: человек пойдёт искать ошибку
    // в именовании слоёв, которой нет. Три разных случая — три разных
    // сообщения; прежде их было два, и одно описывало не своё состояние.
    const имена = [...new Set(wallSegs.map((s) => s.layer))].slice(0, 3).join(", ");
    warnings.push(
      `Слой стен найден (${имена}), но на нём всего ${wallSegs.length} отрезк(ов) — `
      + `для комнаты этого мало, поэтому взяты ВСЕ ${segs.length}. `
      + "Проверьте глазами: размерные линии могли стать «стенами».",
    );
  } else {
  ...
  if (blocks.length > 0) {
    warnings.push(
      ...
      + (recognised > 0
        // Допущение, которое иначе осталось бы невидимым. Точка вставки блока
        // берётся за СЕРЕДИНУ проёма — это верно для клика человека, но в
        // библиотеках блоков она часто стоит у КОСЯКА. Тогда каждая дверь
        // смещена на половину своей ширины: замерено — вставка на x=3 при
        // ширине 0.9 даёт отступ 2.55.
        //
        // Угадывать соглашение по одному файлу нельзя: в разных библиотеках
        // оно разное, и «поправить» вслепую значило бы сместить двери у тех,
        // у кого сейчас всё верно. Поэтому допущение названо, а не исправлено.
        ? ". Точка вставки блока считается СЕРЕДИНОЙ проёма; если в вашей"
          + " библиотеке она у косяка, двери сместятся на полширины — видно на"
          + " модели сразу"
        : "")
