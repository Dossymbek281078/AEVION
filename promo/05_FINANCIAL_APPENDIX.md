...
> Этот документ объясняет **экономику единицы (AEV)**, **источники выручки по столпам** и **базовые predicates**, на которых строится оценка ≈ **$1.05B** (единая цена $10.5M за 1%; инвестор выбирает глубину входа — см. `02_DEAL_TERMS.md`).
...
**Вход по оценке $1.05B сегодня = потенциал 10-20× за 4 года для покупателя — на любой выбранной доле, т.к. цена за процент постоянна.**
...
**Оценка $1.05B — это ~20% от lower-bound mark-to-market суммы по столпам.** Это оптимизация на **быстрый closing**, не «справедливая цена». Цена за 1% ($10.5M) постоянна — инвестор выбирает только объём, не торгует оценку.
