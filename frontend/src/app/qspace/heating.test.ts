...
describe("подпись шага называет ВЫБРАННЫЙ шаг", () => {
  // Прежде первые две ветки возвращали «10 см» и «15 см» при порогах 0.11 и
  // 0.16: шаг 12 см подписывался как десять. Сегодня кнопки дают только
  // 10/15/20/25, поэтому недостижимо, — но функция экспортирована и принимает
  // что угодно, а подпись, называющая не то значение, хуже её отсутствия.
  it("для каждого шага в подписи стоит его собственное число", () => {
    // ⚠️ Значения подобраны так, чтобы КАЖДАЯ ветка получила число, отличное
    // от вписанного в неё прежде. Первый список (10, 12, 15…) мутацию не
    // ловил: 12 уходит во вторую ветку, а первая проверялась только десяткой —
    // ровно тем числом, которое в ней и было записано.
    // 11 -> первая ветка, 13 -> вторая, 20 и 25 -> третья.
    for (const см of [10, 11, 13, 15, 16, 20, 25, 30]) {
      expect(stepHint(см / 100), `шаг ${см} см подписан не своим числом`)
        .toMatch(new RegExp(`^${см} см`));
    }
  });

  it("смысл сохранён: тёплый шаг и экономный названы по-разному", () => {
    // без этого «число совпало» неотличимо от «текст выродился в одно число»
    expect(stepHint(0.1)).toMatch(/теплее/);
    expect(stepHint(0.15)).toMatch(/жилая/);
    expect(stepHint(0.25)).toMatch(/прохладнее/);
  });
});
