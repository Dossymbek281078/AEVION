...
export default function Globus3D({
  ...
  useEffect(() => {
    ...
    // Прокрутка считается по ВСЕЙ цепочке предков и по элементу под пальцем,
    // поэтому «pan-y» на обёртке мало: значение на самом холсте решает не
    // меньше. У соседнего окна одна строка на обёртке НЕ подействовала —
    // их библиотека управления камерой ставила своё "none" прямо на canvas,
    // и замер после правки дал прежние 978 -> 978. Здесь такой библиотеки нет
    // (проверил: preventDefault только у колеса мыши и клавиатуры, у касаний
    // его нет), но пишу значение явно — чтобы намерение читалось в обоих
    // местах и не зависело от того, что поставит будущая библиотека.
    ...
    >
      <div
        ...
        style={{
          ...
          // 13.09.2026: было "none" — палец, попавший на холст, НЕ листал
          // страницу. Глобус на главной идёт во всю ширину, то есть на телефоне
          // это широкая полоса, об которую прокрутка останавливается: человек
          // упирается в неё и уходит, считая страницу сломанной.
          //
          // "pan-y" отдаёт браузеру ВЕРТИКАЛЬ (прокрутка страницы) и оставляет
          // нам ГОРИЗОНТАЛЬ (поворот глобуса). Размен осознанный: вертикальный
          // наклон пальцем теряется, прокрутка страницы важнее — с клавиатуры
          // и мышью наклон остаётся (см. aria-label выше: стрелки вращают).
          //
          // Класс принесло соседнее окно, у себя они замерили его с контролем:
          // свайп по холсту 978 -> 978, свайп рядом 978 -> 1183.
          touchAction: "pan-y",
          userSelect: "none",
        }}
      />
      ...
      {!initError ? (
        <div
          style={{
            ...
            zIndex: 5,
            // Полоса шире своего контейнера (замер 01.09.2026 на 1280: полоса
            // 571px, контейнер 420px), а центрирование выталкивает её за оба
            // края — справа она уходила за экран, и у главной появлялась
            // горизонтальная прокрутка: документ 1332 при окне 1280.
            //
            // Ограничение по ОКНУ (100vw) здесь не годится и было проверено:
            // ширина растёт, центр остаётся на контейнере, и элемент уезжает
            // ещё дальше. Ограничиваем по КОНТЕЙНЕРУ, содержимое прокручивается
            // внутри полосы.
            maxWidth: "100%",
            overflowX: "auto",
            display: "flex",
