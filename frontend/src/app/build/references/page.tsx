  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {r.recommend && (
              <span className="text-[10px] bg-emerald-900/40 text-emerald-400 px-2 py-0.5 rounded-full border border-emerald-800/40">
                {t("build.references.recommends")}
              </span>
            )}
          </div>
          {r.projectTitle && (
            <p className="text-xs text-slate-500 mt-0.5">
              {t("build.references.project")}{" "}
      ...
      <div className="flex items-center justify-between mt-3 text-xs text-slate-600">
        <span>{r.authorName && t("build.references.byAuthor", { authorName: r.authorName })}</span>
...
function MyReferencesContent() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="mb-6">
        <h1 className="text-xl font-bold">{t("build.references.title")}</h1>
        <p className="text-slate-400 text-sm mt-0.5">
          {t("build.references.subtitle")}
        </p>
      </div>

      {loading && <div className="text-center py-12 text-slate-500 text-sm animate-pulse">{t("build.references.loading")}</div>}

      {!loading && given.length === 0 && (
        <div className="text-center py-16 text-slate-500">
          <div className="text-4xl mb-3">📝</div>
          <p className="text-sm">{t("build.references.emptyTitle")}</p>
          <p className="text-xs text-slate-600 mt-2">
            {t("build.references.emptyHint")}
          </p>
          <Link href="/build" className="text-violet-400 underline text-sm mt-4 inline-block">{t("build.references.backToProjects")}</Link>
        </div>
      )}
