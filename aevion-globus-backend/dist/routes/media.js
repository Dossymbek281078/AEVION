...
function signContentHash(contentHash) {
...
mediaRouter.post("/submissions", async (req, res) => {
    try {
        ...
        const inputMimeType = typeof file?.mimeType === "string" ? file.mimeType : "application/octet-stream";
        if (!dataUrl)
...
mediaRouter.get("/submissions", async (req, res) => {
    try {
        ...
        const items = submissions.map((s) => {
            ...
            const avgRating = votesCount === 0 ? 0 : votes.reduce((a, b) => a + b, 0) / votesCount;
            return {
                id: s.id,
                type: s.type,
                title: s.title,
...
mediaRouter.post("/submissions/:id/vote", async (req, res) => {
    try {
        ...
        const rating = typeof ratingRaw === "number" ? ratingRaw : Number(ratingRaw ?? NaN);
        ...
        const votesCount = votes.length;
        const avgRating = votesCount === 0 ? 0 : votes.reduce((a, b) => a + b.rating, 0) / votesCount;
