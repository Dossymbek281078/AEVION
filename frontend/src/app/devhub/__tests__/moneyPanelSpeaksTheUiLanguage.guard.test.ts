 ...
 * «Уже оплатили?».
 *
 * ⚠️ ТОЧНАЯ СИЛА УТВЕРЖДЕНИЯ, перемерено после первой редакции этого файла.
 * Сперва написал «EN-покупатель видел русскую кнопку» — это СИЛЬНЕЕ замера.
 * Страница `/devhub` не входит в APP_PREFIXES, значит машинный доводчик её
 * наблюдает: MutationObserver + POST /api/i18n/translate, кэш в localStorage
 * по паре строка×язык. Поэтому честно так:
 *   • вернувшийся EN-визитёр с тёплым кэшем — видел английскую кнопку сразу;
 *   • первый заход — русскую, пока не вернётся сетевой вызов перевода;
 *   • вызов не удался (нет сети, отказ DeepL и Claude) — русскую навсегда.
 * То есть дефект был не «всегда», а «самая ценная строка продукта держалась на
 * сетевом вызове в момент, когда человек достаёт карту». Этого достаточно,
 * чтобы починить, и недостаточно, чтобы называть аварией.
 *
