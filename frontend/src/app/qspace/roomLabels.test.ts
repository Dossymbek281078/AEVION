...
describe("экспликация помещений: номера на плане ↔ строки таблицы", () => {
  ...
  it("номер в кружке: точка вне комнат, но комната рядом (до 0.6 м) — назначается; «02» штампа — не номер", () => {
    const лист3 = [эл("1", 900, 230), эл("Коридор", 920, 230), эл("6,45", 1000, 230), эл("2", 900, 213), эл("Санузел", 920, 213), эл("6,93", 1000, 213), эл("1", 300, 300), эл("02", 800, 20)];
    const экспл = экспликацияИзТекста(лист3);
    expect(номераНаПлане(лист3, экспл).map((x) => x.text)).toEqual(["1"]);
    // кружок радиусом 0.2 м вокруг (3.02, 3.03): внутри null, снаружи — комната 1
    const roomAt = (x: number, y: number) => (Math.hypot(x - 3.024, y - 3.028) < 0.2 ? null : 1);
    const r = назначенияПоНомерам(номераНаПлане(лист3, экспл), экспл, { x: 0, y: 0 }, 0.01, roomAt);
    expect(r.names).toEqual({ 1: "Коридор" });
  });
  it("строка легенды со строчной буквы — не экспликация; комната втрое больше чертежа — номер не назначается", () => {
    const лист4 = [эл("1", 900, 230), эл("Коридор", 920, 230), эл("6,45", 1000, 230), эл("2", 900, 100), эл("перегородки из бетонных блоков", 920, 100), эл("1", 300, 300)];
    const экспл = экспликацияИзТекста(лист4);
    expect(экспл).toEqual([{ n: 1, name: "Коридор", area: 6.45 }]);
    const roomAt = () => 1;
    expect(назначенияПоНомерам(номераНаПлане(лист4, экспл), экспл, { x: 0, y: 0 }, 0.01, roomAt, () => 226.9).names).toEqual({});
    expect(назначенияПоНомерам(номераНаПлане(лист4, экспл), экспл, { x: 0, y: 0 }, 0.01, roomAt, () => 5.3).names).toEqual({ 1: "Коридор" });
  });
