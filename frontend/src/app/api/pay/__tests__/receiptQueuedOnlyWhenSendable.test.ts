 ...
 *
 * ⚠️ РЯДОМ ЛЕЖИТ receiptQueuedIsHonest.test.ts (29.08) — про тот же дефект.
 * Он не дубль и не устарел: там намеренно проверен ТОЛЬКО случай без ключа,
 * потому что случай с ключом автор счёл требующим настоящего вызова почтового
 * провайдера («писем из проверок не шлём» — верная осторожность).
 *
 * Здесь эта осторожность снята подменой глобального fetch: письма не уходят
 * никуда, а проверить можно и успех, и отказ провайдера. Считать один из двух
 * файлов лишним не надо — второй проверяет то, чего первый не мог.
 *
