...
// Simple in-memory message board so the UI can demonstrate the Z-Tide concept
// (people sharing contributions / ideas around adaptive social-economic
// signals) without needing a new schema or auth. Capped to keep RAM bounded.

interface ZTideConceptMessage {
  id: string;
...
const ZTIDE_CONCEPT_MAX = 200;
const ztideConceptMessages: ZTideConceptMessage[] = [];
...
ztideRouter.get("/status", readLimit, (_req, res) => {
  res.json({
    ...
    conceptMessagesCount: ztideConceptMessages.length,
    timestamp: new Date().toISOString(),
  });
});

ztideRouter.get("/concept/messages", readLimit, (req, res) => {
  const limit = Math.min(Math.max(parseInt(String(req.query.limit ?? "20"), 10) || 20, 1), 100);
  const items = ztideConceptMessages.slice(0, limit);
  res.json({ items, total: ztideConceptMessages.length, moduleId: "z-tide", noun: "concept/messages" });
});

ztideRouter.post("/concept/messages", writeLimit, (req, res) => {
  try {
    ...
    const tagsRaw = Array.isArray(payload.tags) ? payload.tags : ["z-tide"];
    const tags = tagsRaw.map((t) => String(t).trim().slice(0, 30)).filter(Boolean).slice(0, 6);
    const msg: ZTideConceptMessage = {
      id: crypto.randomUUID(),
      payload: { idea, rationale, author },
      tags: tags.length ? tags : ["z-tide"],
      createdAt: new Date().toISOString(),
    };
    ztideConceptMessages.unshift(msg);
    if (ztideConceptMessages.length > ZTIDE_CONCEPT_MAX) {
      ztideConceptMessages.length = ZTIDE_CONCEPT_MAX;
    }
    res.status(201).json(msg);
  } catch (err: unknown) {
    console.error("[ztide] concept_post_failed", err instanceof Error ? err.message : err);
    res.status(500).json({ error: "concept_post_failed" });
  }
});

ztideRouter.get("/concept-stats", readLimit, (_req, res) => {
  const now = Date.now();
  const sevenDays = 7 * 86_400_000;
  const last7d = ztideConceptMessages.filter(
    (m) => now - new Date(m.createdAt).getTime() <= sevenDays,
  ).length;
  const tagCounts = new Map<string, number>();
  for (const m of ztideConceptMessages) {
  ...
  res.json({
    ...
    total: ztideConceptMessages.length,
    last7d,
    topTags,
  });
});
