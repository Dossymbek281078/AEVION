...
export function листПлана(текстыСтраниц: string[], линийНаСтранице: number[]): number | null {
  ...
  // Сперва лист, в чьём штампе стоит «обмерный»: это исходник, по которому чертят
  // остальные (OTDL, 45 листов: обмерный план — лист 4, а «план перегородок» — лист 12
  // с 3903 линиями против 3906, и правило «меньше линий» брало его). Среди равных —
  // с наименьшим числом линий: обмерный чище плана мебели и розеток.
  const обмерный = (t: string) => /обмерн/i.test(t);
  кандидаты.sort((a, b) => Number(обмерный(текстыСтраниц[b.k])) - Number(обмерный(текстыСтраниц[a.k])) || a.линий - b.линий);
