...
describe("A/B-вариант не стоит в зависимостях эффекта", () => {
  ...
  it(
    "ни одна страница не ставит вариант в зависимости",
    () => {
      expect(findVariantDepViolations(files)).toEqual([]);
    },
    // Обход всего дерева, а не юнит: при полном прогоне (54 файла параллельно)
    // на загруженной машине этот тест не укладывался в дефолтные 5 секунд и
    // краснел из-за очереди на диск, а не из-за находки. Правка не моя по
    // происхождению — трогаю потому, что красный без причины делает весь
    // набор нечитаемым. Та же мера стоит в retiredPrices и checkoutFunnel.
    30_000,
  );
