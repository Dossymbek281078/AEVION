...
## Доказано исполнением, а не только чтением (28.07, добавлено после)

Сначала вывод был сделан чтением. Чтобы не повторить сегодняшнюю ошибку — я уже
называл «публичной» соседнюю ручку, которая на самом деле требует токен, —
проверил прогоном: временный тест шлёт POST **без токена** и ждёт 401.

Фактический ответ: **503** «Lemon Squeezy not configured». То есть запрос без
токена спокойно прошёл все проверки обработчика и упёрся только в отсутствие
ключа в локальном окружении. Авторизации нет — подтверждено выполнением кода.

Отсюда важное для оценки срочности: **локально нас спасает лишь незаданный
ключ**. В проде, где `LEMON_SQUEEZY_API_KEY` задан, тот же анонимный запрос
пойдёт дальше и создаст настоящий checkout.

Временный тест удалён, в репозитории не оставлен: постоянный тест приложен ниже
и должен появиться вместе с патчем.
