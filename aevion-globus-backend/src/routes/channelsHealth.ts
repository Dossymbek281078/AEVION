...
channelsHealthRouter.get("/channels", (_req: Request, res: Response) => {
  ...
  const mail = {
    ...
    //
    // 🔴 ПОПРАВЛЕНО 23.09.2026. Признак смотрел ТОЛЬКО на SMTP и отвечал
    // «не настроено» на проде, где SMTP действительно нет, — а письма при этом
    // уходят через Resend. Прибор врал в обе стороны: я по нему решил, что
    // заявки с витрины не доходят до человека (они доходят), и едва не начал
    // чинить исправное. Теперь признак называет ТОТ канал, которым письмо
    // уйдёт на самом деле: сперва SMTP, иначе Resend.
    founderNotify: {
