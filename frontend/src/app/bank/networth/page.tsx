  ...
  return (
    <main
      style={{
        minHeight: "100vh",
        background:
          "radial-gradient(circle at 30% 0%, rgba(13,148,136,0.18), transparent 55%), linear-gradient(180deg, #0f172a 0%, #1e293b 60%, #0f172a 100%)",
        color: "#f8fafc",
    ...
    >
      <div style={{ maxWidth: 880, margin: "0 auto" }}>
        ...
        {loaded && !myAccount && (
          ...
          >
            <div style={{ fontSize: 16, fontWeight: 800, color: "#fff", marginBottom: 6 }}>{t("networthPage.empty.title")}</div>
            <div style={{ fontSize: 13, lineHeight: 1.6, maxWidth: 460, margin: "0 auto" }}>{t("networthPage.empty.body")}</div>
          </div>
        )}
        ...
        {myAccount && (
          <EcosystemDataProvider accountId={myAccount.id} operations={operations}>
            <div style={{ background: "#fff", color: "#0f172a", borderRadius: 16, padding: 4, marginBottom: 18 }}>
              <NetWorthTracker account={myAccount} operations={operations} />
            </div>
        ...
        >
          <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: 4, color: "#5eead4", textTransform: "uppercase" }}>
            {t("networthPage.cta.kicker")}
          </div>
          <div style={{ fontSize: 20, fontWeight: 900, marginTop: 6, lineHeight: 1.25 }}>{t("networthPage.cta.title")}</div>
          <div style={{ display: "flex", gap: 10, marginTop: 14, flexWrap: "wrap" }}>
            <Link href="/bank/forecast" style={ctaPrimary}>{t("networthPage.cta.forecast")} →</Link>
            <Link href="/bank/savings" style={ctaSecondary}>{t("networthPage.cta.savings")} →</Link>
          </div>
        </section>
