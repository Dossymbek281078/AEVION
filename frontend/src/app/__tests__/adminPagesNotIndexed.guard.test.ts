...
describe("страницы управления не показываются в поиске", () => {
  ...
  it("контроль охвата: обход видит и весь сайт, и раздел управления", () => {
    ...
    // Порог осмысленный, а не формальный: 30.08.2026 таких страниц 28 в семи
    // разделах (admin, build/admin ×7, qpaynet/admin ×7, constitution, pricing,
    // smeta-trainer). Ручной обход по признаку «требует входа» нашёл тогда 4 из
    // 7 страниц build/admin — адресный признак оказался вдвое полнее, и порог
    // взят от него. Число со временем растёт, поэтому нижняя граница не хрупкая.
