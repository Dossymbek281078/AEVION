...
/* ⚠️ ПЕРЕПРОВЕРЕНО 08.09.2026 — исключение УСТАРЕЛО и снято.
 *
 * Довод выше («единственный способ дать шапке канал — useSearchParams, а это
 * риск увести 762 статические страницы в динамику») был верен, когда его
 * писали. С тех пор появился KeepChannelLink: он подставляет метку В МОМЕНТ
 * КЛИКА, из window, — без useSearchParams, без Suspense и без динамического
 * рендера. Шапка на него уже переведена ЦЕЛИКОМ: пять ссылок через
 * KeepChannelLink, обычных внутренних Link/a — ноль (замер 08.09).
 *
 * То есть охрана была выключена для файла, который давно исправен. Список
 * прощённых пуст, и сторож снова работает на всём.
 *
 * Урок тот же, что и с «Источником ограничения» в этот день: оговорка в
 * списке исключений живёт дольше причины. Перепроверять её надо, открывая
 * файл, а не перечитывая объяснение. */
...
const LOSING_SINCE_2026_08_30 = new Set([
  // ✅ ВЫЧЕРКНУТО 16.09.2026: `app/devhub/page.tsx → /apps` метку больше НЕ
  // теряет. Починил владелец файла (та же правка, что на витрине: метка в
  // состоянии и keep у href) — именно то, на что рассчитывала записка выше.
  // Тест «долг не протух» назвал это место вылеченным и потребовал вычеркнуть;
  // убрана ровно одна строка, ничего не добавлено: список долга уменьшается по
  // факту починки, а не переписывается целиком.
  ...
  it("ни одна ссылка на них не потеряла метку", () => {
    ...
    for (const file of allFiles) {
      ...
      for (const t of targets) {
        ...
        // ЧЕТВЁРТАЯ форма, добавлена 08.09.2026: <KeepChannelLink href="/shop">.
        // Этот компонент подставляет метку В МОМЕНТ КЛИКА (из window), поэтому
        // литеральный href в нём — НЕ потеря. Сторож знал только функцию
        // keepChannel(...) и считал такую ссылку потерянной; ровно поэтому
        // шапка сайта висела в списке прощённых, хотя давно переведена
        // на KeepChannelLink целиком. Исключение прикрывало не дефект файла,
        // а слепоту проверки — а это худший вид исключения: оно молчит и
        // про настоящие потери в том же файле.
        const потеряна = forms.some((form) => {
          let from = 0;
          for (;;) {
            const at = src.indexOf(form, from);
            if (at < 0) return false;
            // тег, внутри которого стоит этот href
            const open = src.lastIndexOf("<", at);
            const tag = open >= 0 ? src.slice(open, at) : "";
            if (!tag.includes("KeepChannelLink")) return true;
            from = at + form.length;
          }
        });
        if (потеряна) lost.push(`${rel} → ${t}`);
      }
    }
