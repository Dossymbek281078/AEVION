...
| QReal (worktree `aevion-qreal`) | `aevion-globus-backend/src/routes/qreal.ts` (новый) + `routes/moduleManifest.ts` (append 1 mount) + `frontend/src/app/qreal/**` (новая) + `AEVION_QREAL_SPEC.md` | **QReal Studio P1**: «полностью живое» AI-видео без съёмки актёра. Бриф→AI-раскадровка→render-промты с директивами реализма→QC-чеклист 14 критериев→неотключаемый provenance-манифест. Демо «Утро в степи» засеяно. Ветка `feat/qreal-studio-mvp`. | 2026-07-21 UTC+5 |
...
|------|--------|-----|--------|-----------|
| `data/{fanDiscounts,pricing,lemonSqueezyVariants}.ts` + `routes/{pricing,checkout,provisioning,events}.ts` + `lib/{ownedModules,appSubscriptions,discountIntegrityLog}.ts` + `lib/payment/**` + `components/{FanDiscountPanel,ModulePricingChip,PaywallModal,PaywallScreen}.tsx` + `lib/pricingI18n*` | ✅ ГОТОВО К МЕРЖУ, ждёт руки основателя | `aevion-fan-pricing` (`feat/fan-discounts`) | **36 коммитов, PR не открыт.** Веерные скидки целиком (движок, 3 эндпоинта, панель, витрины, стена 402, оферта, аналитика) + **10 живых дефектов монетизации**, каждый класса «не падает, а тихо работает неправильно»: скидки не доходили до счёта на LS/Gumroad; промо считалось двумя способами; `/quote` отдавал 400 на тариф `pro`; окно веера обнулялось продлением; welcome-письмо уходило каждый месяц; покупатель Universe звался Lite; веер был слеп к `AppSubscription`; `SUBSCRIPTIONS_FILE` в тестах не работал (краснел тест и на main); панель залипала в тупике при ошибке базы; плейсхолдер `{cur}` уезжал в интерфейс. Гейты: backend tsc 0, frontend tsc 0, test:stable **1319 PASS**, frontend 484 PASS, `next build` зелёный (738 страниц), смоук 21/21, мутационный аудит 13/13. Обоснование — `docs/FAN_DISCOUNTS_2026-07.md`, описание мержа — `PR_BODY_fan-discounts.md`. ⚠️ Прогон сюиты — через `npm run test:stable`: при дефолтной параллельности под нагрузкой флакуют `devhub-integrations` и `tier3OgRoutes` (замерено: тот же коммит, разный результат). | 2026-07-26 16:05 UTC |
...
## Находка для владельца зоны QMelanin/QRenew (2026-07-26, сессия fan-pricing)

**Не правил — зона занята** (живые сессии `aevion-qrenew`, `aevion-aevia-checkout`).
Передаю диагноз, чтобы владелец починил у себя.

**Где:** `aevion-globus-backend/src/routes/qmelanin.ts:254` и `:359`
```ts
deficientKeys = b.deficientKeys.filter((k: string): k is BiomarkerKey => k in BIOMARKER_BY_KEY);
```

**Что не так:** `in` находит и УНАСЛЕДОВАННЫЕ свойства. `BIOMARKER_BY_KEY`
построен через `Object.fromEntries` (строка 64) — значит это обычный объект с
`Object.prototype`, и ключи `"constructor"`, `"__proto__"`, `"toString"`,
`"valueOf"`, `"hasOwnProperty"` проходят фильтр как «валидные биомаркеры».

**Последствие — не падение, а тихий мусор** (проверено прогоном):

| ключ | проходит фильтр | что уходит клиенту |
|---|---|---|
| `constructor` | да | `{}` в `targeted`, `[null]` в списке продуктов |
| `__proto__` | да | `{"foods":{}}` — объект вместо массива еды |
| `toString` | да | `{}` + `[null]` |

`BIOMARKER_BY_KEY[k].label` не бросает (у функции просто нет `.label`), поэтому
ответ HTTP 200 — и человек видит пустые карточки рекомендаций, а не ошибку.
`FOOD_SOURCES[k]` в `flatMap` (строки 266, 368) добавляет в список продуктов
`null`/объект.

**Тело запроса приходит от клиента** (`POST` с `deficientKeys: [...]`, док-строка
на 245), то есть ключи внешние — на дисциплину вызывающего опереться нельзя.

**Починка — одна строка на оба места:**
```ts
=> Object.prototype.hasOwnProperty.call(BIOMARKER_BY_KEY, k)
```
и тест: пять прототипных ключей дают пустой `targeted`, а реальный ключ
(`"copper"`) по-прежнему работает — без второй половины проверка ничего не значит.

**Откуда взялось:** тот же класс дефекта найден и починен в монетизации
(`feat/fan-discounts`): `currency: "constructor"` отдавал смету с `total: null`
...
### Ещё два отказа в тех же OG-роутах банка (2026-07-26, сессия fan-pricing)

Прогнал все три OG-роута, читающих `searchParams` (все — зона банка, не правил).
Кроме уже описанного `gift`, нашлись два в
`frontend/src/app/bank/share/[handle]/opengraph-image.tsx`:

**1. 🔴 Повтор query-параметра роняет рендер.** Сигнатура обещает
`searchParams?: { name?: string; ... }`, но Next при повторе параметра отдаёт
**массив**: `?name=a&name=b` → `["a","b"]`. Дальше
```ts
const name = (searchParams?.name ?? "").slice(0, 36) || handle;
```
`.slice` у массива работает, результат непустой и правдивый, поэтому фолбэк на
`handle` НЕ срабатывает — массив уходит в `initialsOf()`, где `s.replace(...)`
бросает `s.replace is not a function` (проверено прогоном). То же у `tier`
(`.toLowerCase()`) и `bio`.

**2. 🟠 `TIER_COLOR[tier] ?? "#cbd5e1"` — прототипный ключ без всякого `in`.**
`?tier=constructor` даёт функцию, а `??` её не ловит (функция — не `null`), и
она уходит в CSS как цвет. Здесь даже проверки `in` нет — достаточно голого
доступа по внешнему ключу.

**Общая починка для всех трёх роутов:** нормализовать `searchParams` на входе
одним хелпером — `const one = (v) => Array.isArray(v) ? v[0] : typeof v === "string" ? v : ""`
— и заменить словарные лукапы на `Object.prototype.hasOwnProperty.call`.
Тип в сигнатуре здесь не защищает: он описывает намерение, а не то, что
приходит из сети.

**Итог по классу (весь репозиторий прогрепан):** 6 мест — 3 в монетизации
(починены в `feat/fan-discounts`), 2 в банке и 1 в qmelanin (переданы владельцам),
плюс `aev/aevToken.ts` низкой важности. Внешний вход — у банковских: OG-ссылки
пересылают посторонним.

### `Math.max(parseInt(...), 1)` — обманчивое выражение, 8 мест (2026-07-26, сессия fan-pricing)

Нашёл в своём коде и прогрепал репозиторий. Выражение читается как «фолбэк плюс
пол», но фолбэка в нём нет:

```ts
Math.min(Math.max(parseInt(String(req.query.limit ?? "100"), 10), 1), 500)
```
`?limit=abc` → `parseInt` даёт `NaN` → **`Math.max(NaN, 1)` — это `NaN`, а не 1**
→ `.slice(0, NaN)` возвращает ПУСТОЙ массив. `?? "100"` не спасает: он ловит
только `undefined`, а не мусорную строку. У меня это показывало админу пустой
список лидов вместо ста записей (починено в `feat/fan-discounts`, `parseLimit()`).

**Важно, что большинство мест в репозитории БЕЗОПАСНЫ** — там после `parseInt`
стоит `|| 50`, и `NaN` как falsy ловится (`conceptBoardStore`, `bureau` ×2,
`devhub` ×3, `build/messaging`, `build/stats`, `events:254`). Трогать их не надо.

**Уязвимы ровно те, где `||` нет** — по одному владельцу:

| Файл | Строки | Владелец |
|---|---|---|
| `routes/events.ts` | 181, 182, 322, 366 | метрики/инфра |
| `routes/shadownet.ts` | 419, 467 | shadownet |
| `routes/deepsan.ts` | 95 | deepsan |
| `routes/qreal.ts` | 804 | QReal Studio |

Отдельно про **`qreal.ts:804`** — это разбор HTTP-заголовка `Range` при отдаче
видео:
```ts
const end = range[2] ? Math.min(parseInt(range[2], 10), stat.size - 1) : stat.size - 1;
```
Заголовок приходит от клиента, и `Range: bytes=0-abc` даёт `end = NaN`, который
уходит в поток. Это единственное место класса, где вход — заголовок, а не query,
и последствие стоит проверить отдельно (частичная отдача видео — не список).
...
**Уточнение к прогону смоуков 2026-07-27:** из 19 красных значительная часть —
не дефекты, а отсутствие локального Postgres. Проверено на `qcontract`:
`/api/qcontract/health` делает запрос к базе и честно отдаёт 503 с текстом
ошибки, когда её нет. Смоук ждёт 200, потому что рассчитан на среду с базой.
То же объясняет `qgood`, `qfusionai`, `ztide`, `qchaingov` и другие exit=1.
Прежде чем заводить по ним issue — поднять базу и перепроверить.

---

## 📋 Все находки для чужих зон собраны в один файл

Записи выше добавлялись по мере находок и разбросаны по документу. Сводка с
приоритетами, репро и готовыми патчами — **`HANDOFF_FINDINGS_2026-07-27.md`**
в корне worktree. Там же раздел «что проверено и дефектом НЕ является», чтобы
никто не тратил время повторно на 503 от qcontract, крах 0xC0000409 в смоуках и
права доступа Universe.
