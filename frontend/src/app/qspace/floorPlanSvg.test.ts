...
describe("чертёж сверху", () => {
  ...
  // С 13.09 в заголовок попадает ИМЯ ФАЙЛА, который выбрал человек, а не наша
  // строка. Значит экранирование стало защитой от чужого ввода, и его порядок
  // важен: амперсанд обязан заменяться ПЕРВЫМ. Переставь его после «<» — и
  // «&lt;» превратится в «&amp;lt;», то есть в чертеже человек увидит
  // «&lt;» буквами вместо своего названия.
  it("амперсанд в имени экранируется ОДИН раз, а не дважды", () => {
    const svg = floorPlanSvg(box(6, 4), { title: "план & смета <v2>" });
    expect(svg, "амперсанд не экранирован — это уже невалидный XML").toContain("&amp;");
    expect(svg, "двойное экранирование: порядок замен переставлен")
      .not.toContain("&amp;lt;");
