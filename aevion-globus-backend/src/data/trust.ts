...
export const TRUST_NUMBERS: TrustNumber[] = [
  ...
  // 14.09.2026: DevHub теперь в реестре и в этом счёте, но подписка Full его НЕ
  // открывает — тариф DevHub выдают только его собственные покупки
  // (upgradeDevHubByEmail в двух вебхуках), а getUserTierChecked не спрашивает
  // planGate. Подпись «в одной подписке» без оговорки стала бы ложью про один
  // модуль из 42. Входит ли DevHub в Full — решение основателя (состав пакета):
  // Desktop/АЕВИОН/00-НАЧНИ-ОТСЮДА/2026-09-14-РЕШИТЬ-DevHub-входит-ли-в-Full.md
  { label: "Модулей платформы", value: "42", hint: "в одной подписке (на Full), кроме DevHub Studio — он отдельно" },
