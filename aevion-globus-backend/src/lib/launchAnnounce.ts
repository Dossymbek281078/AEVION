...
> = {
  ...
  // ⚠️ ДОБАВЛЕНЫ 08.09.2026 — РАСХОЖДЕНИЕМ ДВУХ НАШИХ СПИСКОВ, не глазами.
  //
  // Письмо-подтверждение при подписке (constitutionBrevo.LAUNCH_MODULES) знает
  // ДЕВЯТЬ модулей и каждому обещает «напишем в день запуска». Здесь их было
  // ПЯТЬ. То есть подписчики qsign, qskyway, биржи стартапов и qventure —
  // четырёх модулей плана на 20 сентября — обещание получили, а письма о
  // запуске не получили бы: механизма для них не существовало.
  //
  // Это тот же класс, что нашли 31.08 в соседнем файле: там сверили письмо со
  // списком основателя и дописали три модуля. Починили ОДИН файл из двух.
  //
  // Даты здесь по-прежнему null — сознательно, как и у соседей выше. Опора для
  // «20 сентября» есть (план основателя 30.08 на 10 сентября, перенесённый им
  // же 13.09 на 20-е), но она названа в СОСЕДНЕМ
  // файле, а не проверена мной по документу; письмо без даты честно говорит
  // «модуль открыт», и это лучше уверенной даты со слов. Проставит тот, кто
  // откроет документ и сверит.
  //
  // Адреса ведут на страницы МОДУЛЕЙ: страниц /launch у этих четырёх нет, а
  // выдуманный адрес увёл бы человека из письма в 404. Все четыре проверены на
  // проде 08.09: 200, контроль выдуманным адресом даёт 404.
  qsign: {
    name: "AEVION QSign",
    date: null,
    dateSource: "",
    page: "/qsign",
    opens: "подпись документа и проверка подписи по публичной ссылке",
  },
  qskyway: {
    name: "AEVION QSkyway",
    date: null,
    dateSource: "",
    page: "/qskyway",
    opens: "маршрут по городскому небу с проверкой по опубликованным правилам",
  },
  startup: {
    ...
    opens: "заявки трёх уровней с названными условиями сделки и разбором цены",
  },
  qventure: {
    name: "AEVION QVenture",
    date: null,
    dateSource: "",
    page: "/qventure",
    opens: "разбор сделки по восьми факторам и совет из четырёх ролей",
  },
};
...
export function isGeneralWaitlist(source: string): boolean {
  const parts = String(source || "")
    ...
    .filter(Boolean);
  /*
   * `home` и `pricing` добавлены 09.09.2026, за сутки до рассылки.
   *
   * Замер: форма подписки стоит на двенадцати страницах, и две писали метку,
   * не попадавшую НИ ПОД ОДНО правило отбора — главная (`home`) и страница цен
   * (`pricing`). Обе показывают текст компонента по умолчанию, а он обещает
   * дословно: «Оставьте адрес — напишем в день запуска того, что вам ближе».
   *
   * Ошибки при рассылке не было бы вовсе: она отработала бы, отчиталась числом
   * и записала файл списка. Этих людей просто не оказалось бы в нём — тихая
   * потеря аудитории ровно там, где мы её собираем.
   *
   * Это ВТОРОЙ раз: 28.08 то же нашли у метки `go` и починили одной строкой,
   * но остальные одиннадцать форм тогда не пересмотрели.
   *
   * Проверено КАЖДОЙ меткой, а не аналогией. Прогон всех живых меток через эти
   * же функции дал ещё кандидатов, и почти все законны:
   *   `qpaynet-deposit`, `qrenew` — модули, которых нет среди девяти в письме;
   *   `en-go`, `longevity`, `en-longevity` — исключены осознанно (письмо только
   *     по-русски; у книги своё обещание);
   *   `shop` — НЕ форма подписки, а метка канала у кнопки покупки. Добавить её
   *     значило бы написать тем, кто ни на что не подписывался.
   */
  return parts.some(
    (p) => p === "go" || p.startsWith("go-") || p === "home" || p === "pricing",
  );
}
