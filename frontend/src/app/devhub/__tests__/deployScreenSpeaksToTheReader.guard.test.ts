...
describe("экран выкатки говорит на языке читателя", () => {
  ...
  test("«неудача» с выданным адресом объясняется человеку", () => {
    // Замер прода 08.09.2026: у четырёх записей статус failed, а их адреса
    // отвечают 200 до сих пор — проверка ждала 25 секунд, новый Pages-проект
    // поднимается дольше. Окно расширено, но СТАРЫЕ записи останутся с прежним
    // статусом, и человек прочитает «не вышло» о работающем сайте.
    expect(PAGE, "условие исчезло — экран снова молчит о живом адресе").toContain(
      'selectedDeployment.status === "failed" && selectedDeployment.deployUrl',
    );
    expect(PAGE).toContain("DL.failedButUrl");
    for (const язык of ["ru", "en", "kk"]) {
      const блок = PAGE.slice(PAGE.indexOf(`  ${язык}: {`), PAGE.indexOf("},", PAGE.indexOf(`  ${язык}: {`)));
      expect(блок.includes("failedButUrl:"), `в языке ${язык} нет объяснения`).toBe(true);
    }
  });

  test("объяснение зовёт ОТКРЫТЬ ссылку, а не просто извиняется", () => {
    // «Что-то пошло не так» не говорит человеку, что делать. Здесь есть
    // действие: открыть адрес и посмотреть самому.
    const ru = PAGE.slice(PAGE.indexOf("  ru: {"), PAGE.indexOf("},", PAGE.indexOf("  ru: {")));
    expect(/откройте ссылку/i.test(ru), "в русском тексте нет действия для человека").toBe(true);
  });
});
