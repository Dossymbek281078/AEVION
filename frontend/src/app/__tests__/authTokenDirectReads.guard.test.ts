// Храповик прямых чтений токена. Пришёл из ветки fix/pitch-one-denominator
// и живёт отдельным файлом от authTokenKey.guard.test.ts намеренно: тот
// сторож отвечает на вопрос «читают ли мёртвый ключ», этот — на вопрос
// «не разрастается ли прямое чтение в обход lib/auth». Предметы разные,
// вспомогательные функции у обоих свои, и слияние в один файл потеряло бы
// половину проверок ради косметики.
