...
#### Обязательно для QSign v2 (новая платформа подписей)

| Имя переменной | Значение |
|----------------|----------|
| `QSIGN_HMAC_V1_SECRET` | 64-символьный hex (32 байта). **Сгенерируйте**, не придумывайте. |
| `QSIGN_ED25519_V1_PRIVATE` | 64-символьный hex (Ed25519 seed). **Сгенерируйте**. |
| `QSIGN_ED25519_V1_PUBLIC` | Опционально: 64-hex public key. Если не задан — backend выводит из private. |

**Как получить значения** (один раз, локально):

```bash
cd aevion-globus-backend
npm run qsign-v2:gen-prod-secrets
```

Скрипт **выведет в ваш терминал** свежие три значения. Скопируйте каждое в отдельную переменную в Railway (`Variables` → `New Variable`).

> ⚠️ **Backup в пароль-менеджер ДО вставки в Railway.** Эти секреты подписывают каждую сигнатуру. Если потеряете — все ранее подписанные документы становятся неверифицируемыми (восстановления нет). Если когда-нибудь нужна ротация — используйте `POST /api/qsign/v2/keys/rotate` (admin), старый ключ останется в registry как `retired` и будет вечно валидировать историю.
>
> ⚠️ **Не используйте dev-значения из `aevion-globus-backend/.env`** — они в gitignore, но всё равно слабые. Только свежесгенерированные через `qsign-v2:gen-prod-secrets`.

#### Обязательно для аттестаций QSkyway и страницы Trust

| Имя переменной | Значение |
|----------------|----------|
| `QSKYWAY_SIGN_SK` | Ed25519-ключ в base64 (PKCS8 DER). **Сгенерируйте**, не придумывайте. |

Без неё ключ подписи **генерируется заново при каждом старте процесса**. Тогда
подпись цифрового двойника и значения на странице Trust доказывают только, что
их подписал ЭТОТ запуск минуту назад: связи со вчерашним состоянием нет, и
внешний человек ничего проверить не может.

Проверить, что стоит сейчас: `GET /api/qskyway/verify?city=astana` — поле
`ephemeral` и пояснение `keyNote` рядом с вердиктом. На странице модуля рядом с
«✓ подпись верна» появляется «· ключ временный».

Как получить значение (один раз, локально):

```bash
node -e "console.log(require('crypto').generateKeyPairSync('ed25519').privateKey.export({type:'pkcs8',format:'der'}).toString('base64'))"
```

> ⚠️ Вставляйте одной строкой, без переводов строки. Повреждённое значение
> модуль не примет и уйдёт на временный ключ — но скажет об этом прямо
> (`ephemeral: true` и предупреждение в логе), а не сделает вид, что всё в
> порядке.

Для legacy QSign v1 (используется `pipeline.ts` и `planetCompliance.ts`):

| `QSIGN_SECRET` | Длинная случайная строка. Если уже задано в проде — **не меняйте**, иначе старые HMAC-подписи в IPCertificate перестанут совпадать. |
