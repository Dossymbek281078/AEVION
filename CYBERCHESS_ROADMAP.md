...
### 🐞 Найдено при браузерном тестировании (2026-06-22) — ПРИОРИТЕТ

**Панель «Варианты» (engine lines / multiPV) в анализе НЕ работает — висит на «Analyzing…».**
Это claimed killer-фича и она сломана. Стрелка лучшего хода (PR #399) тоже не показывается по той же причине.

**Корень:** класс `SF` использует ОДИН Stockfish-воркер и общие колбэки `this.cb/ecb/mpvCb`.
`multiPV()` ставит `this.cb=null`, а `eval()` ставит `this.mpvCb=null` — взаимное обнуление.
В анализе авто-`runMultiPV` (effect ~4600) и live-eval (effect ~2176) стартуют почти одновременно
и `stop`+`go` друг друга → ни один не доходит до `bestmove` → колбэк не срабатывает, `mpvLines` пуст.
Одиночный eval-бар работает (он «выигрывает» воркер), а multiPV — нет.

**Проверено в браузере:** SAB активен (`crossOriginIsolated=true`), воркер грузится («ready»),
но кнопка «▶ Analyze» зависает на «Analyzing…» бесконечно. Не регрессия — состояние было таким и на main.

**Фикс (нужен аккуратно + браузерный тест, НЕ вслепую):**
- Вариант A (по-lichess): отдельный выделенный Stockfish-воркер под анализ/multiPV, изолированный
  от live-eval. Чисто, но +память и второй wasm-load.
- Вариант B (минимальный): в анализе НЕ гонять live single-eval — брать eval из `multiPV[0]`
  (тогда контеншна нет, multiPV доходит до bestmove). Риск: eval-бар и коуч в анализе читают
  `evalCp` от live-eval — нужно переключить их источник на multiPV.
- Осторожно: от общего `eval()` зависят eval-бар, коуч, классификация ходов (blunder/!!) — менять с тестом всех потоков.
