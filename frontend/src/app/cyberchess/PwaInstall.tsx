    ...
    <>
      <div role="region" aria-label="Установка приложения" style={banner}>
        ...
        <ul style={bulletsList}>
          ...
          <li style={bulletItem}>
            <span aria-hidden style={{ color: '#34d399' }}>🔔</span>
            {/* Обещание приведено к устройству. 19.08.2026 проверено: у
                service worker НЕТ обработчиков push и periodicsync (только
                install/activate/fetch/message), а напоминание крутится
                setInterval-ом на открытой странице. Напоминание, требующее уже
                быть в приложении, не может в него вернуть — прежний текст
                обещал именно возврат.

                Настоящая починка — Web Push: нужен VAPID-ключ и рассылка со
                стороны сервера. Это работа, а не формулировка, поэтому вынесена
                решением, а обещание пока сказано честно. */}
            <span>Напоминание о задаче, пока приложение открыто</span>
          </li>
        </ul>
