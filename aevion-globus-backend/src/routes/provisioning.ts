...
export interface Subscription {
  ...
  /**
   * НАШ идентификатор намерения оплаты (тот, что уходит в кассу как
   * custom.bureauIntentId и возвращается вебхуком).
   *
   * Зачем отдельным полем, а не вместо providerPaymentId: последний хранит
   * идентификатор КАССЫ и используется для сверки с провайдером — подмена
   * сломала бы сверку. А страница успеха спрашивает именно наш: она получает
   * его в адресе возврата и не знает идентификаторов кассы.
   *
   * 🔴 Замер 22.09.2026: у Lemon Squeezy этот идентификатор уходил в кассу и
   * терялся на обратном пути — боевой маршрут вебхука его не читал. Поэтому
   * /checkout/status не находил выдачу НИКОГДА, и экран после оплаты у
   * каждого покупателя оставался на «оплата принята» вместо «тариф
   * активирован». Выдача при этом проходила: молчал не механизм, а связь.
   */
...
export function findSubscriptionByPaymentId(
  ...
  for (const line of content.split(String.fromCharCode(10))) {
    ...
    try {
      ...
      const точное = sub.providerPaymentId === цель || sub.bureauIntentId === цель;
