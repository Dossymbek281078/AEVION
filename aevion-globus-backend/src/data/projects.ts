...
export const projects: GlobusProject[] = [
  ...
  },
  {
    ...
    description:
      "3D-модельер помещений из 2D-плана прямо в браузере. Вход — три формата: DXF из AutoCAD (LINE/LWPOLYLINE, единицы из $INSUNITS), ВЕКТОРНЫЙ PDF из AutoCAD/Revit (разбор операторов m/l/re и распаковка FlateDecode средствами платформы, без внешних библиотек) и РАСТРОВЫЙ план — JPEG/PNG/скан, где стены находятся обходом пикселей, а результат обязательно проходит экран ручной правки: линии рисуются поверх исходной картинки, лишнее снимается кликом. Масштаб у PDF и картинки не угадывается, его называет человек — иначе вышла бы правдоподобно неверная модель. Дальше модель показывается тремя переключаемыми слоями. Черновая отделка: разводка электрики по типовым нормам (розетки 0.3 м, выключатели 0.9 м у дверей, магистраль под потолком, щиток у входа), трубы ХВС/ГВС и канализация с уклоном 2 см/м к стояку, плюс состав конструкций — пирог пола 80 мм, перегородка на выбор (газоблок 123 мм или каркас ГКЛ 103 мм с кабелями внутри), потолок 61 мм, и главное число: 2.70 м в бетоне превращается в 2.56 м после ремонта. Чистовая: каталог 14 материалов (паркет, ламинат, плитка, бетон, шесть красок, обои, стеновая плитка), окна и двери в проёмах, светильники сеткой. Декор: 21 предмет мебели и оборудования, включая кухонный гарнитур, сантехнику, сплит-систему и радиатор, с перетаскиванием мышью. Спецификация материалов считается ИЗ модели и меняется вместе с планом: площади, литры краски, метры кабеля и труб, счёт розеток и светильников. Честно: разводка и толщины — черновик по практике ремонта для разговора с прорабом, а не проектная документация под подпись; распознавание растра даёт предположение, которое человек обязан проверить; проёмы (окна и двери) из чертежа пока не распознаются. Бэкенд модулю не нужен, всё считается в браузере. MVP 2026-09-07 — страница /qspace.",
    kind: "product",
    status: "mvp",
    priority: 3,
