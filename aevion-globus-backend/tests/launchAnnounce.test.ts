...
/**
 * Сказуемое согласовано с ИМЕНЕМ модуля, а не написано один раз на всех.
 *
 * 🔴 Замер 08.09.2026: тема письма для биржи читалась «Биржа стартапов
 * открыт» — единственное русское имя женского рода среди девяти. Это первое,
 * что подписчик видит в ящике, и правит его не корректор, а мы.
 *
 * Проверка идёт по ВСЕМ модулям, а не по списку известных имён: следующий
 * добавленный модуль иначе проедет так же тихо. Прежние двенадцать сторожей
 * письма слово «открыт» упоминали одиннадцать раз, а «Биржа» — ни разу.
 */
describe("письмо запуска согласовано с именем модуля", () => {
  const РУССКОЕ_ИМЯ = /[А-Яа-яЁё]/;

  test("прибор видит предмет: модулей много и среди них есть русские имена", () => {
    const всего = Object.keys(LAUNCH_MODULES).length;
    expect(всего, "список модулей пуст — сторож проверял бы пустоту").toBeGreaterThan(5);
    const русские = Object.values(LAUNCH_MODULES).filter((m) => РУССКОЕ_ИМЯ.test(m.name));
    expect(русские.length, "русских имён нет вовсе — проверять согласование не на чем").toBeGreaterThan(0);
  });

  test("у русского имени сказуемое задано явно", () => {
    // Латинским именам умолчание «открыт» не мешает: род там не читается.
    // Русскому имени умолчание может не подойти — и это должно быть решением,
    // а не случайностью.
    for (const [slug, m] of Object.entries(LAUNCH_MODULES)) {
      if (!РУССКОЕ_ИМЯ.test(m.name)) continue;
      expect(
        typeof m.openedWord === "string" && m.openedWord.length > 0,
        `${slug} («${m.name}»): русское имя без openedWord — письмо скажет «${m.name} открыт». ` +
          "Задайте согласованное слово рядом с именем.",
      ).toBe(true);
    }
  });

  test("слово доезжает до всех трёх мест письма", () => {
    const письмо = buildLaunchEmail("startup", "probe@example.com");
    expect(письмо.subject, "тема письма не согласована").toContain("Биржа стартапов открыта");
    expect(письмо.htmlContent, "заголовок внутри письма не согласован").toContain("Биржа стартапов открыта");
    expect(письмо.textContent, "текстовая часть не согласована").toContain("Биржа стартапов открыта");
    // И контроль в другую сторону: у латинского имени умолчание сохранилось.
    const шахматы = buildLaunchEmail("cyberchess", "probe@example.com");
    expect(шахматы.subject).toContain("CyberChess открыт");
  });
});


/**
 * Метки форм подписки, которым письмо ОБЕЩАНО, попадают в отбор.
 *
 * 🔴 Замер 09.09.2026, за сутки до рассылки: форма стоит на двенадцати
 * страницах, и две писали метку вне всех правил — главная (`home`) и страница
 * цен (`pricing`). Обе показывают текст компонента по умолчанию: «напишем в
 * день запуска того, что вам ближе». Рассылка отработала бы, отчиталась числом
 * и записала файл — этих людей просто не было бы в списке.
 *
 * Проверка держит ОБЕ стороны. Односторонняя («home попадает») позеленела бы и
 * на правиле «попадает всё», а тогда письмо ушло бы тем, кто ни на что не
 * подписывался.
 */
describe("общая очередь: кому обещали письмо — тот в отборе", () => {
  test("метки форм с обещанием письма попадают", () => {
    for (const метка of ["go", "go-ig", "home", "pricing"]) {
      expect(isGeneralWaitlist(метка), `метка ${метка} выпала из отбора`).toBe(true);
    }
  });

  test("метки БЕЗ такого обещания не попадают — это граница, а не придирка", () => {
    // shop — вообще не форма подписки, а метка канала у кнопки покупки;
    // en-go — письма по-английски нет; longevity — у книги своё обещание;
    // qrenew и qpaynet-deposit — модули, которых нет среди девяти в письме.
    for (const метка of ["shop", "en-go", "longevity", "en-longevity", "qrenew", "qpaynet-deposit"]) {
      expect(
        isGeneralWaitlist(метка),
        `метка ${метка} попала в общую очередь — письмо уйдёт тому, кому его не обещали`,
      ).toBe(false);
    }
  });
});
