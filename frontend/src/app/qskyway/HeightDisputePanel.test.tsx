...
describe("HeightDisputePanel", () => {
  ...
  test("тот же компонент говорит по-русски, когда язык русский", async () => {
    ...
    try {
      ...
      // Ждём НУЖНЫЙ ЯЗЫК, а не «что-нибудь появилось».
      //
      // 01.09.2026, подсказка окна запуска: при разбитом по языкам словаре
      // провайдер сперва отдаёт английский, а язык посетителя приезжает
      // динамическим import(). Ожидание непустоты наступает РАНЬШЕ — на
      // английском тексте, — и тест читает "segments 3" вместо "участков 3".
      //
      // Признак, отличающий этот случай от настоящей поломки: не пусто, а
      // ЧУЖОЙ язык. Поэтому условие ожидания — кириллица в подписи.
      await waitFor(() => {
        const t = screen.getByTestId("height-dispute").textContent ?? "";
        expect(/[а-яА-Я]/.test(t), "подпись всё ещё не по-русски: " + t.slice(0, 60)).toBe(true);
      }, { timeout: 10000 });
