...
describe("расхождение по высоте — коридор поднят числом, которому мы не верим", () => {
  ...
  /**
   * Спорная высота появляется в твине раньше, чем до неё доходит человек:
   * пометку ставит пересборка (12.08.2026 дрейф OSM привёз в Астану пять новых
   * домов), а разбор по опубликованным данным — отдельная работа. До этой
   * правки на место неизвестного публиковался ноль и прочерк, и карточка
   * печатала «382 м в теге OSM против 0 м в статье объекта» — правдоподобное
   * число вместо «не знаем», ровно та подмена, которую модуль ищет в чужих
   * данных. Сегодня на живых городах не выстреливает: единственная спорная
   * высота, доходящая до коридоров, разобрана.
   */
  test("без разбора отвечает «не знаем», а не нулём", () => {
    const twin = twinWithTowerOnTheOnlyPath();
    // то же здание, но разбора по нему нет: другой индекс и другой элемент OSM
    twin.buildings[100] = { ...twin.buildings[194] };
    twin.buildings[194] = { h: 10, hs: 0, r: [[0, 0], [1, 0], [1, 1], [0, 1]] };
    twin.dataQuality.suspect = [{ i: 100, h: 382, osm: "way/999000111", times: 4.66, why: "towers over the city" }];

    const route = __engineForTests.buildRoute("astana", twin, 0, 1, false);
    const d = __engineForTests.heightDisputeFor("astana", twin, route!)!;
    expect(d.building).toBe(100);
    // Тег известен всегда — он и поднял коридор.
    expect(d.taggedM).toBe(382);
    // А опубликованного числа нет. Ноль здесь означал бы «в статье объекта
    // опубликовано 0 м», и отличить его от «разбора нет» было бы нечем.
    expect(d.publishedM).toBeNull();
    expect(d.publishedSource).toBeNull();
    // Элемент источника называем даже без разбора: по нему высоту можно
    // проверить и завести замечание — иначе находка никуда не ведёт.
    expect(d.osm).toBe("way/999000111");
    // Сравнивать не с чем, значит и цены расхождения нет — молчим, а не считаем.
    expect(d.cruiseAltMIfPublished).toBeNull();
    expect(d.cruiseDeltaM).toBeNull();
  });
