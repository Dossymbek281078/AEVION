/* Chess piece sets.
   Default: Cburnett SVG (CC BY-SA 3.0, https://commons.wikimedia.org/wiki/Category:SVG_chess_pieces)
   Alternatives: Unicode-based sets (Neon, Minimal, Bold) — minimal bytes, distinct look. */
...
type PieceColor = "w"|"b";
type Props = { type: PieceType; color: PieceColor; size?: number|string; setOverride?: PieceSetId };

export type PieceSetId = "cburnett" | "neon" | "minimal" | "bold";
...
export function getActivePieceSet(): PieceSetId {
  ...
  try {
    const v = localStorage.getItem(LS_KEY) as PieceSetId|null;
    if (v && PIECE_SETS.some(s=>s.id===v)) return v;
  } catch {}
  return "cburnett";
}

export function setActivePieceSet(id: PieceSetId) {
  try { localStorage.setItem(LS_KEY, id); } catch {}
  if (typeof window !== "undefined") window.dispatchEvent(new Event("aevion-pieceset-change"));
}

export function useActivePieceSet(): PieceSetId {
  const [s, set] = useState<PieceSetId>(()=>getActivePieceSet());
  useEffect(()=>{
    const handler = ()=>set(getActivePieceSet());
    window.addEventListener("aevion-pieceset-change", handler);
    window.addEventListener("storage", handler);
    return ()=>{
      window.removeEventListener("aevion-pieceset-change", handler);
...
const CBURNETT: Record<string,string> = {
...
function GlyphPiece({ type, color, size, fill, stroke, glow, weight, font }:{
  ...
  const g = GLYPH[type];
  const c = color==="w"?fill.w:fill.b;
  const s = color==="w"?stroke.w:stroke.b;
  return (
    <div style={{
      ...
      <span style={{
        fontSize:typeof size==="number"?Math.round(size*0.92):"92%",
        lineHeight:1,
        fontWeight:weight,
        fontFamily:font||"'Noto Sans Symbols 2','Segoe UI Symbol','Apple Symbols',sans-serif",
        color:c,
...
export default function Piece({ type, color, size = "100%", setOverride }: Props) {
  const fromHook = useActivePieceSet();
  const active = setOverride || fromHook;
  const sz = size;
  ...
  if (active === "cburnett") {
    ...
    return (
      <svg viewBox="0 0 45 45" width={sz} height={sz} xmlns="http://www.w3.org/2000/svg"
  ...
  if (active === "neon") {
    return <GlyphPiece type={type} color={color} size={sz}
      fill={{ w:"#e0f2fe", b:"#1e1b4b" }}
      stroke={{ w:"#22d3ee", b:"#7c3aed" }}
      glow={color==="w"
  ...
  if (active === "minimal") {
    return <GlyphPiece type={type} color={color} size={sz}
      fill={{ w:"#fafafa", b:"#0f172a" }}
      stroke={{ w:"#475569", b:"#0f172a" }}
      weight={500}
      font="'Noto Sans Symbols 2','Segoe UI Symbol',sans-serif"/>;
  }

  // bold
  return <GlyphPiece type={type} color={color} size={sz}
    fill={{ w:"#ffffff", b:"#000000" }}
    stroke={{ w:"#000000", b:"#000000" }}
    weight={900}
