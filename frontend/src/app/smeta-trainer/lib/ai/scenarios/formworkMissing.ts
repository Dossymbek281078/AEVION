 ...
 *
 * Триггер: позиция по УСТРОЙСТВУ монолитной ж/б конструкции (в корпусе такие
 * расценки всегда называются «Устройство монолитн-…» или «…бетонирование…»)
 * без сопровождающей позиции «опалубк».
 *
 * Важно: ловим именно монолитное БЕТОНИРОВАНИЕ, а не любое упоминание плиты
 * перекрытия. Старый широкий список («перекрыт», «фундамент», «колонн») давал
 * ложные срабатывания на «стяжке по плитам перекрытия», демонтаже фундамента
 * и сборных конструкциях — опалубка там не нужна.
 */
const CONCRETE_KEYWORDS = ["монолитн", "бетонирование"];
