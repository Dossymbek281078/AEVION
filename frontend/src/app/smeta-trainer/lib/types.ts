...
export interface IndexSet {
  ...
  toSMR?: number;
  /** Источник официального индекса (НДЦС РК ...) — для честности данных. */
  source?: string;
  /** Учебная пометка (например: индекс национальный, по ресурсам не делится). */
  note?: string;
}
...
export interface LearningObject {
  ...
  /**
   * Объект эксплуатируется во время работ (действующее здание): корректный
   * расчёт требует коэффициента «действующий-объект»/«стеснённые» (К≈1.15).
   * AI-советник подсказывает, если коэффициент не применён ни к одной позиции.
   */
  occupied?: boolean;
  /**
   * Работы ведутся на высоте (≥ 8 м / с лесов), когда это не видно из geometry
   * (например, высотный электромонтаж): корректный расчёт требует коэффициента
   * «высота» (К=1.05..1.20). AI-советник подсказывает при его отсутствии.
   */
  atHeight?: boolean;
...
export interface SmetaPosition {
  ...
  drawingRef?: string;
  /**
   * Переопределение состава ресурсов расценки (полная замена `rate.resources`).
   * Если задано — calc использует эти ресурсы вместо нормативных. Используется,
   * когда студент меняет расход материала, заменяет один материал на другой
   * (например, бетон В12,5 → В22,5), или подгоняет смету под факт. Сохраняется
   * вместе с ЛСР — будет видно при импорте.
   */
  resourceOverrides?: Resource[];
}
