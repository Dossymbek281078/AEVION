 ...
 *
 * Секции вынесены в `./pricingI18n/sections/*.ts` (паттерн: `{ ru, en }`).
 * Дальнейший split — по той же схеме (см. migrations.ts).
 */

type Lang = "en" | "ru";

const baseDict: Record<Lang, Record<string, string>> = {
  ru: {
    ...
    "affiliate.faq.a1": "От нетто-суммы подписки приведённого клиента (после промо-кодов и налогов). Если клиент платит $24/мес на Lite — вы получаете $4.80 каждый месяц, пока он остаётся клиентом.",
    ...
    /* Migration guides — вынесено в ./pricingI18n/sections/migrations.ts */
  ...
  en: {
    ...
    "affiliate.faq.a1": "From the net subscription amount of the referred customer (after promo codes and taxes). If they pay $24/mo on Lite — you get $4.80 every month while they're a customer.",
    ...
    /* Migration guides — extracted to ./pricingI18n/sections/migrations.ts */
