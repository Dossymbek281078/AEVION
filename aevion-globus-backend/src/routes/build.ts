...
// recruiter proposes a small paid task, candidate completes it,
// recruiter approves payment + decides to hire. Real signal both ways.
// ──────────────────────────────────────────────────────────────────────

// POST /api/build/trial-tasks — recruiter proposes a paid task tied to
// an existing application. Owner-only (vacancy's project owner).
buildRouter.post("/trial-tasks", async (req, res) => {
  try {
    const auth = requireBuildAuth(req, res);
    if (!auth) return;
    ...
    const paymentAmount = req.body?.paymentAmount == null
      ? 0
      : Math.round(Math.max(0, Math.min(1e9, Number(req.body.paymentAmount) || 0)));
    const paymentCurrency = typeof req.body?.paymentCurrency === "string"
      ? req.body.paymentCurrency.trim().slice(0, 8) || "RUB"
      : "RUB";
    ...
    if (app.recruiterId !== auth.sub && auth.role !== "ADMIN") {
      return fail(res, 403, "only_vacancy_owner_can_propose_trial");
    }

    const id = crypto.randomUUID();
    const r = await pool.query(
      `INSERT INTO "BuildTrialTask"
         ("id","applicationId","vacancyId","recruiterId","candidateId",
          "title","description","paymentAmount","paymentCurrency","status")
...
async function loadTrialAndCheck(
  ...
  if (expectedId !== authSub && authRole !== "ADMIN") {
    return { row, err: { status: 403, code: `only_${expectedActor}_can_act` } };
  }
  return { row };
}

// POST /api/build/trial-tasks/:id/accept — candidate accepts (PROPOSED → ACCEPTED)
buildRouter.post("/trial-tasks/:id/accept", async (req, res) => {
  try {
    const auth = requireBuildAuth(req, res);
    if (!auth) return;
    ...
    const r = await pool.query(
      `UPDATE "BuildTrialTask" SET "status" = 'ACCEPTED', "updatedAt" = NOW() WHERE "id" = $1 RETURNING *`,
      [row.id],
    );
...
// POST /api/build/trial-tasks/:id/submit — candidate hands work in (ACCEPTED → SUBMITTED)
buildRouter.post("/trial-tasks/:id/submit", async (req, res) => {
  try {
    const auth = requireBuildAuth(req, res);
    if (!auth) return;
    const { row, err } = await loadTrialAndCheck(String(req.params.id), "candidate", auth.sub, auth.role);
    ...
    const submissionUrl = req.body?.submissionUrl == null
      ? null
      : String(req.body.submissionUrl).trim().slice(0, 2000) || null;
    const submissionNote = req.body?.submissionNote == null
      ? null
      : String(req.body.submissionNote).trim().slice(0, 4000) || null;

    const r = await pool.query(
      `UPDATE "BuildTrialTask"
         SET "status" = 'SUBMITTED', "submissionUrl" = $2, "submissionNote" = $3, "updatedAt" = NOW()
...
// POST /api/build/trial-tasks/:id/approve — recruiter approves (SUBMITTED → APPROVED)
//   Side-effect: creates a PENDING BuildOrder kind=TRIAL_PAYOUT for the
//   payment amount. Real escrow plugs in later via the existing
//   /orders/:id/pay flow.
buildRouter.post("/trial-tasks/:id/approve", async (req, res) => {
  try {
    const auth = requireBuildAuth(req, res);
    if (!auth) return;
    ...
    try {
      ...
      const r = await pool.query(
        `UPDATE "BuildTrialTask"
           SET "status" = 'APPROVED', "payoutOrderId" = $2, "updatedAt" = NOW()
         WHERE "id" = $1 RETURNING *`,
        [row.id, orderId],
      );
      await pool.query("COMMIT");
...
// POST /api/build/trial-tasks/:id/reject — either side may reject
//   - candidate rejects PROPOSED (declines task)
//   - recruiter rejects SUBMITTED (rejects work, no payment)
buildRouter.post("/trial-tasks/:id/reject", async (req, res) => {
  try {
    const auth = requireBuildAuth(req, res);
    if (!auth) return;
    const id = String(req.params.id);
    ...
    if (!isCandidateAtProposed && !isRecruiterAtSubmitted && auth.role !== "ADMIN") {
      return fail(res, 403, "cannot_reject_at_this_stage");
    }

    const reason = req.body?.reason == null
      ? null
      : String(req.body.reason).trim().slice(0, 1000) || null;
    const r = await pool.query(
      `UPDATE "BuildTrialTask" SET "status" = 'REJECTED', "rejectReason" = $2, "updatedAt" = NOW()
       WHERE "id" = $1 RETURNING *`,
      [id, reason],
    );
    return ok(res, r.rows[0]);
...
// GET /api/build/trial-tasks/by-application/:applicationId — owner or candidate
buildRouter.get("/trial-tasks/by-application/:applicationId", async (req, res) => {
  try {
    const auth = requireBuildAuth(req, res);
    if (!auth) return;
    const id = String(req.params.applicationId);
    const r = await pool.query(
      `SELECT * FROM "BuildTrialTask" WHERE "applicationId" = $1 ORDER BY "createdAt" DESC`,
      [id],
    );
    // Caller must be either candidate or recruiter on at least one row;
    // empty list is fine to return (200 [] for unauth probes is leaky,
    // so we 403 if any row exists and user is neither side).
    if ((r.rowCount ?? 0) > 0) {
      const row = r.rows[0];
      if (row.recruiterId !== auth.sub && row.candidateId !== auth.sub && auth.role !== "ADMIN") {
        return fail(res, 403, "not_party_to_trial");
      }
    }
...
// GET /api/build/trial-tasks/my — bearer's tasks (both as candidate and recruiter)
buildRouter.get("/trial-tasks/my", async (req, res) => {
  try {
    const auth = requireBuildAuth(req, res);
    if (!auth) return;
    const r = await pool.query(
...
// suppress lint on unused import if nothing references TRIAL_TASK_STATUSES
...
// ── Reviews / Ratings ───────────────────────────────────────────────
// Bidirectional review system anchored on a project. Eligibility:
// either side of an ACCEPTED application can review the other side
// (clientId ↔ application.userId), capped at one review per
// (project, reviewer, reviewee) triple.
...
async function resolveReviewDirection(
  ...
  // The non-client side must have at least one ACCEPTED application on a
  // vacancy in this project. We test it on whichever side isn't the client.
  ...
  // Discard the early null guard — TS narrowing.
  void workerCandidate;
  return {
    direction: reviewerIsClient ? "CLIENT_TO_WORKER" : "WORKER_TO_CLIENT",
  };
}

// POST /api/build/reviews — submit a review. Body: { projectId, revieweeId, rating, comment? }
buildRouter.post("/reviews", async (req, res) => {
  try {
    const auth = requireBuildAuth(req, res);
    if (!auth) return;
    ...
    try {
      ...
      // 23505 = unique_violation — pair already reviewed.
...
// GET /api/build/reviews/by-user/:userId — public, paginated reviews
// authored ABOUT this user. Includes reviewer name + project title.
buildRouter.get("/reviews/by-user/:userId", async (req, res) => {
  try {
...
// GET /api/build/reviews/by-project/:id — public, both directions of
// review on a single project. Used by /build/project/[id] and /build/p/[id].
buildRouter.get("/reviews/by-project/:id", async (req, res) => {
  try {
...
// GET /api/build/reviews/eligible — what review pairs the current user
// can post right now (projects where they have a counterpart and haven't
// already reviewed). Drives the "leave a review" UI.
buildRouter.get("/reviews/eligible", async (req, res) => {
  try {
    const auth = requireBuildAuth(req, res);
    if (!auth) return;

    // Two paths:
    //   (a) I'm the project client → eligible to review every ACCEPTED
    //       applicant who isn't yet reviewed by me on this project.
    //   (b) I have an ACCEPTED application → eligible to review the
    //       project client (once per project).
