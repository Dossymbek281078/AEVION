...
describe("проходимость", () => {
  ...
  // ⚠️ Порог MIN_CUT_AREA был закреплён ТОЛЬКО СНИЗУ: проверка ниже следит,
  // чтобы щели не попадали в находки. Мутационный аудит 27 ядровых констант
  // модуля 13.09.2026 показал, что порог можно поднять с 1 до 9 м², и ни один
  // тест не краснеет — дальняя часть стенда 12 м², она переживает и такой порог.
  //
  // Чем это грозит: отрезанная ниша в 2-5 м² перестала бы считаться проблемой.
  // Человек расставляет мебель, до части комнаты не дойти, а модуль молчит —
  // и узнаётся это уже на объекте, когда шкаф куплен и стоит.
  //
  // Поэтому здесь кусок СЕРЕДИННОГО размера: больше щели, меньше дальней части.
  it("отрезанная ниша 2-5 м² названа: порог держится и СВЕРХУ", () => {
    // Длинный шкаф поперёк ближней части отрезает полосу у северной стены.
    const items = [item(1, "Шкаф", 1.5, 2.8, [3.0, 0.5, 2.3])];
    const r = checkPassage(twoParts(), items);
    expect(r.issues.length, "отрезанная ниша потеряна — порог подняли молча")
      .toBeGreaterThanOrEqual(1);
    const ниша = r.issues.find((i) => i.area >= 1 && i.area <= 9);
    expect(ниша, `кусок среднего размера не найден: ${r.issues.map((i) => i.area.toFixed(1)).join(", ")}`)
      .toBeTruthy();
    expect(ниша!.text).toContain("не дойти от двери");
    // Замер на этом стенде: кусок ровно 2.34 м² — между щелью и дальней частью.
  });

  // ЧЕСТНАЯ ГРАНИЦА нижней стороны порога. Мутация MIN_CUT_AREA 1.0 -> 0.05
  // НЕ ловится, и это не лень: на этой геометрии карман меньше метра построить
  // не удалось — заливка находит обход вокруг любого предмета, который я
  // пробовал (три расстановки, кусков ноль при пороге 0.02). Значит проверять
  // нечего: нижняя сторона бережёт от ШУМА, а шума здесь не возникает.
  //
  // Записано, чтобы следующий аудит не считал это пропущенной работой и не
  // подгонял стенд ради зелёной мутации.
  it("граница: нижняя сторона порога на этом стенде непроверяема", () => {
    const r = checkPassage(twoParts(), [item(1, "Шкаф", 1.5, 2.8, [3.0, 0.5, 2.3])]);
    // Единственное, что можно утверждать честно: мелочи в находках нет.
