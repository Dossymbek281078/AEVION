 ...
 * НЕ изобретая правду.
 *
 * ЧТО ЭТО ЗА ЧИСЛА (важно, чтобы не сравнивать несравнимое). Скрипт считает по
 * ИСХОДНИКУ OSM в границах твина, а не по самому твину: у него своё число
 * элементов (Астана 12.08.2026 — 448) против числа зданий в твине (475).
 * Знаменатели разные, и «224 слепых» отсюда не равно «224 слепых в твине».
 *
 * И с 12.08.2026 они разошлись по сути, а не только по счёту: твин подставляет
 * слепой высоте 75-й процентиль её типа (`dataQuality.substituted`, 38 зданий в
 * Астане). Здесь показана КАРТИНА ИСТОЧНИКА — то, чем город отвечает о себе сам
 * до всякой подстановки. Именно поэтому цифры ниже не сойдутся с твином, и это
 * не расхождение, а разные вопросы.
 *
 * ЗАЧЕМ. Замер 12.08.2026 по Астане: 448 элементов, с тегом height всего 4, а 224 стоят на
...
// Порог из сборщика твина, чтобы текст отчёта не разошёлся с правилом.
const TYPE_MIN_SAMPLES = 3;
...
console.log(
  `\nОтчёт показывает СЫРЫЕ теги OSM и ничего не меняет. С 12.08.2026 сборщик`
  + ` твина уже подставляет слепой высоте 75-й процентиль её собственного типа`
  + ` (только для типов, которые о высоте что-то говорят, и при 3+ известных`
  + ` высотах этого типа), поэтому «слепых» в колонках выше и «угаданных 12 м» в`
  + ` самом твине — разные множества. Класс при подстановке остаётся guessed:`
  + ` процентиль по типу — догадка получше, но догадка, и страховочный просвет`
  + ` за неё платить надо.`,
);
