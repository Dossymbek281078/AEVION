...
2. **Проверка загрузки ESM-модуля СБОРКОЙ Next/Turbopack.** В пробе — простой
   статический сервер + нативный `import()`. В сборке нужно убедиться, что
   Turbopack не пытается разрешить `import(url)` на build-этапе (стоит
   `webpackIgnore`/`@vite-ignore`, файл в /public). Проверять реальной сборкой
   (локальный `next build` + `next start`, как верифицировали Level 2).
