 ...
 * 36 `status:"live"` + 5 `status:"mvp"`. The public-facing node count excludes
 * the `globus` entry, which is the interactive map shell itself, not a product
 * node (41 registry entries − 1 map shell = 40 nodes on the map).
 *
 * Пересчитано 10.08.2026 по факту. Прежние 37/35 отстали от реестра: там уже
 * было 41 запись и 36 живых, боевой API `/api/globus/projects` отдавал те же
 * 36, а витрина занижала платформу на четыре узла. Сторож при этом был
 * зелёный — он сравнивал константы сами с собой и реестр не открывал. Теперь
 * он считает по projects.ts, поэтому добавление модуля роняет тест до
 * обновления этих двух чисел.
 */
