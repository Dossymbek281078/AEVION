...
const PAGES = [
  ...
  // Добавлены 21.08.2026 после сплошного замера: у сайта 91 раздел верхнего
  // уровня, а сторож знал 38 адресов. То есть правило 10.5 из CLAUDE.md
  // («каждый новый live-модуль со страницей — добавь сюда») выполнялось не
  // всегда, и 53 раздела не проверял никто. Все 53 обойдены вручную в этот
  // день: сломанных нет, 200 у каждого. Добавляю НЕ все — только те, чья
  // тихая пропажа стоит дороже проверки; остальные (эксперименты вроде
  // /z-tide, /reconstruct-demo, /psyapp-deps) намеренно оставлены снаружи,
  // чтобы сторож не краснел на том, что никому не дорого.

  // (а) Адреса, которые читает ВНЕШНИЙ проверяющий. Их поломка возвращается
  // не тревогой, а отказом в заявке — то есть узнаём мы через недели и не
  // понимая причины. Проверено, что это не догадка: /data-deletion написана
  // прямо под TikTok и Meta (4 упоминания каждого в тексте страницы) и
  // фигурирует в наших документах заявки TikTok от 16.08 и 19.08.
  "/privacy",
  "/terms",
  "/data-deletion",
  "/security",
  ...
  // (б) Каталог и страница состояния. /modules — самая большая страница
  // сайта (319 КБ), вход в весь список продуктов; /status обещает честность
  // про доступность, и её собственная поломка особенно неловка.
  "/modules",
  "/status",

  // (в) Страницы модулей, чьи ручки чинили 19–21.08. Семь из них не работали
  // с самого начала (поиск вакансий, доступ к купленному, подпись
  // инструктажа), и ни одна проверка этого не видела. Раз бэкенд починен —
  // пусть хотя бы пропажа страницы больше не проходит молча.
  "/qjobs",
  "/qlife",
  "/qpersona",
  "/qreal",
  ...
  // (г) Две страницы-обещания. /verify-offline — заявленная офлайн-проверка
  // подписи, опора доверия к бюро (запуск 06.09); /developers — 119 КБ
  // документации, куда придёт покупатель DevHub за $149 (запуск 13.09).
  "/verify-offline",
  "/developers",
];
