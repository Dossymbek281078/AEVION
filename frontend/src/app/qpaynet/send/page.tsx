...
function SendForm() {
  ...
  async function handleSend() {
    ...
    try {
      ...
      if (!r.ok) throw new Error(d.error ?? t("qpaynet.send.err.generic"));
      setDone(d);
    } catch (e) { setError(e instanceof Error ? e.message : t("qpaynet.send.err.generic")); }
  ...
  if (done) return (
    <div className="text-center py-12 px-4">
      ...
      <h2 className="text-xl font-bold mb-2">{t("qpaynet.send.done.title")}</h2>
      <p className="text-slate-400 text-sm mb-1">{t("qpaynet.send.done.fee")} <strong className="text-amber-400">{done.fee.toLocaleString("ru-RU", { minimumFractionDigits: 2 })} ₸</strong></p>
      <p className="text-slate-400 text-sm mb-6">{t("qpaynet.send.done.balance")} <strong className="text-emerald-400">{done.newBalance.toLocaleString("ru-RU", { minimumFractionDigits: 2 })} ₸</strong></p>
      <div className="flex flex-col sm:flex-row gap-2 justify-center max-w-sm mx-auto">
        <button onClick={() => router.push("/qpaynet")} className="px-6 py-3 bg-violet-600 hover:bg-violet-700 rounded-lg text-sm font-semibold min-h-[44px]">{t("qpaynet.send.done.toWallet")}</button>
        <button onClick={repeatFlow} className="px-6 py-3 bg-slate-800 hover:bg-slate-700 rounded-lg text-sm font-semibold min-h-[44px]">{t("qpaynet.send.done.again")}</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-sm mx-auto py-8 sm:py-12 px-4 sm:px-6">
      <h1 className="text-xl font-bold mb-6">{t("qpaynet.send.title")}</h1>
      <div className="space-y-4">
        <div>
          <label htmlFor="qpaynet-send-to" className="text-xs text-slate-400 mb-1 block">{t("qpaynet.send.toLabel")}</label>
          <div className="relative">
            <input id="qpaynet-send-to" type="text" value={toWalletId} onChange={e => setToWalletId(e.target.value)}
              placeholder={t("qpaynet.send.toPlaceholder")}
            ...
            <button
              ...
              title={t("qpaynet.send.pasteTitle")}
              aria-label={t("qpaynet.send.pasteAria")}
            ...
            >
              {t("qpaynet.send.paste")}
            </button>
          </div>
          <p id="qpaynet-send-to-help" className="sr-only">{t("qpaynet.send.toHelp")}</p>
          {recipient === "loading" && <p className="text-[11px] text-slate-500 mt-1">{t("qpaynet.send.checking")}</p>}
          {recipient === "notfound" && <p className="text-[11px] text-red-400 mt-1" role="alert">⚠ {t("qpaynet.send.notFound")}</p>}
          {recipient && typeof recipient === "object" && (
            <p className={`text-[11px] mt-1 ${recipient.active ? "text-emerald-400" : "text-amber-400"}`}>
              {recipient.active ? "✓" : "⚠"} {recipient.name}{!recipient.active && ` ${t("qpaynet.send.inactive")}`}
            </p>
          )}
        </div>
        <div>
          <label htmlFor="qpaynet-send-amount" className="text-xs text-slate-400 mb-1 block">{t("qpaynet.send.amountLabel")}</label>
          ...
          {amount && parseFloat(amount) > 0 && (
            <p className="text-[11px] text-slate-500 mt-1">
              {t("qpaynet.send.feeHint")}: {(parseFloat(amount) * 0.001).toLocaleString("ru-RU", { minimumFractionDigits: 2 })} ₸
            </p>
          )}
        </div>
        <div>
          <label htmlFor="qpaynet-send-desc" className="text-xs text-slate-400 mb-1 block">{t("qpaynet.send.descLabel")}</label>
          <input id="qpaynet-send-desc" type="text" value={description} onChange={e => setDescription(e.target.value)}
            placeholder={t("qpaynet.send.descPlaceholder")}
            maxLength={500}
            className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/40" />
        </div>
        ...
        <button onClick={handleSend} disabled={loading || !toWalletId || !amount}
          ...
          {loading ? t("qpaynet.send.sending") : t("qpaynet.send.submit")}
        </button>
        <Link href="/qpaynet" className="block text-center text-xs text-slate-500 hover:text-slate-300 py-2">{t("qpaynet.send.back")}</Link>
      </div>
    </div>
  );
}
