/**
 * ВЫМЫШЛЕННЫЕ истории — НЕ ПУБЛИКУЮТСЯ (снято 20.09.2026).
 *
 * Этот файл никуда не импортируется намеренно. Шесть «кейсов клиентов» жили в
 * data/cases.ts и отдавались публичной ручкой /api/pricing/cases: названия
 * компаний («Банк Алматы»), метрики ROI (73 % экономии против DocuSign) и
 * цитаты с именами и должностями. Комментарий в шапке того файла признавал
 * прямо: «цитаты фиктивные».
 *
 * Мы продаём доказуемость происхождения. Ложное свидетельство на продающей
 * странице бьёт ровно по тому, чем торгуем, и по бюро, которое мы продаём
 * отдельно. В тот же день по той же причине сняты демо-отзывы и счётчики без
 * источника с /pricing (TESTIMONIALS = [], TRUST_NUMBERS почищены) и четыре
 * выдуманные истории найма с витрины QBuild.
 *
 * Тексты сохранены, потому что форма годная: если основатель решит показывать
 * СЦЕНАРИИ применения — без имён компаний, без цитат и с явной пометкой, что
 * числа расчётные, — брать отсюда. Возвращать как «кейсы клиентов» нельзя:
 * настоящих клиентов с такими результатами у нас нет (оплат за всё время три,
 * последняя 02.06.2026).
 */
import type { CaseStudy } from "./cases";

export const UNPUBLISHED_FICTIONAL_CASES = [] = [
  {
  ...
  },
  {
    ...
    solution:
      "Внедрили AEVION IP Bureau (Business) как единый back-office. QRight для первичной регистрации цифровых артефактов, QSign для подписи контрактов с клиентами, IP Bureau для генерации фиксации авторства и приоритета.",
    outcome:
