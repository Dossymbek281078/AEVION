...
>
> Пересчитал правильно (вырезав комментарии перед поиском `<PaywallScreen`): из 15 страниц,
> где компонент упоминается, его реально рендерят **14**. Единственное исключение —
> `qfusionai`. Для рекомендованного первого флипа это значит:
>
> | Модуль | Что увидит бесплатный пользователь |
> |---|---|
> | `qcoreai` | страничный `<PaywallScreen>` |
> | `healthai` | страничный `<PaywallScreen>` |
> | `multichat-engine` | страничный `<PaywallScreen>` |
> | `qfusionai` | обычная страница + глобальная модалка при вызове платной ручки |
>
> Оба варианта рабочие — но если ждать одинаковой картинки у всех четырёх, `qfusionai`
> будет выглядеть как «гейт не сработал», хотя API отказывает корректно.
>
