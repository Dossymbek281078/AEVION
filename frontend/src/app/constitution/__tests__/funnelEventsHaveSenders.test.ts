...
const NO_FEATURE: Record<string, string> = {
  academy_lesson_done: "академии в продукте нет — маршрута /academy не существует",
  academy_cert: "сертификатов академии нет — см. academy_lesson_done",
  comment_posted: "комментариев в конституции нет ни на одной странице",
  vote_cast: "голосований в продукте нет — маршрута нет",
};
...
describe("объявленное событие воронки кто-то отправляет", () => {
  ...
  test("у каждого события есть отправитель либо названная причина его не иметь", () => {
    ...
    for (const e of EVENTS) {
      if (sendersOf(e) === 0 && !NO_FEATURE[e]) orphans.push(e);
    }
    expect(
      orphans,
  ...
  test("список разрешённых не разросся: в нём только то, чего нет в продукте", () => {
    ...
    const stale = Object.keys(NO_FEATURE).filter((e) => sendersOf(e) > 0);
