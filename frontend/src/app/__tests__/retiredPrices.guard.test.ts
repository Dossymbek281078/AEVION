...
const RETIRED: Array<{ amount: string; was: string }> = [
  ...
  //
  // 15.09.2026 — ЛЕСТНИЦА СРОКОВ (слово основателя). Помесячные тарифы и подписки
  // сняты целиком. «24» УБРАНО из списка: это снова живая цена — месяц CyberChess
  // на ступени Lite (STANDALONE_APPS). Держать её отставной значило бы ловить
  // живую цену как мёртвую.
  ...
  //
  // 🔴 $59 (All-Access), $149 (Universe и DevHub помесячно), $19/$29/$49
  // (прежние Lite/Medium/Full) в список ПОКА НЕ ВНЕСЕНЫ — и это осознанно,
  // а не забывчивость. Две причины, обе проверены прогоном 15.09.2026:
  //
  //   1. Эти номиналы живы у ДРУГИХ товаров: гайды на Gumroad ($19),
  //      сертификат Verified в IP Bureau ($19/серт.), API-тариф Build
  //      ($49/mo, apiQuotas.ts), цены конкурентов в сравнениях.
  //   2. Пробный прогон с $59 и $149 в списке дал 23 находки, и 20 из них — в
  //      файлах ВНЕ этой правки: финансовые модели (data/pitchFacts.ts,
  //      data/pitchModel.ts), словари pricing.* и бэкенд. Сторож, красный из-за
  //      чужой незавершённой работы, перестают читать — это ровно тот случай,
  //      о котором предупреждает «аудит, который всегда красный».
  //
  // Вносить их сюда — вместе с правкой финансовых моделей и бэкенда, одним
  // заходом. Пока снятые ПОДПИСКИ ловятся по смыслу — тестом ниже про снятые
  // товары Gumroad, каталожными сторожами и сторожами страницы цен.
];
...
const ALLOWED: Array<{ fragment: string; reason: string }> = [
  ...
  // ── Продающая модель: исторических фраз о репрайсе 13.08 больше нет ──────
  // УБРАНО 16.09.2026, вместе с переходом финансовых моделей на лестницу сроков.
  //
  // Здесь стояли два исключения — «$89 → $49/mo» и «repriced $249.99 → $149 on
  // 2026-08-13». Оба указывали на фразы в data/pitchFacts.ts и
  // data/pitchModel.ts, объяснявшие репрайс 13.08.2026. Обе модели переписаны
  // под лестницу сроков (тариф — это СРОК доступа), и этих фраз в них больше
  // нет: значит исключения стали ПРОТУХШИМИ.
  //
  // Протухший фрагмент — тихая дыра: он ничего не разрешает, но выглядит
  // осознанным решением, и следующий человек не станет его перепроверять.
  // Ровно это и ловит тест «каждое исключение действительно что-то исключает»
  // ниже, поэтому строки УДАЛЕНЫ, а не оставлены «на всякий случай».
  //
  // Исключение Bureau выше остаётся: $89 за сертификат — живая цена своей
  // лестницы, к тарифам платформы отношения не имеющая.
];
...
describe("отставные цены тарифов не возвращаются ни на одну поверхность", () => {
  ...
  it("снятые подписки не продаются ни с одной поверхности (15.09.2026)", () => {
    // All-Access (xpxzam), Constitution Pro (pyiaz) и Team (wjvquw) сняты с
    // продажи: подписка AEVION — это срок доступа ко всей планете. Ссылка на
    // их кассу Gumroad в коде фронтенда (не в комментарии) — это кнопка, которая
    // продаёт снятый товар.
    const re = /gumroad\.com\/l\/(xpxzam|pyiaz|wjvquw)|GUM\("(xpxzam|pyiaz|wjvquw)"\)/;
    const violations: string[] = [];
    for (const { rel, lines } of corpus()) {
      if (!rel.startsWith("frontend/")) continue;
      const isComment = commentMask(lines);
      lines.forEach((line, idx) => {
        if (!isComment[idx] && re.test(line)) violations.push(`${rel}:${idx + 1}  «${line.trim().slice(0, 100)}»`);
      });
    }
    expect(violations, "снятый товар Gumroad снова продаётся:\n  " + violations.join("\n  ")).toEqual([]);
    // Контроль прибора: шаблон ловит ровно ту форму, что жила в каталоге.
    expect(re.test('href: GUM("xpxzam"),')).toBe(true);
    expect(re.test('ctaHref: "https://aevion.gumroad.com/l/pyiaz",')).toBe(true);
    expect(re.test('href: GUM("kkiavh"),')).toBe(false);
  }, SWEEP_TIMEOUT_MS);
