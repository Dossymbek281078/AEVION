...
describe("ключи, которые только читают", () => {
  ...
  /* Исключения — с ПРИЧИНОЙ у каждого, проверенной 11.08.2026. Без причины список
     превращается в место, куда сметают неудобные находки, и сторож тихо слепнет.
     Проверял отдельно: ложная тревога здесь стоит дороже пропуска, потому что «чинить»
     побегут работающее. */
  const OUTSIDE_MODULE = new Set([
    // пишет страница входа (`app/auth/page.tsx`) через константу TOKEN_KEY
    "aevion_auth_token_v1",
    /* запасные имена в цепочке `getItem(v1) || getItem(token) || getItem(jwt)`:
       их не пишет НИКТО и не должен — это совместимость со старыми сборками */
    "aevion_jwt",
    "aevion_token",
    // читается один раз как запасное имя рядом с профилем аккаунта
    "aevion_user_display_name",
    // читает billing.ts; значение приходит из платформенного слоя, не из шахмат
    "aevion_platform_wallet_id",
    // отладочный флаг, ставится руками в консоли
    "aevion_debug",
  ]);
