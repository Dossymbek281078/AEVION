...
export function calcLevel(totalXp: number): { level: number; currentXp: number; nextLevelXp: number } {
  let level = 1;
...
async function getPlanetUser(userId: string) {
  const db = getPrisma();
...
export async function awardXp(userId: string, amount: number, reason: string, metadata?: object) {
  const db = getPrisma();
  ...
  const xpLogData: { planetUserId: string; amount: number; reason: string; metadata?: object } = {
...
export async function getAllTerritories(userId: string) {
  const db = getPrisma();
  const pu = await getPlanetUser(userId);
  ...
  return territories.map((t: (typeof territories)[number]) => {
...
export async function unlockTerritory(userId: string, slug: string) {
  const db = getPrisma();
  const pu = await getPlanetUser(userId);
...
export async function updateTerritoryProgress(userId: string, slug: string, progress: number) {
  const db = getPrisma();
  const pu = await getPlanetUser(userId);
  const territory = await db.territory.findUniqueOrThrow({ where: { slug } });
...
export async function getAvailableQuests(userId: string) {
  ...
  return quests.map((q: (typeof quests)[number]) => {
...
export async function startQuest(userId: string, questId: string) {
  ...
  if (existing) throw new Error("Quest already started");
...
export async function getUserRank(userId: string) {
  ...
  if (!entry) return { rank: null, xp: 0 };
...
export async function seedPlanetEngine() {
  ...
  const quests = [
    { title: "Complete your profile", description: "Fill in your bio, avatar and links", type: "ONE_TIME" as const, xpReward: 50, territoryId: onboarding?.id ?? null },
    { title: "First post", description: "Create your first piece of content", type: "DAILY" as const, xpReward: 30, territoryId: content?.id ?? null },
    { title: "React to 5 posts", description: "Like or comment on 5 different posts", type: "DAILY" as const, xpReward: 20, territoryId: social?.id ?? null },
    { title: "Invite a friend", description: "Send an invite link to someone", type: "ONE_TIME" as const, xpReward: 100, territoryId: social?.id ?? null },
    { title: "7-day streak", description: "Log in 7 consecutive days", type: "WEEKLY" as const, xpReward: 150, territoryId: onboarding?.id ?? null },
    { title: "Reach level 5", description: "Accumulate enough XP to reach level 5", type: "STORY" as const, xpReward: 300, territoryId: analytics?.id ?? null },
    { title: "Explore AI Core", description: "Use an AI feature for the first time", type: "STORY" as const, xpReward: 200, territoryId: aicore?.id ?? null },
  ];
  for (const q of quests) {
    const exists = await db.quest.findFirst({ where: { title: q.title } });
    if (!exists) await db.quest.create({ data: { ...q, isActive: true } });
  }
