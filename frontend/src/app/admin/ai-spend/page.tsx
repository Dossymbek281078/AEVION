...
export default function AiSpendPage() {
  ...
  return (
    <main style={{ maxWidth: 960, margin: "0 auto", padding: "24px 20px", fontFamily: "system-ui, sans-serif", color: "#0f172a" }}>
      ...
      {/*
        02.09.2026: строки с суффиксом «-БЕЗ-ЦЕНЫ» приходят НЕ через
        smartComplete. Это поверхности DevHub, зовущие поставщиков, которых нет
        в нашей таблице цен: звук, клонирование голоса, распознавание речи,
        перевод, шаблон письма. У них виден ОБЪЁМ и стоит ноль — не потому что
        бесплатно, а потому что посчитать нечем (наш расчёт считает токены, а
        там секунды и символы). Подпись выше говорит про smartComplete, поэтому
        пояснение стоит здесь: иначе ноль прочитают как факт о деньгах.
      */}
      <p style={{ color: "#92400e", fontSize: 12, margin: "0 0 14px" }}>
        Строки <code>…-БЕЗ-ЦЕНЫ</code> — это объём без цены: поставщика нет в
        таблице тарифов, поэтому ноль здесь означает «посчитать нечем», а не
        «бесплатно». Суммы эти строки не меняют.
      </p>
