...
describe("предел отправки заметно строже предела генерации", () => {
  // Числом, а не на глаз. У Brevo на текущем плане потолок 300 писем в сутки
  // (CLAUDE.md окна запуска). При общем пределе 30/мин один адрес выжигал бы всю
  // суточную квоту платформы за ДЕСЯТЬ МИНУТ, и подтверждения подписки перестали
  // бы приходить всем — включая тех, кого ждём после запуска.
  //
  // Поэтому проверяется порядок величины, а не факт наличия предела: иначе
  // возврат к 30 пройдёт молча и выглядеть будет как «предел на месте».
  test("отказ приходит в первом десятке попыток, а не на тридцатой", async () => {
    const at = await firstThrottled("/api/devhub/media/email", 40);
    expect(at, "предела нет вовсе").not.toBeNull();
    expect(
      at!,
      `отказ пришёл на попытке ${at} — при потолке 300 писем в сутки это позволяет ` +
        `выжечь суточную квоту платформы с одного адреса за минуты`,
    ).toBeLessThanOrEqual(10);
  });

  test("но не с первой попытки: одну отправку сделать можно", () => {
    // Обратная граница. Предел, отвергающий и первый запрос, — это не предел, а
    // отключённая ручка, и его легко получить, ошибившись на единицу.
    expect(Number(process.env.DEVHUB_SEND_RATE_LIMIT) || 5).toBeGreaterThan(1);
  });
});
