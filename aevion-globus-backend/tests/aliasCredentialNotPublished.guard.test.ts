 ...
 *
 *   mapReality:  ПОГАШЕНО 28.07 — права переехали на отдельный секрет правки,
 *                псевдоним остался просто отображаемым именем. Остаток: у
 *                сигналов, созданных до правки, секрета нет, и для них
 *                сохранено прежнее поведение (иначе автор потерял бы доступ);
 *   shadownet:   ПОГАШЕНО 28.07 — удаление переехало на delete_secret,
 *                псевдоним остался подписью автора.
 *
...
describe("псевдоним-удостоверение не публикуется", () => {
  ...
  it("обход нашёл модули, где псевдоним вообще используется как ключ", () => {
    ...
    // Семейство СОКРАТИЛОСЬ, потому что его починили: 28.07 их было четыре
    // (lifebox, mapReality, shadownet, qpersona), у трёх права переехали на
    // отдельный секрет. Остался lifebox — единственный, где псевдоним-ключ
    // изначально сделан правильно и наружу не отдаётся.
    //
    // Порог оставлен ненулевым намеренно: ноль означал бы сломанный признак
    // ALIAS_AUTHZ, и тогда «нарушений нет» не значило бы ничего.
    expect(usesAlias, "не найдено ни одного модуля с псевдонимом-ключом").toBeGreaterThan(0);
  });
