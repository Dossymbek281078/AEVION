...
describe("сравнением покрыт весь реестр модулей", () => {
  ...
  it("ссылка «Открыть модуль» ведёт на существующий маршрут", () => {
    ...
    //
    // ОГРАНИЧЕНИЕ, важное при падении: проверяется наличие СВОЕЙ папки
    // страницы. В приложении есть общий динамический маршрут `app/[id]`,
    // отдающий страницу модуля по id из каталога, — поэтому адрес без своей
    // папки (например /globus) на проде отвечает 200, а не 404. Значит
    // красный здесь читается как «нет выделенной страницы», и прежде чем
    // считать ссылку битой, надо проверить, не обслуживает ли её `[id]`.
    // Для всех сравниваемых модулей выделенная страница есть, поэтому
    // проверка строгая: так она заодно ловит опечатку в пути.
