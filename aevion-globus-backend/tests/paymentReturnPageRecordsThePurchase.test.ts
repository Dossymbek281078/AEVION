...
const KNOWN_UNMEASURED: Record<string, string> = {
  // Пусто — и это ЗАРАБОТАННАЯ пустота, а не отключённая проверка.
  //
  // Здесь стояли два долга (/bureau и /qpaynet/deposit/success) с условием
  // исчезновения, записанным прямым текстом: «убрать после мержа ветки
  // verify/paylinks-merge-29b». Мерж состоялся 01.09.2026, замер на обеих
  // страницах появился, и храповик потребовал сократить список сам —
  // проверкой «долг не растёт: починенное убрано». Он же и назвал, какие
  // именно записи убирать.
  //
  // Новая запись здесь допустима только с ДВУМЯ вещами: причиной и условием
  // исчезновения. Без второго список превращается в кладбище: он объясняет,
  // почему страница не считается, и ничего не требует.
};
...
const PURCHASE_MARKERS = [
  ...
  // 01.09.2026: страницы возврата перешли на ОДИН общий компонент, и учёт
  // уехал внутрь него. Признак остался инлайновым — собственный контроль
  // сторожа честно покраснел словами «не вижу замер на странице, где он ТОЧНО
  // есть». Это лучший исход из возможных: сторож сказал, что ослеп, вместо
  // того чтобы молча пропускать страницы.
  //
  // Ищем ОТРИСОВКУ, а не имя: импорт без отрисовки ничего не считает. Тот же
  // урок, что был у сторожа возвратов на сайте этим утром.
...
describe("страница возврата после оплаты фиксирует покупку", () => {
  test("контроль: адреса возврата вообще найдены", () => {
    ...
    expect(PATHS.length, "разбор не нашёл ни одного адреса возврата — сторож пуст").toBeGreaterThan(3); // 15.09.2026: адресов стало 4 — касса Конституции снята с продажи (410) и адрес возврата больше не строит
  });
