...
describe("статус возможности не спорит с пробой", () => {
  ...
  test("возвращает возможность отдельный осознанный флаг", () => {
    ...
    // 30.08.2026: флаг заменён ЖИВОЙ ПРОБОЙ зоны, и это строже. Флаг ставит
    // человек, проверив зону, — он протухает МОЛЧА в день, когда зона
    // перестанет отвечать, и панель снова объявит домен рабочим. Проба
    // протухнуть не может.
    //
    // Требование прежнее по СМЫСЛУ: статус выводится не из наличия ключей, а
    // из проверенного факта. Принимаем оба способа его получить.
    const line = capabilityLine("domain");
    expect(
      line.includes("DEVHUB_AEVION_BUILD_ZONE_ACTIVE") || line.includes("zoneActive"),
      "статус домена не опирается ни на флаг, ни на пробу — значит на ключи",
    ).toBe(true);
  });
