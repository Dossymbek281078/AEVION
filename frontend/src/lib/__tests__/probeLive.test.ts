...
describe("daysUntil — счёт по UTC-полуночи", () => {
  ...
  test("до даты — положительное число, в день — ноль", () => {
    vi.useFakeTimers();
    // Даты здесь ПРОИЗВОЛЬНЫЕ и к дате запуска отношения не имеют: важно
    // только, что «сейчас» ровно на три дня раньше цели. 13.09.2026 общий
    // перенос запуска с 10 на 20 сентября механически сдвинул «сейчас» и
    // НЕ сдвинул цель — тест честно упал (ожидал 3, получил -7). Меняя
    // одну из двух дат, меняйте и вторую: закреплено здесь ОТНОШЕНИЕ.
...
describe("тело недоступно — запасное правило по статусу (19.08.2026)", () => {
  // Мутация `if (!body) return true` выживала: ветвь, работающая когда мы знаем
  // МЕНЬШЕ всего, не была закреплена ничем. Она же решает публичное утверждение
  // «модуль жив» на посадочных — то есть ошибка здесь выходит наружу текстом.
  //
  // Пустое тело — не выдумка: так отвечает ответ через прокси, ответ 204 и
  // оболочка fetch без text(). Правило по статусу здесь и есть весь ответ.

  test("пустое тело при 404 — НЕ живо: иначе страница объявит живым отсутствующий адрес", async () => {
    stubFetch(404, "");
    expect(await probeLive("/api/no-such")).toBe(false);
  });

  test("пустое тело при 200 — живо", async () => {
    stubFetch(200, "");
    expect(await probeLive("/api/ok")).toBe(true);
  });

  test("пустое тело при 402 — живо: доменный отказ означает, что обработчик есть", async () => {
    // Ровно то, что написано в шапке probeLive: 400/402/404 с доменной ошибкой —
    // это ответ НАШЕГО кода. Пустое тело статус не меняет.
    stubFetch(402, "");
    expect(await probeLive("/api/paid")).toBe(true);
  });

  test("оболочка без text() не роняет пробу и решает по статусу", async () => {
    // typeof r.text !== "function" — тот же путь, что пустое тело.
    vi.stubGlobal(
      "fetch",
      vi.fn(() => Promise.resolve({ status: 404, ok: false } as unknown as Response)),
    );
    expect(await probeLive("/api/no-such")).toBe(false);
  });
});
