 ...
 * Заявление «соответствует закону» без второго источника не отслеживается
 * никем — его судьба по-прежнему решение основателя.
 *
 * ⚠️ ПОПРАВКА 08.09.2026, и она про сам этот комментарий. Здесь стояло
 * «pcidss.status: Level 1 — формальный уровень, подтверждаемый ежегодным
 * аудитом QSA». Это УЖЕ НЕ ТАК: в словаре давно другое — «Данные карты к нам
 * не попадают — касса на стороне провайдера» (en: «Card data never touches us
 * — hosted checkout»), и живая страница показывает именно эту строку.
 * То есть заявление превратилось из сертификационного в проверяемый факт, а
 * объяснение осталось прежним и пугало сильнее, чем предмет.
 *
 * Сам `pcidss` из списка НЕ убираю: сторож сверяет наличие записи в реестре
 * доверия, а её как не было, так и нет. Меняется только повод — не «заявляем
 * уровень без аудита», а «утверждение о платежах не отслеживается вторым
 * источником».
 *
 * Урок общий: комментарий, называющий текущее состояние, стареет как отчёт,
 * а тестов у него нет. Тот, кто прочитает его сегодня, решит, что мы
 * обещаем PCI DSS Level 1 — и пойдёт чинить починенное.
 */
