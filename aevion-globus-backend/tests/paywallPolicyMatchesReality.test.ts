...
describe("окно выборки воронки ограничено разумными пределами", () => {
  // `Number(x) || 30` не останавливает отрицательные: «zzz» даёт 30, а «-5»
  // проходит и уезжает в запрос. Последствие в проекции выручки измерить нечем
  // (в тестовой среде данных нет), но само ограничение проверяемо.
  test.each([
    ["обычное", "30"],
    ["мусор", "zzz"],
    ["отрицательное", "-5"],
    ...
    ["огромное", "999999"],
    ["пустое", ""],
  ])("days=%s (%s) — ручка отвечает, а не падает", async (_имя, d) => {
    const res = await request(приложение()).get(`/api/paywall/funnel?days=${d}`);
    expect(res.status, `days=${d} уронил ручку`).toBe(200);
    // Проекция не обязана быть числом при пустых данных, но обязана быть
    // определённой: undefined на витрине превращается в «—» без объяснения.
    expect(res.body, `days=${d} вернул пустое тело`).toBeTruthy();
  });
});
