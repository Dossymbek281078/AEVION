...
describe("экспликация помещений: номера на плане ↔ строки таблицы", () => {
  ...
  it("имя с номером внутри («Помещение 1», обмерный альбом) — тоже строка экспликации", () => {
    const лист2 = [эл("1", 617, 476), эл("Помещение 1", 640, 476), эл("32,39м", 720, 476), эл("55,19м", 780, 476), эл("2", 616, 468), эл("Помещение 2", 640, 468), эл("6,73м", 720, 468), эл("1", 318, 323), эл("2", 165, 226)];
    expect(экспликацияИзТекста(лист2)).toEqual([{ n: 1, name: "Помещение 1", area: 32.39 }, { n: 2, name: "Помещение 2", area: 6.73 }]);
    expect(номераНаПлане(лист2, экспликацияИзТекста(лист2)).map((x) => x.text).sort()).toEqual(["1", "2"]);
  });
