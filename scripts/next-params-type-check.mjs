...
// `params`, объявленный объектным типом — сам по себе или в объединении с
// Promise. Данные вида `params: "текст"` и `let params: any` под это не
// попадают: требуется фигурная скобка.
const BAD_PARAMS = /params:\s*(?:Promise<[^>]*>\s*\|\s*)?\{[^}]*\}/g;
