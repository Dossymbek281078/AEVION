 ...
 * с ценой ошибки: сумма только по записям, где она есть
 *
 * КТО ЭТО ЧИТАЕТ (проверено 02.09.2026, не предположение). Имена полей ниже
 * закреплены утверждениями не для красоты: панель админки «фактически
 * списано по каналам» разбирает ИМЕННО их —
 *
 *     frontend/src/app/pricing/admin/page.tsx
 *     fetch(apiUrl("/api/pricing/provisioning/subscriptions/by-channel") + `?hours=${hours}`)
 *     byChannel: Record<string, { count; amountUsdSum; withAmount }>, withAmount, windowHours
 *
 * Панель живёт в ДРУГОЙ ветке и придёт мержем. Пока она там, переименование
 * любого из этих полей здесь ничего не сломает у меня и молча опустошит
 * панель у них: строка через границу HTTP типами не проверяется, а тесты
 * есть только у отправителя. Поэтому имена — контракт, а не деталь.
 * Меняете имя — идите и меняйте читателя тем же заходом.; знаменатели withAmount
...
describe("выручка по каналам: закрыта и честна", () => {
  ...
  test("ноль в окне не выдаётся за отсутствие продаж", async () => {
    // Замер на проде 01.09.2026: покупок 31, свежайшей 36 дней. При окне по
    // умолчанию сводка вернёт ноль — и панель покажет «выручка 0», что
    // читается как «продаж нет». Правда другая: продаж не было ЗА МЕСЯЦ.
    //
    // Пара «самая старая — самая свежая» и общее число разводят эти два
    // случая: пустое хранилище против затишья.
    const r = await request(await load())
      .get("/api/pricing/provisioning/subscriptions/by-channel?hours=1.5")
      .set("X-Admin-Token", TOKEN);
    // 1.5 ч, а не 1: запись стенда стоит РОВНО на часовой границе, и такое
    // окно даёт то 0, то 1 — проверка мерила бы удачу, а не поведение.
    expect(r.body.total, "узкое окно берёт лишнее").toBe(1);
    expect(r.body.storage.recordsTotal, "общее число покупок потеряно").toBe(4);
    expect(
      Date.parse(r.body.storage.newestRecord),
      "дата последней покупки не названа — ноль не отличить от пустого хранилища",
    ).toBeGreaterThan(Date.parse(r.body.storage.oldestRecord));
  });
