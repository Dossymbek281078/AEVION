 ...
 *
 * ⚠️ 13.09.2026: список пополнен `middleware.ts`. Он пришёл из линии сборки и
 * является штатным файлом Next.js, а сторож объявил его посторонним и
 * покраснел на ветке доставки. То есть проверка, написанная против ЧУЖИХ
 * скриптов, встала на пути законного файла платформы.
 *
 * Цена ошибки тут несимметрична: пропустить чужой файл — потерять день,
 * покраснеть на своём — остановить доставку и заставить разбираться. Поэтому
 * список ведётся вручную и пополняется осознанно: новый корневой файл Next.js
 * добавляется сюда, а всё, чего в списке нет, по-прежнему считается чужим.
 */
