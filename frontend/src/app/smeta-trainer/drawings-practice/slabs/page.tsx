              ...
              {!done.has(ex.id)?(
                <div className={`border-2 rounded-lg p-3 ${ok?"border-emerald-300 bg-emerald-50 dark:bg-emerald-900/20":err?"border-red-300 bg-red-50 dark:bg-red-900/20":"border-slate-200 dark:border-slate-700"}`}>
                  ...
                  <div className="flex gap-2">
                    ...
                    {!rev[k]&&<button onClick={go} aria-label="Проверить ответ" disabled={!inp[k]?.trim()} className="px-3 py-1.5 bg-slate-700 text-white text-xs font-semibold rounded hover:bg-slate-800 disabled:opacity-40">✓</button>}
                  </div>
