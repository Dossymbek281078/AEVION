...
describe("подписи отделены от ключей", () => {
  ...
  test("у кнопок скоростей есть русские подписи — теперь из словаря", () => {
    // 08.09.2026 подписи переехали из зашитой карты в словарь модуля, чтобы
    // следовать языку страницы. Прежняя редакция требовала дословно
    // Bullet:"Пуля" в page.tsx и краснела НА ПОЧИНКЕ — сторож охранял способ,
    // а обещание было про другое: человек видит русские слова, а не ключи.
    const s = src();
    expect(s, "подпись снова берётся из ключа").not.toContain("<span>{c}</span>");
    // Подпись собирается из словаря по каждому ключу.
    expect(s).toContain('Bullet:cc.t("tc.bullet")');
    expect(s).toContain('Custom:cc.t("tc.custom")');
    // Ключи на месте — иначе сломается выбор категории.
    expect(s).toContain('["Bullet","Blitz","Rapid","Custom"]');
    // И русские значения существуют: словарь без них дал бы ключи на экране.
    const dict = fs.readFileSync(path.join(ROOT, "i18n.ts"), "utf-8");
    expect(dict).toContain('"tc.bullet": "Пуля"');
    expect(dict).toContain('"tc.custom": "Свой"');
  });
