...
describe("тестовые прогоны спрятаны, чужая работа — нет", () => {
  ...
  test("выдача многих проектов ОДНА — иначе вторая появится без фильтра", () => {
    // Урок соседнего окна, стоивший им пятой двери: запись, спрятанная в
    // списке, но открытая в другой выдаче, спрятана НАПОЛОВИНУ. Здесь путь
    // сегодня один; сторож заметит, если появится второй.
    const src = require("node:fs").readFileSync(
      require("node:path").join(__dirname, "..", "src", "routes", "devhub.ts"), "utf8");
    const многие = (src.match(/FROM "DevHubProject" WHERE "userId"/g) ?? []).length;
    expect(
      многие,
      "появилась вторая выдача многих проектов — проверьте, стоит ли на ней фильтр нашТестовыйПрогон",
    ).toBe(1);
    const зовущие = (src.match(/dbListProjects\(/g) ?? []).length;
    // одно объявление + один вызов
    expect(зовущие, "список проектов зовут из нового места — проверьте фильтр там").toBe(2);
  });
});
