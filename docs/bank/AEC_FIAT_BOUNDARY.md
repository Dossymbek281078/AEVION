...
- [ ] Определить `BUREAU_CERT_AEC_REWARD` (product decision: сколько AEC за Verified, Notarized, Filed)
- [ ] Реализовать Trust Graph edge record в `bureau.ts` (таблица `TrustEdge` или JSON-store)
