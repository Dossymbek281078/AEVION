...
vi.mock("../src/routes/provisioning", () => ({
  ...
  // И само правило «касается ли возврат действующей подписки». `true` =
  // отзываем, то есть поведение, которое этот тест и проверяет. Подмена
  // обязана отдавать ВСЮ поверхность, которой пользуется обработчик: стоит
  // добавить в модуль новый экспорт — и тест краснеет по причине, не
  // связанной со своим предметом (поймано дважды: 04.09 и 05.09).
  возвратКасаетсяДействующей: () => true,
}));
...
/**
 * Идентификатор платежа ПО ПОКУПКЕ: возврат обязан ссылаться на неё.
 *
 * Здесь стояло «каждый вызов свой, иначе сработает защита от повторной
 * доставки». Проверено 04.09.2026 — это не так: ключ дедупликации включает
 * статус (`${paymentId}:${result.status}`, payboxWebhook.ts:122), поэтому
 * `paid` и `refunded` с ОДНИМ идентификатором дают разные ключи и оба
 * проходят.
 *
 * А разные идентификаторы описывали кассу, которой не существует: у PayBox
 * возврат приходит тем же уведомлением с `pg_refund=1`, сохраняя
 * `pg_payment_id` исходного платежа (payboxProvider.ts:182,207).
 */
const платёжПоМодулю = new Map<string, string>();

function оплата(модуль?: string, вид = "paid") {
  статус = вид;
  const ключ = модуль ?? "(без модуля)";
  if (вид === "paid") {
    счётчик += 1;
    платёжПоМодулю.set(ключ, `pay-${счётчик}`);
  }
  полезная = {
    pg_user_contact_email: "buyer@example.com",
    pg_order_id: "aevion-lite-monthly",
    pg_payment_id: платёжПоМодулю.get(ключ) ?? `pay-${++счётчик}`,
...
beforeEach(() => {
  // Карта живёт внутри ТЕСТА, а не файла: иначе два теста, возвращающие
  // один и тот же модуль, шлют одинаковый идентификатор с одинаковым
  // статусом — и второй возврат честно попадает в дедупликацию, ломая
  // проверку, которая ждёт настоящей обработки.
  платёжПоМодулю.clear();
  upserts.length = 0;
  captured.length = 0;
  пуститьОшибку = false;
  статус = "paid";
