...
describe("DevHub не выдаёт память за сохранение", () => {
  ...
  // Было пять; шестое добавлено 28.08.2026 — создание снипета. Замер
  ...
  test("шесть мест объявляют локальный признак", () => {
  ...
  test("признак выставляется РОВНО в catch, а не угадывается", () => {
    ...
    expect(setInCatch.length, "подмена происходит в catch — там же и помечается").toBe(6);
  });
