/**
 * Ported from feat/bureau-v2 (commit b3c4a6e3) — confirmed absent from
 * main (no seed:bureau script / seed-bureau.ts anywhere in the repo).
 *
 * Seeds the Bureau registry with 8 demonstration IP certificates written
 * directly to Postgres so they survive restarts and are visible to every
 * process — useful for a fresh dev DB or a first demo.
 *
 ...
 *
 * Only touches the base QRightObject / QuantumShield / IPCertificate
 * columns that have existed since before the OTS / cosign / distributed-
 * Shamir columns were added — every column added later by
 * pipeline.ts's ensureTables() (qsignKeyVersion, authorPublicKey,
 * otsProof, fileHash, ...) is nullable or has a DEFAULT, so this script
 * stays compatible with the current, much richer schema without needing
