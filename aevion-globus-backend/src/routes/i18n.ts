  ...
  try {
    ...
    if (missTexts.length) {
      ...
      for (let j = 0; j < missIdx.length; j++) {
        // Слияние двух починок 08.09.2026 — они про РАЗНОЕ и нужны обе.
        // Их сторона (28.08): не кэшировать при degraded — отказ модели не
        // должен залипать на сутки Cache-Control. Наша (замер 28.07): не
        // кэшировать результат, РАВНЫЙ исходнику — 39 подписей модулей на
        // главной возвращались непереведёнными каждому немецкому гостю,
        // потому что кэш запомнил ответ-тождество. Взять только одну сторону
        // значило бы молча вернуть чужой дефект (правило 7в).
        const engine = batch.texts[j];
        const tr = engine ?? missTexts[j];
        out[missIdx[j]] = tr;
        if (!batch.degraded && shouldCache(missTexts[j], engine) && cache.size < MAX_CACHE) {
