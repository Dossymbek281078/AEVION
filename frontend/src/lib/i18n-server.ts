...
function parseAcceptLanguage(al: string): Lang | null {
  ...
  for (const part of parts) {
    ...
    if (!tag) continue;
    if (tag.startsWith("kk") || tag.startsWith("kz")) return "ru"; // kk falls back to ru
...
export function tServer(
  ...
  const effectiveLang = (lang as string) === "kk" ? "ru" : lang;
  const raw = translations[effectiveLang as Lang]?.[key] ?? translations.en[key] ?? key;
