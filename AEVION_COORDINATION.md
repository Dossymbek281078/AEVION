...
## ⚠️ Дубликат 10.08.2026, 20:00 — две ветки чинят числа платформы

Две сессии независимо нашли один дефект и написали одну и ту же правку.
Записано, чтобы третья не сделала это в третий раз и чтобы мерж не затёр
чужое.

**Дефект (подтверждён обеими):** сторож `pitchNumbers.guard.test.ts` обещал
«числа синхронны с реестром», а сравнивал константы сами с собой
(`expect(MODULE_NODES).toBe(37)`), реестр не открывая. В `projects.ts` уже
41 запись и 36 живых, боевой `/api/globus/projects` отдаёт те же 36, а витрина
писала 37 и 35 — занижение на четыре узла.

**Обе ветки пришли к одинаковым числам:** `MODULE_NODES = 40`,
`LIVE_MODULES = 36`. Совпадение независимое, значит числа верные.

| Файл | `fix/pitch-one-denominator` | `fix/explore-hero-deterministic-ru` | Кого брать |
|---|---|---|---|
| `data/pitchFacts.ts` | 41/9 | 14/9 | **их** — шире |
| `__tests__/pitchNumbers.guard.test.ts` | 251/9 | 93/8 | **их** — шире, есть проверка фраз и OG |
| `app/demo/page.tsx` | 20/7 | 85/27 | **моя** — значок «живое» по каждой метрике |
| `components/AutoTranslate.tsx` | 42/5 | 37/1 | разобрать руками, обе трогают |
| `components/SiteHeader.tsx` | — | есть | моя |
| `components/LanguageSwitcher.tsx` | — | есть | моя |
| `app/explore/page.tsx` | — | есть | моя |

**Почему проверка зон не спасла.** `session-claim.mjs pitchFacts` отвечал FREE:
чужая ветка заявляет зону `pitch`, а не `pitchFacts`, и сопоставление имён их
не связало. Вывод на будущее: перед правкой ОБЩИХ файлов данных
(`data/*.ts`, сторожа в `__tests__/`) проверять не только своё имя зоны, но и
`git diff --name-only main...<чужая ветка>` по соседним активным веткам —
имена зон коарзные, файлы точные.

## Находка 11.08.2026: витрина называет посетителю устаревший домен

На каждой странице `/explore` в подвале написано **`aevion.vercel.app`**, а сайт
живёт на `aevion.app` (оба отвечают 200, но брендовый — второй). Посетитель
читает адрес, которого мы не используем в письмах, на страницах и в заявках.

Три места, все в ЗАНЯТЫХ зонах — поэтому не правил, передаю владельцам:

| Файл | Строка | Что стоит | Зону держат |
|---|---|---|---|
| `app/explore/page.tsx` | 220 | `<a href="/">aevion.vercel.app</a>` | feat/compare-page, feat/free-fleet-keys, feat/pitch-monetization-funnel |
| `app/build/guide/page.tsx` | 251 | `aevion.vercel.app/build` | зона build |
| `app/qskyway/opengraph-image.tsx` | 83 | `aevion.vercel.app/qskyway` | зона qskyway |

Последнее особенно заметно: это картинка предпросмотра для соцсетей, её видят
все, кому отправляют ссылку на QSkyway.

**Не трогать** в `app/cyberchess/WorkspaceMediaPane.tsx` и `WorkspacePiP.tsx` —
там оба домена перечислены осознанно, как разрешённые родители для встраивания
Twitch, и удаление сломает плеер.

Проверено 11.08.2026: `https://aevion.app` → 200, `https://aevion.vercel.app`
→ 200. То есть это вопрос бренда, а не работоспособности.
