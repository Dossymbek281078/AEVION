...
describe("атрибуты пути новичка не зашиты по-русски", () => {
  ...
  /**
   * Обратный случай той же болезни, и он тише: подпись зашита ПО-АНГЛИЙСКИ.
   * Замер прода 08.09.2026, страница прочитана браузером: из 27 подписей на
   * русской витрине 26 были русскими, а одна — aria-label="Title" у поля
   * названия сниппета. Соседние поля той же формы уже брали подпись из
   * словаря; это «правильная функция есть, но зовут её не все».
   *
   * Цена не косметическая: aria-label СИЛЬНЕЕ placeholder, поэтому читалка
   * объявляла поле «Title» на всех языках, хотя переведённый placeholder
   * лежал строкой ниже. Доводчик сюда не дотянется — он правит текст, а не
   * атрибуты.
   *
   * Общий сторож английских подписей этот случай пропускает ОСОЗНАННО:
   * значение без пробела он считает идентификатором (KEY, welcome-v1,
   * src/x.tsx), иначе утонул бы в них. Однословная подпись для человека —
   * ровно та щель, и закрыть её можно только там, где известен словарь.
   */
  const LITERAL_ARIA = /aria-label="[^"]+"/g;

  test("прибор работает: видит литеральную подпись и молчит о словарной", () => {
    expect('<input aria-label="Title" />'.match(LITERAL_ARIA)?.length, "прибор ослеп").toBe(1);
    expect('<input aria-label={t("field.title")} />'.match(LITERAL_ARIA), "прибор клевещет на словарь").toBeNull();
  });

  for (const rel of NEWCOMER_FILES) {
    test(`${rel}: подписи для читалки берутся из словаря, а не строкой`, () => {
      const src = fs.readFileSync(path.join(APP, rel), "utf8");
      expect(
        src.match(LITERAL_ARIA) ?? [],
        "зашитая подпись: читалка объявит её так на ЛЮБОМ языке, доводчик атрибуты не правит",
      ).toEqual([]);
    });
  }
