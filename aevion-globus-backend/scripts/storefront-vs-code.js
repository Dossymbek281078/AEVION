...
/**
 * Ожидаемые имена товаров витрины. С 15.09.2026 таблица `STOREFRONT_NAME_TO_REFERENCE`
 * не литерал, а ВЫЧИСЛЯЕТСЯ из данных (ступени × приложения), поэтому прежний
 * разбор «ищем пары "Имя": "ссылка"» находил НОЛЬ — и вся половина проверки по
 * Lemon Squeezy молча не проверяла ничего (замер 22.09.2026: «ссылок в коде 0»).
 * Строим имена тем же правилом, что и код: «AEVION <Что> — <Ступень> (N mo)».
 */
function readNameMap() {
  const src = fs.readFileSync(PRICING_TS, "utf8");
  const map = {};
  const ступени = [];
  const мес = src.slice(src.indexOf("TERM_MONTHS"), src.indexOf("TERM_MONTHS") + 220);
  for (const m of мес.matchAll(/([a-z]+):\s*(\d+)/g)) ступени.push({ ключ: m[1], мес: Number(m[2]) });
  const имена = {};
  const блокИмён = src.slice(src.indexOf("TERM_NAME"), src.indexOf("TERM_NAME") + 220);
  for (const m of блокИмён.matchAll(/([a-z]+):\s*"([^"]+)"/g)) имена[m[1]] = m[2];
  const приложения = [...src.matchAll(/\{ slug: "([^"]+)", moduleId: "[^"]+", name: "([^"]+)"/g)]
    .map((m) => ({ slug: m[1], name: m[2] }));

  for (const с of ступени) {
    const имя = имена[с.ключ] || с.ключ;
    map[normName(`AEVION Planet — ${имя} (${с.мес} mo)`)] = `tier_${с.ключ}`;
    for (const a of приложения) {
      map[normName(`AEVION ${a.name} — ${имя} (${с.мес} mo)`)] = `app_${a.slug}_${с.ключ}`;
    }
  }
  return map;
}
...
(async () => {
  ...
  const узнали = store.filter((i) => nameMap[i.name]).length;

  // ⚠️ ГРАНИЦА, названная 22.09.2026. С 15.09 лестница (5 ступеней × 6 позиций)
  // живёт ВАРИАНТАМИ внутри одного товара "AEVION Planet" (id 1365937), а
  // публичная страница витрины показывает только ТОВАРЫ. То есть проверить
  // цены и периоды лестницы по этой странице нельзя ни при какой разметке —
  // их там нет. Прежняя версия считала «товаров меньше, чем ссылок» признаком
  // сломанного разбора и отвечала кодом 2 навсегда; вечное «не знаю» читают
  // ровно так же, как вечное «всё хорошо», — то есть не читают.
  //
  // Поэтому исходов три, а не два:
  //   ни одного имени лестницы   -> так устроена витрина: печатаем границу,
  //                                 проверяем то, что страница ПОКАЗЫВАЕТ;
  //   часть имён есть, часть нет -> разметка поехала, разбор НЕПОЛОН (код 2);
  //   все имена на месте         -> полная сверка.
  if (узнали > 0 && узнали < nuzhno) {
    console.error(
      `storefront-vs-code: узнано ${узнали} имён лестницы из ${nuzhno} при ${store.length} разобранных товарах — ` +
  ...
  //
  // Под границей (лестница заведена вариантами, страница показывает товары)
  // эта проверка спрашивает у страницы то, чего на ней нет ПО УСТРОЙСТВУ, и
  // даёт ровно столько ложных тревог, сколько позиций в лестнице — замер
  // 22.09.2026: 50 из 50. Сторож, красный целиком и всегда, перестают читать
  // в первый же день, и вместе с ним перестают читать Gumroad-половину,
  // которая честно работает. Вопрос «заведены ли позиции в кабинете» при этом
  // НЕ снимается — на него отвечает счёт заданных вариантов в
  // /api/payment/healthz, и граница выше называет его прямо.
  if (узнали > 0) {
