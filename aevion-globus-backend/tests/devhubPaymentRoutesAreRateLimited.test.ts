...
describe("платёжные ручки DevHub под пределом частоты", () => {
  test("после серии запросов приходит 429 — и первые проходят", async () => {
    ...
    // С 15.09.2026 бесплатному гостю ручка отвечает 402 ИЗ обработчика (после ограничителя),
    // то есть запрос до него дошёл. Прежде было 400 «name required» — та же роль.
    expect(kody[0], "первый запрос не дошёл до обработчика").toBe(402);
    expect(kody.filter((k) => k === 402).length, "предел бьёт по годным").toBeGreaterThan(20);
