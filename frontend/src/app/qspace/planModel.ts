...
export interface Wall {
  ...
  thickness: number;
  /**
   * Несущая — ПРЕДПОЛОЖЕНИЕ по чертежу: закрашенный контур (заливка) или толщина ≥ 0.25 м.
   * Юридически ничего не значит: снос и проёмы согласует проектировщик. В 3D — тёмные торцы.
   */
  bearing?: boolean;
  /**
   * Толщина ПРИНЯТА нами, а не измерена по чертежу: одиночная линия без пары,
   * которой мы сами назначили 0.10–0.15 м. У таких линий нет обещания «это
   * перегородка»: ими же начерчены шкафы, ванны и кухонные блоки в
   * дизайн-проекте. Для разбивки на комнаты это различие решающее — см.
   * «мебель не режет помещение» в rooms.ts.
   */
  assumed?: boolean;
  /** высота, м */
  height: number;
  /**
   * Стеклянная стена: витраж или окно во всю высоту, пришедшее со своего слоя
   * чертежа. Для разбивки на комнаты — граница (иначе открытая планировка
   * с витражным фасадом «утекает» на улицу и комнатой не становится), в 3D —
   * прозрачная панель, в смете стен под отделку — не считается.
   */
  glass?: boolean;
}
...
export interface Plan {
  ...
  /** стены неточные (картинка, PDF без слоёв): дверь — и торец перегородки против стены, допуск соосности шире */
  looseWalls?: boolean;
}
