...
### Проверка авто-слитых файлов (10.08.2026, ветка `-merged`)

Мерж дал не только 4 конфликта: ещё 4 общих файла git слил АВТОМАТИЧЕСКИ.
Текстовый успех — не смысловой, поэтому проверены отдельно:

- `frontend/src/lib/gumroad.ts` — денежный путь, обе стороны правили. Мой
  сторож от прототипа (`Object.prototype.hasOwnProperty.call`, коммит b64793b9c:
  `GUMROAD_PERMALINKS["constructor"]` возвращала функцию Object, и из неё
  собирался URL чекаута) на месте. Правка main (+16/−3, коммит 2d4384913 —
  примечание, что devhub/smeta/qventure/bureau/qpaynet/cyberchess/qcontract
  продаются через LemonSqueezy, а не Gumroad) легла рядом, не поверх.
  Гвардия `gumroadPermalinkGuard.test.ts` на слитом дереве зелёная.
- `frontend/src/lib/i18n-data.ts` — гейт i18n-parity (en/ru/kk) на слитом
  дереве зелёный.
- `aevion-globus-backend/scripts/all-smokes.js` — гвардия
  `qskywaySmokesWired.test.ts` зелёная, смоки доверия из общего прогона не
  выпали.
- `aevion-globus-backend/package.json` — гвардия `npmScriptRunners.test.ts`
  зелёная, ни один скрипт не зовёт несуществующий раннер.

**main не тронул НИ ОДНОГО файла QSkyway** за 182 коммита — по модулю мерж
чистый по построению, а не по везению.

Итог слитого дерева: фронтенд 559/559, бэкенд 1636 прошло при 2 падениях
(оба — чужой `devhub-integrations.test.ts`, плавающие моки), `tsc` обеих частей
чистый, i18n-parity зелёный.
...
### Финальная проверка сдаточной ветки (10.08.2026, 21:0x)

Прогнано на `feat/qskyway-airspace-trust-merged` @ `38f9ed73f` — ровно то дерево,
которое пойдёт в main:

| проверка | результат |
|---|---|
| `npm test` (оба пакета) | бэкенд 1643 прошло / 8 skipped, фронт 570 — **0 падений**, 113 с |
| `tsc --noEmit` бэкенда | чисто |
| `tsc --noEmit` фронта (из `frontend/`) | чисто |
| i18n-parity en/ru/kk | зелёный |

Про `tsc` отдельно: запускать его НАДО из каталога пакета. Из корня подхватится
другой бинарник, который печатает «This is not the tsc command you are looking
for» и выходит с нулём — выглядит как успех и не проверяет ничего.

Что мерж меняет для пользователя, помимо кода: снимает с прода ложное ⚠ на
регуляторном чипе Нью-Йорка и перестаёт выдавать 33 смоук-брони за глубину
рынка слотов.
...
**Единственный непроверенный твин — Токио.** ⛔ УСТАРЕЛО той же ночью: Токио
проверен, см. раздел «Токио проверен» ниже. Абзац оставлен, потому что в нём
записана причина задержки; читать его как открытый пункт — нельзя.
Сравнение требует ~420 МБ PLATEAU,
...
## Вечер 12.08.2026 — семнадцать коммитов честности

Ветка `feat/qskyway-airspace-trust`, всё продублировано в
`feat/qskyway-airspace-trust-merged` (она поедет на прод), обе в `mirror`.

### Что изменилось в поведении

1. **Оговорка о границах теперь ПОД подписью.** Была полем ответа рядом с
   `document`: пара `{document, attestation}`, переданная дальше, проверялась как
   подлинная уже без неё. Комментарий в коде обещал ровно эту защиту, структура
   её не держала. Добавлены `scopeEn` (английская версия там же, под подписью),
   `authorityEn` в `CityPermission` и `noteEn` в ответе проверки документа.
2. **Подстановка высоты по типу помечена в данных** — `dataQuality.substituted`
   во всех трёх твинах (Астана 38, Нью-Йорк 1, Токио 1). Видна на странице
   чипом, названа в подписанном документе (`substitutedHeights`) и измерена
   ручкой `GET /height-substitution`.
3. **Панель площадок и легенда карты** называют полный запрет полётов
   (Астана, UAP28, 100 % твина). Легенда приведена к словам списка: было
   «годна» против «кандидат на площадку».
4. **Карточка спорной высоты** без разбора больше не печатает «против 0 м в
   статье объекта» — говорит, что второго числа нет.

### Числа, которые стоит знать следующему

- Подстановка ДОХОДИТ до коридоров: Астана 23 маршрута из 42 (27 зданий из 38),
  Нью-Йорк 16 из 42, Токио 1 из 42. У спорной высоты по Астане — **0 из 42**.
  Два похожих на слух показателя дают противоположные ответы.
- Нью-Йорк: вокзал получил 171 м вместо 12 по семи известным высотам своего
  типа; эшелон над ним поднялся на 87.5 м. Из 42 пар изменились 8 (длина у 6,
  максимум +0.08 км), ни один маршрут не потерян.
- Пересборки: у Астаны дрейфа не было (0 ячеек из 11040), у Токио 2, у
  Нью-Йорка 42 — и там эффекты разделены флагом `--no-type-median`: дрейф 2,
  подстановка 40.

### Грабли, на которые я наступил (не наступайте снова)

- **Объявление `CityData` живёт в ШАПКЕ генерируемого файла** `qskyway.city.ts`.
  Правка типа руками стирается первой же пересборкой; править надо шаблон в
  `scripts/fetch-city-twin.mjs`. Ни vitest, ни `ts-node-dev --transpile-only`
  этого не видят — ловит только `tsc --noEmit`, поэтому после пересборки
  запускать его ПЕРВЫМ.
- **`ts-node-dev --respawn` оставляет обёртку после убийства слушателя.** За
  вечер накопилось 17 процессов и 3.9 ГБ. Порядок: `TaskStop` фоновой задачи →
  убить слушателя по PID (сверив путь worktree) → проверить сирот по своему
  worktree.
- **Тест на одном городе не ловит счётчики.** Проверка нового поля гоняла
  Нью-Йорк с ЕДИНСТВЕННОЙ подстановкой и пропустила дефект, который на Астане
  давал «участков 15, зданий 30». Живой ответ поймал, тест — нет.
- **Полный набор бэкенда краснеет под нагрузкой** (таймауты динамического
  `import`) и от недоступного календаря OpenTimestamps. Отличать по времени
  прогона (норма ~30 с) и `OT_SKIP_NETWORK=1`.

### Состояние проверок на конец дня

247 тестов qskyway, 1606 в полном наборе бэкенда, 509 фронтенда, `tsc` чист,
`next build` зелёный, живой смок модуля **127/127** (`BASE=http://127.0.0.1:<порт>
READ_ONLY=1 node scripts/qskyway-smoke.js`).

**Ничего из этого на проде нет:** прод стоит на сборке от 27.07, выкатка — из
панели Railway, то есть за основателем.

---

## 13.08.2026 — рынок прав, квитанция и ключ подписи

Осмотрена последняя непроверенная поверхность модуля — рынок 4D-слотов. Это
единственное место, где выдаётся ПРАВО, и там нашлась цепочка из пяти вещей
одного класса: сказано сильнее, чем сделано.

### Что изменилось в поведении

1. **Рынок больше не подменяет хранилище на ходу.** При ошибке Postgres чтение
   отвечает 503 «рынок недоступен» (раньше — пустой список, то есть сбой
   выдавался за «слотов нет»), а бронь отказывает без квитанции (раньше —
   писалась в память и отдавала «право зафиксировано», хотя список и лимит
   читаются из базы). Память осталась законным режимом ТОЛЬКО там, где Postgres
   не настроен вовсе, и `store: "memory"` говорит об этом прямо.
2. **Квитанция называется контрольной суммой, а не «якорем»** — якорь в этом
   модуле занят привязкой слоя ограничений к Bitcoin. Появилась настоящая
   проверка `GET /slots/:id/verify`; формула, написанная в коде трижды, сведена
   к одной функции `slotReceipt`.
3. **`GET /verify` везёт оговорку о ключе.** Без `QSKYWAY_SIGN_SK` ключ подписи
   генерируется при старте процесса; ответ теперь несёт `ephemeral` и пояснение
   на двух языках, страница показывает «· ключ временный».
4. **Повреждённый ключ считается временным.** Признак брался по НАЛИЧИЮ
   переменной; обрезанный base64 давал `ephemeral: false` при временном ключе.
5. **Оговорки регулятора — на двух языках** (12–13.08): `regimeEn`, `noteEn`,
   `provenanceNoteEn`, вердикт по потолку, примечание слоя, отказ строгого
   режима. У Токио поле `regime` было заполнено ПО-АНГЛИЙСКИ — русскому
   читателю показывали английский текст.

### Что нужно на проде

`QSKYWAY_SIGN_SK` (Ed25519, base64 PKCS8 DER) — внесена в `AEVION_DEPLOY.md` с
командой генерации. Без неё подписи двойника и значения на странице Trust всей
платформы привязаны к одному запуску процесса.

### Приёмы, которые здесь сработали

- **Грепом по словарю модуля.** `grep -rn "якор"` нашёл два места с одним словом
  и разным весом гарантии. Тем же приёмом проверены «подпись», «доказательство»,
  «гарантия» — там чисто.
- **Проверка проверки.** Первый тест на квитанцию ловил мутацию «сумма не
  зависит от записи», но НЕ ловил «ручка всегда отвечает „сходится“»: покрыта
  была формула рядом, а не сравнение внутри. Для второго тесту нужен доступ к
  ХРАНИМОЙ записи — открыт `__slotStoreForTests`.
- **Мок, который не воспроизводит дефект.** Тест на сбой базы сначала ронял ВСЕ
  запросы, включая создание таблицы, — модуль честно уходил в законный режим
  памяти и отвечал 200/201. Опасен разрыв НА ХОДУ: мок обязан пропустить
  инициализацию (опознавать её по тексту SQL, а не счётчиком «первый вызов»).

### Состояние проверок

277 тестов qskyway, 1640 полного набора бэкенда, 518 фронтенда, `tsc` чист,
`next build` зелёный, живой смок **129/129** (только чтение) и **137/137**
(с записью).

### Что осталось

Страница модуля по-прежнему русская: 119 видимых строк, из них 40 длинных
пояснений. Половинчатый перевод даст экран на двух языках сразу — это хуже
последовательного русского, поэтому отдельный заход. Серверная часть, которую
страница перевести не могла в принципе, закрыта.
