...
describe("DevHub: надстройка только там, где выдача работает", () => {
  ...
  // 14.09.2026: DevHub ВХОДИТ в Full (решение основателя), цену надстройки он не назначал.
  // Прежняя редакция требовала цену безусловно — решала за основателя. Теперь признание
  // плана требует одного из двух: надстройка с ценой ИЛИ модуль входит в Full.
  test("если модуль научился читать план — он доступен надстройкой ИЛИ входит в Full", () => {
    // Та же правка: раньше выходили раньше при honoursPlan=false.
    expect(
      !honoursPlan || (typeof entry?.addonMonthly === "number" && (entry.addonMonthly as number) > 0) || (entry?.includedIn ?? []).includes("full"),
