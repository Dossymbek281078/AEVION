...
describe("отдельная подписка на модуль открывает этот модуль", () => {
  ...
  test("сбой базы закрывает доступ, а не открывает", async () => {
    ...
    expect(allowed).toBe(false);
    // 503, а не 402. Правка b3d88733e (02.09.2026) развела два ответа, которые
    // раньше выглядели одинаково:
    //   402 — «вы не платили»       утверждение о ЧЕЛОВЕКЕ
    //   503 — «мы не смогли узнать» утверждение о НАС
    // Главное прежнее и проверяется строкой выше: allowed === false, сбой
    // ЗАКРЫВАЕТ доступ. Изменился адресат обвинения: прежнее 402 отправляло
    // ЗАПЛАТИВШЕГО покупать второй раз и попадало в recordDeny — сбой
    // инфраструктуры искажал цифру, по которой решают, что продавать.
