...
// перебирает все живые worktree через git — на этой машине их 18. В одиночку
// файл проходит за ~33 с, в общем прогоне вместе с остальными 119 файлами
// отдельные случаи упираются в дефолтные 10 с и краснеют (поймано 10.08.2026:
// «известный модуль по-прежнему проверяется» — Test timed out in 10000ms при
// 52 с на файл). Это не поломка инструмента, а цена честной проверки на живом
// состоянии: тест намеренно спрашивает git, а не мок. Даём ему запас, вместо
// того чтобы мигать.
