...
describe("публичная подпись не выдаёт личность и место", () => {
  ...
  it("тип публичного ответа тоже сужен — иначе поля вернут молча", () => {
    // Тип это второе место, где видно, что уходит наружу. Если он по-прежнему
    // разрешает userId и координаты, вернуть их можно одной строкой и tsc
    // промолчит.
    expect(TYPES.includes("issuer: { email: string | null } | null")).toBe(true);
    expect(/lat:\s*number \| null/.test(TYPES), "тип всё ещё разрешает координаты").toBe(false);
  });
