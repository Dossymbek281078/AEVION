...
>
> **Важный контекст:** AEVION — это не портфель полированных продуктов.
> Это доказательство скорости идей одного человека. Каждый продукт — MVP:
> концепция подтверждена, технический фундамент заложен, пользовательская
> полировка — работа вашей команды. Именно так устроена «мозг + руки» модель.

---

## 1. Что проверять технической команде

### Что впечатляет инженера (смотрите сюда)

| URL / Команда | Что это доказывает |
|---------------|-------------------|
| **github.com/Dossymbek281078/AEVION** | 130+ PR, 500+ коммитов — один человек за 6 месяцев |
| `git log --oneline \| wc -l` | Объём работы — верифицируется по истории |
| `curl .../api/aevion/registry` | 30+ модулей с реальными backend-эндпоинтами |
| `curl .../api/qsign/v2/sign` | ML-DSA-65 FIPS 204 — постквантовая подпись в проде |
| `npm test` в backend | 490+ тестов — кто-то думал об архитектуре |
| **aevion.app/constitution** | Реальный документ с криптографической атестацией |
| **aevion.app/transparency** | Health-board — прозрачность, ничего не скрыто |

### Что НЕ нужно оценивать как готовый продукт

| Продукт | Честный статус | Что это доказывает вместо этого |
|---------|---------------|--------------------------------|
| CyberChess | Механика отличная, UX перегружен, пазлы дорабатываются | Идея рабочая, нужна команда на polish |
| AEVION Bank | 30+ экранов UI, но без лицензии и real transactions | Scope понимания банкинга впечатляет |
| HealthAI | Рабочий screener, но без медверификации контента | Правильная гипотеза, нужна команда |
| DevHub | 9 интеграций работают, UX требует polish | Концепция подтверждена |
| QTrade | Торговый UI, без реальной ликвидности | Понимание trading mechanics есть |

**Правильный вопрос технарю:** «Что вы можете сделать из ЭТОГО за 6 месяцев с командой 20 человек?» — а не «Готово ли это к production прямо сейчас?»
