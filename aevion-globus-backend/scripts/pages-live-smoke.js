...
const PAGES = [
  ...
  "/qright",
  "/qsign",
  // Чек цепочки: последний экран пути «авторство -> договор -> подпись ->
  // выплата». Без параметров отдаёт пустое состояние и код 200 — этого
  // стражу и достаточно: он спрашивает «ответил ли сервер», а не «что
  // увидел человек» (ворота §3 отвечают на второй вопрос отдельно).
  "/chain",
  "/bureau",
  "/planet",
  "/awards",
  ...
  /*
   * Короткие ссылки западного запуска. Их публикуют В ПОСТЕ на Hacker News и
   * Product Hunt, то есть исправить опечатку после публикации уже нельзя —
   * ссылка живёт в чужом посте навсегда. До 08.09.2026 они отвечали 404, и
   * это была единственная причина не публиковать.
   *
   * ГРАНИЦА: сторож идёт ПО перенаправлению и проверяет конечную страницу.
   * Он поймает главный риск — что /hn снова станет 404 или уведёт на битый
   * адрес. Он НЕ поймает подмену цели на другую ЖИВУЮ страницу: если /hn
   * начнёт вести на главную, метка канала пропадёт, а сторож останется
   * зелёным. Проверка самой цели живёт в тестах каталога (products.ts).
   */
  "/hn",
  "/ph",
  "/qskyway",
