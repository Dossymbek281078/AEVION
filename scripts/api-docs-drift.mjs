...
//        /api/bureau/protect        → реально pipelineRouter.post("/protect")
//        /api/bureau/protect-batch  → реально pipelineRouter
//        /api/planet/submit         → реально venturesRouter
//        /api/qright/register       → реально authRouter ("/api/auth/register")
//        /api/qsign/batch           → реально qcoreaiRouter ("/api/qcoreai/batch")
//      Это опечатка в обещании: путь можно поправить, поведение есть.
//
//   2. Ручки НЕТ НИГДЕ — обещано то, чего не написано:
//        /api/quantum-shield/derive|recover|rotate
//        /api/auth/sign-out-everywhere
//        /api/qcoreai/history, /api/qright/verify, /api/qsign/file
//      Здесь выбор за владельцем продукта: завести или убрать из документации.
//      Скрипт намеренно НЕ предлагает второе автоматически.
//
