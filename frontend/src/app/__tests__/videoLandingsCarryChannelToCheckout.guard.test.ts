...
describe("посадочные под ролики не теряют метку канала у кассы", () => {
  ...
  it("метка берётся из адреса, а не выдумывается", () => {
    for (const rel of VIDEO_LANDINGS) {
      const s = src(rel);
      // Читать адрес НАПРЯМУЮ больше не требуется и не поощряется: с 31.08
      // источник канала один — `channelNow()`, он же помнит метку через кассу.
      // Сторож проверяет ЦЕЛЬ (канал доходит до кассы), а не прежний способ.
      expect(
        s.includes("channelNow(") || s.includes('get("c")'),
        `${rel} не спрашивает канал ни общим источником, ни из адреса`,
      ).toBe(true);
      // Метка обязана сверяться со списком известных каналов: иначе в отчёт
      // уехало бы любое `?c=` из чужой ссылки. Сверяет либо `channelFrom`
      // напрямую, либо `channelNow()` — он делает это внутри, и то, что делает,
      // проверено отдельно (channelSurvivesCheckout: «мусор из хранилища не
      // становится каналом»).
      expect(
        s.includes("channelNow(") || s.includes("channelFrom("),
        `${rel} не сверяет метку со списком каналов`,
      ).toBe(true);
    }
  });
});
 ...
 */
// 15.09.2026: единственная запись долга (constitution/page.tsx) закрыта не
// починкой метки, а снятием товара — Constitution Pro на Gumroad больше не
// продаётся, прямой ссылки в чужую кассу на странице нет вовсе. Она ведёт на
// /pricing, а туда метку доводит keepChannel. Долг пуст.
const RAW_CHECKOUT_DEBT = new Map<string, string>([]);
