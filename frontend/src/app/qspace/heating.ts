...
export function stepHint(stepM: number): string {
  // Число берётся ИЗ аргумента, а не переписано в тексте. Прежде первые две
  // ветки возвращали «10 см» и «15 см» при порогах 0.11 и 0.16: шаг 12 см
  // подписывался как десять, то есть подпись говорила не о том значении,
  // которое выбрал человек. Третья ветка была сделана правильно и стала
  // образцом — тот же случай, что в нормах вентиляции.
  const см = Math.round(stepM * 100);
  if (stepM <= 0.11) return `${см} см — санузел и зона у окна, где нужно теплее`;
  if (stepM <= 0.16) return `${см} см — обычная жилая комната`;
  return `${см} см — экономный шаг, пол будет прохладнее`;
}
