...
const PAGES = [
  ...
  // Вход и последний шаг регистрации. Их отсутствие в этом списке — самый дорогой
  // пробел: если /auth перестанет открываться, войти не сможет НИКТО, а узнаем мы об
  // этом от человека, а не от сторожа. /build/verify-email — страница, на которую
  // ведёт ссылка из письма подтверждения адреса; сломается она — регистрация тихо
  // перестанет завершаться (проверено 21.08: ссылку письмо строит именно на неё).
  "/auth",
  "/build/verify-email",
  // Продающая страница кейсов: на неё ведут ссылки из /pricing, и её падение
  // выглядит как «у них ничего нет», а не как ошибка.
  "/pricing/cases",
];
