...
describe("страница показывает высоту ЭТОГО плана, а не константу", () => {
  ...
  it("поле высоты видно НА ЛЮБОМ слое, а не только на черновом", () => {
    // Высота нужна всем слоям: по ней считается и штукатурка, и краска.
    // Первая редакция стояла внутри блока {layers.rough && ...} — человек,
    // смотрящий чистовую отделку, поля не видел, хотя его расход от неё и
    // зависит. Проверка структурная: поле обязано стоять ВЫШЕ условия слоя.
    const iПоле = client.indexOf('id="qspace-height"');
    const iСлой = client.indexOf("{layers.rough && (");
    expect(iПоле, "поля высоты нет вовсе").toBeGreaterThan(0);
    expect(iСлой, "контроль: условие слоя не найдено — проверка ничего не значит")
      .toBeGreaterThan(0);
    expect(iПоле, "поле высоты спрятано внутрь чернового слоя").toBeLessThan(iСлой);
  });
