...
function dailyBuckets() {
    ...
    for (let i = EARNINGS_DAILY_WINDOW - 1; i >= 0; i--) {
        const d = new Date(todayUtcMidnight.getTime() - i * 86400000);
...
exports.ecosystemRouter.get("/earnings", (req, res) => {
    ...
    const addToDay = (iso, key, amount) => {
        const bucket = buckets.get(iso.slice(0, 10));
        if (bucket)
            bucket[key] += amount;
    };
    for (const x of r)
        addToDay(x.paidAt, "qright", x.amount);
    for (const x of c)
        addToDay(x.finalizedAt, "chess", x.amount);
    for (const x of p)
        addToDay(x.certifiedAt, "planet", x.amount);
...
function trimSummary(s, max = 160) {
...
exports.ecosystemRouter.get("/activity", async (req, res) => {
        ...
        try {
            ...
            for (const r of rows) {
                items.push({
                    ...
                    at: r.publishedAt instanceof Date
    ...
    if (sources.has("qright")) {
        try {
            ...
            const params = mine ? [userId, email] : [];
            params.push(perSourceCap);
            ...
            for (const r of rows) {
                items.push({
                    ...
                    at: r.createdAt instanceof Date
    ...
    if (sources.has("earnings")) {
        for (const r of exports.royaltyEvents) {
            if (r.email !== email)
                continue;
            items.push({
                id: `earn:r:${r.id}`,
        ...
        for (const c of exports.chessPrizes) {
            if (c.email !== email)
                continue;
            items.push({
                id: `earn:c:${c.id}`,
        ...
        for (const p of exports.planetCerts) {
            if (p.email !== email)
                continue;
            items.push({
                id: `earn:p:${p.id}`,
    ...
    for (const it of items)
        bySource[it.module] = (bySource[it.module] || 0) + 1;
    res.json({
...
// ───────────────────────────────────────────────────────────────────────────
exports.ecosystemRouter.get("/graph", async (_req, res) => {
    try {
        ...
        const royaltyByProduct = new Map();
        for (const r of exports.royaltyEvents) {
            if (r.email !== email)
                continue;
            royaltyByProduct.set("qright", (royaltyByProduct.get("qright") || 0) + 1);
        }
        const userChess = exports.chessPrizes.filter((x) => x.email === email).length;
        const userPlanet = exports.planetCerts.filter((x) => x.email === email).length;
        const userTouch = {
        ...
        const byHealth = matrix.reduce((acc, m) => {
...
exports.ecosystemRouter.get("/health-matrix", (_req, res) => {
    try {
        const enriched = (0, moduleRuntime_1.enrichProjects)(projects_1.projects);
        const matrix = enriched.map((p) => ({
            id: p.id,
            ...
            apiCount: (p.runtime.apiHints || []).length,
            primaryPath: p.runtime.primaryPath || null,
            hasRuntime: !!moduleRuntime_1.MODULE_RUNTIME[p.id],
        }));
        res.json({
            generatedAt: new Date().toISOString(),
            total: matrix.length,
