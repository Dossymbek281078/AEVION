...
describe("дверь пути файла отказывает мусору сама", () => {
  test.each([
    ...
    // Дверей на этом пути теперь ДВЕ, и это защита в глубину, а не дубль.
    // 08.09.2026 добавлена прослойка роутера против управляющих байтов в
    // адресе (замер прода: `%00` давал 503 «Хранилище недоступно» — клиентская
    // ошибка притворялась нашей аварией). Она срабатывает РАНЬШЕ и отвечает
    // `invalid_path`; дверь файла со своим `invalid file path` осталась на
    // месте и ловит остальное — обратные слэши, ведущий слэш, «..», пустые
    // сегменты. Утверждение прежнее по смыслу: мусор не проходит и получает
    // человеческий отказ; какая из двух дверей закрылась — не важно.
    expect(
      ["invalid file path", "invalid_path"].some((код) => String(r.body.error).includes(код)),
      `отказ пришёл, но без понятного кода: ${JSON.stringify(r.body)}`,
    ).toBe(true);
  });
