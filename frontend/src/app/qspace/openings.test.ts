...
describe("типоразмеры проёмов осмысленны", () => {
  // Мутация «подоконник окна = 0» не ловилась ничем: окно молча стало бы
  // французским — от пола до 1.45 м. В трёхмерном виде это выглядит как
  // ошибка модели, а человек решит, что модуль неверно прочёл его чертёж.
  it("у двери порога нет, у окна подоконник есть", () => {
    expect(PRESETS.door.sill, "у двери появился порог").toBe(0);
    expect(PRESETS.window.sill, "окно начинается от пола — это уже не окно")
      .toBeGreaterThan(0.3);
    expect(PRESETS.window.sill, "подоконник выше пояса — окно не откроешь")
      .toBeLessThan(1.2);
  });

  it("проём не выше стены и не ниже человека", () => {
    for (const [имя, p] of Object.entries(PRESETS)) {
      expect(p.sill + p.height, `«${имя}» выше стены 2.7 м`).toBeLessThanOrEqual(2.7);
      expect(p.height, `«${имя}» ниже 1.2 м — в него не пройти и не посмотреть`)
        .toBeGreaterThan(1.2);
      expect(p.width, `«${имя}» уже 0.5 м`).toBeGreaterThan(0.5);
    }
  });

  it("контроль прибора: заведомо плохой типоразмер НЕ прошёл бы", () => {
    const плохой = { width: 0.9, height: 2.05, sill: 1.5 };
    expect(плохой.sill + плохой.height).toBeGreaterThan(2.7);
  });
});
