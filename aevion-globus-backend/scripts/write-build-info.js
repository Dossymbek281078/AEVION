...
// Замер снимается ЗДЕСЬ, до записи. Первая версия спрашивала existsSync ПОСЛЕ
// writeFileSync и потому всегда отвечала «была» — измеряла файл, который сама
// же только что и создала.
const отметкаДоехала = existsSync(target);
...
// Диагностика 24.09.2026. Симптом: на проде commit=unknown ПРИ ТОМ, что код
// выкачен штатным скриптом, который отметку кладёт и проверяет git-исключения.
// Значит файл не доехал в контекст сборки — но «не доехал» и «доехал и был
// перезаписан» в журнале выглядели одинаково. Теперь строка называет ЭТО прямо,
// и следующая же чужая сборка ответит на вопрос без отдельного эксперимента.
//
// Проверены и ОТВЕРГНУТЫ (24.09): .gitignore (файл специально не игнорируется,
// скрипт это проверяет и останавливается), корневой .railwayignore (в нём
// только каталоги фронтенда и тяжёлые данные), подмена GIT_SHA (запрещена §12).
console.log(
  `[build-info] commit=${info.commit} source=${info.source} branch=${info.branch} ` +
    `отметкаВКонтексте=${отметкаДоехала ? "была" : "НЕ ДОЕХАЛА"}`,
);
if (info.source === "none") {
  console.warn(
