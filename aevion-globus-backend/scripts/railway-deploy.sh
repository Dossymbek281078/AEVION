...
# trap ДО создания файла, а не после: окно между ними узкое, но если процесс
# умрёт именно там (Ctrl+C, нет места), отметка останется в рабочей копии — а
# она не в игноре, значит уедет следующим `git add .` и соврёт коммитом
# несостоявшейся выкатки. `rm -f` по несуществующему файлу безвреден.
