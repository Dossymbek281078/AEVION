...
export default function AccountPage() {
  ...
  /*
   * Названия тарифов с сервера: id → человеческое имя.
   *
   * Вторую карту на клиенте не заводим намеренно — каталог правят в одном
   * месте, и локальная копия разошлась бы с ним молча, а расхождение здесь
   * видит покупатель. Ответ ручки кэшируется браузером, запрос дешёвый.
   *
   * Пусто при отказе, и это осознанно: показать идентификатор честнее, чем
   * пустое место, где человек ждёт название своего тарифа.
   */
  const [тарифы, setТарифы] = useState<Record<string, string>>({});

  useEffect(() => {
    let живо = true;
    fetch(apiUrl("/api/pricing/tiers"))
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => {
        if (!живо || !Array.isArray(d?.items)) return;
        const карта: Record<string, string> = {};
        for (const t of d.items) {
          if (typeof t?.id === "string" && typeof t?.name === "string") карта[t.id] = t.name;
        }
        setТарифы(карта);
      })
      .catch(() => {});
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    <div>
                      ...
                      {/*
                        ...
                        шести идентификатор и название совпадают (free→Free,
                        lite→Lite, medium→Medium, full→Full). Расходится `pro`.

                        Разбудила это чужая починка: до неё кассы физически не могли
                        записать «pro» — он проваливался в «lite». Дефект был старше
                        починки, но проявился после неё.

                        Имя берём у сервера, а не заводим вторую карту на клиенте:
                        каталог тарифов правят в одном месте, и локальная копия
                        разошлась бы с ним молча. Не ответил — показываем то, что
                        знаем, а не пустоту.
                      */}
                      <span style={{ fontWeight: 800, textTransform: тарифы[sub.tierId] ? "none" : "capitalize", color: "#0d9488" }}>
                        {тарифы[sub.tierId] ?? sub.tierId}
                      </span>
                    </div>
                    <div>
