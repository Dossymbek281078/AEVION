...
function serveDocument() {
  mockQuery.mockImplementation(async (sql: string) => {
    ...
    // Атомарное списание просмотра появилось позже этого теста (ветка
    // fix/qcontract-atomic-burn): без ответа на него обработчик падает на
    // `claim.rows[0]` и отдаёт 500 — то есть тест краснел бы не по своей теме.
    if (/^UPDATE qcontract_documents/i.test(String(sql).trim())) {
      return { rows: [{ view_count: 1 }], rowCount: 1 };
    }
