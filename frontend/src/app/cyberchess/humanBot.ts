 ...
 */
/* Паспорт соперника — что игрок вправе знать до начала партии.
 *
 * У Lichess и Chess.com уровень бота — это просто число, и чем оно отличается от
 * соседнего, игроку не объясняют. У нас модель ошибок описана явными
 * коэффициентами, поэтому её можно честно показать: не «сила 400», а «первые N
 * полуходов играет по книге, лучший ход находит в X% позиций, а когда на доске
 * есть грубая ошибка — допускает её в Y% случаев».
 *
 * Все числа берутся ИЗ САМОГО профиля, ничего не сочиняется. Уровни выше третьего
 * человеческой модели не имеют — там чистый движок, и об этом тоже сказано прямо,
 * иначе описание обещало бы человекоподобность, которой нет.
 */
export type BotPassport = { human: boolean; lines: string[] };

const pct = (x: number) => `${Math.round(x * 100)}%`;

export function describeBot(level: number): BotPassport {
  const p = HUMAN_PROFILES[level];
  if (!p) {
    return {
      human: false,
      lines: [
        "Играет чистый движок: считает на полную глубину и не ошибается нарочно.",
        "Человеческая модель ошибок здесь не применяется.",
      ],
    };
  }
  const spread =
    p.temperature >= 90 ? "очень широкий" : p.temperature >= 75 ? "широкий" : "умеренный";
  return {
    human: true,
    lines: [
      `Дебют: до ${p.bookPlies} полуходов по книге, берёт книжный ход в ${pct(p.bookChance)} случаев.`,
      `Лучший ход находит сразу в ${pct(p.bestChance)} позиций.`,
      `Когда на доске есть грубая ошибка, допускает её в ${pct(p.blunderChance)} случаев.`,
      `В остальное время выбирает среди разумных ходов, разброс ${spread}.`,
    ],
  };
}
