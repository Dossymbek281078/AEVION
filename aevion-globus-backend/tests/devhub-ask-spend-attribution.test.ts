...
describe("расход /plan попадает в учёт и отделим", () => {
  ...
  test("генерация кода ЗАПИСЫВАЕТ расход — проверка следствия, а не наличия кода", async () => {
    ...
    // Метка обязана отличать анонимного от вошедшего: именно этот вопрос
    // основатель решает про суточный потолок расходов. Проект заводился без
    // входа, значит и метка должна быть анонимной. Проверка «содержит generate»
    // прошла бы и на постоянной метке — то есть не различала бы ничего.
    expect(
      генерации.map((r: any) => r.module),
      "расход анонимной генерации не помечен анонимным — отделить его от трат " +
        "платящих будет нельзя, а решение о потолке именно об этом",
    ).toContain("devhub-generate-anon");
  });
});
