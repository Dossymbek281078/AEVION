import { captureException } from "./sentry";

/**
 * Сколько писем ушло за сутки — чтобы исчерпание квоты перестало быть НЕВИДИМЫМ.
 *
 * ⚠️ ПОТОЛОК ОДИН, А ПРОВАЙДЕРОВ ДВА (уточнено 31.08.2026).
 *
 * Счётчик считает ВСЕ исходящие письма платформы: два пути идут через
 * Brevo (подтверждение подписки, DevHub), пять — через Resend (вакансии,
 * отклики, приглашение, подтверждение анкеты, общий модуль QBuild).
 * Значение по умолчанию 300 взято у Brevo, но сумма считается общая.
 *
 * Поэтому в тексте тревоги слово «Brevo» заменено на «почта»: сообщение,
 * называющее одного провайдера, читалось бы как утверждение о его квоте,
 * а на деле в счёт входят оба. Это тот же класс, что поле состояния с
 * именем шире того, о чём оно отвечает.
 *
 * Осознанный размен: один общий мягкий потолок проще и он про НАШУ
 * сдержанность, а не про лимит поставщика. Захотим считать по
 * провайдерам — понадобится второй счётчик и второе значение, и тогда
 * тревога сможет называть провайдера честно.
 *
 * У Brevo на текущем плане потолок 300 писем в сутки (CLAUDE.md окна запуска).
 * Публичная подписка шлёт письмо на каждый запрос, и предела «10 в минуту на адрес»
 * хватало, чтобы выбрать суточную квоту с одного адреса за полчаса. После этого
 * подтверждения не приходят НИКОМУ, а снаружи это выглядит как «письма задерживаются».
 *
 * Здесь намеренно СЧЁТЧИК И ТРЕВОГА, а не запрет: запрет никого не спасает — упёрлись
 * мы сами или посторонний, подписчик всё равно останется без письма. Спасает то, что
 * основатель узнаёт об исчерпании до того, как воронка тихо умрёт.
 *
 * Счётчик в памяти процесса: при перезапуске обнуляется, при нескольких экземплярах
 * недосчитывает. Это осознанно — он ПОРОГОВЫЙ сигнал, а не учёт для биллинга, и
 * недосчёт здесь означает «сообщим позже», а не «пропустим навсегда».
 */
/**
 * Суточный потолок — читается ПРИ ВЫЗОВЕ, а не при загрузке модуля.
 *
 * Было `const DAILY_SOFT_CAP = Number(process.env...) || 300`, то есть значение
 * замерзало в момент первого импорта. Снаружи это выглядит безобидно, но
 * означает, что поведение модуля зависит от ПОРЯДКА ИМПОРТА, а порядок никто
 * не контролирует.
 *
 * Замер 31.08.2026: тесты квоты задают потолок 30 в beforeEach, а первый импорт
 * случался внутри первого теста — то есть уже после хука, и всё работало по
 * счастливой случайности. Стоило прогреть модули в beforeAll (обычная мера
 * против таймаута), как модуль взял боевые 300, и три теста перестали видеть
 * тревогу. Зелёный цвет об этой хрупкости не говорил ничего.
 *
 * Ленивое чтение — то, как устроен остальной код (`GUMROAD_TOKEN`, `isProd` и
 * прочие в routes/* объявлены стрелками ровно поэтому). Здесь было
 * исключением: свип по 62 чтениям окружения дал ровно один такой случай.
 */
const dailySoftCap = (): number => Number(process.env.BREVO_DAILY_SOFT_CAP) || 300;
let sentDay = "";
let sentCount = 0;
const warnedAt = new Set<number>();

/** Отмечает отправку и поднимает тревогу на 2/3 и на 9/10 квоты. Только читает наружу. */
/**
 * Отметить отправленные письма.
 *
 * @param recipients сколько писем ушло этим вызовом. По умолчанию одно.
 *
 * Параметр заведён 31.08.2026. Часть путей шлёт ПАЧКОЙ — один запрос к
 * провайдеру с массивом получателей (`to: batch` в рассылке вакансий). Квоту
 * поставщик считает по получателям, а не по запросам, поэтому отметка «плюс
 * один» за пачку из сорока занижала бы расход в сорок раз — то есть счётчик
 * молчал бы ровно там, где потолок и выбирается.
 *
 * Пороги ниже проверяются через `>=` с дедупликацией по отметке, поэтому
 * прыжок сразу на несколько десятков не перескакивает предупреждения: все
 * пройденные отметки сработают за один вызов.
 */
export function noteEmailSent(recipients = 1): { day: string; count: number } {
  const today = new Date().toISOString().slice(0, 10);
  if (today !== sentDay) {
    sentDay = today;
    sentCount = 0;
    warnedAt.clear();
  }
  // Отрицательное или дробное число сюда попасть не должно, но если попадёт —
  // счётчик обязан не поехать назад: тихо занизившийся расход хуже, чем
  // отсутствие отметки, потому что выглядит как исправная работа.
  sentCount += Math.max(1, Math.floor(recipients) || 1);
  // Порог 1.0 добавлен 28.08.2026. Раньше последний сигнал был на 9/10, и
  // САМО ИСЧЕРПАНИЕ проходило молча: письма просто переставали доходить, а
  // снаружи это выглядит как «почта задерживается» — то есть как ничто.
  // Доставку здесь НЕ меняем: наш потолок мягкий (переопределяется
  // переменной), и жёсткий стоп мог бы оборвать письма раньше настоящего
  // лимита провайдера. Задача этого порога — сделать исчерпание ВИДИМЫМ.
  for (const share of [2 / 3, 9 / 10, 1]) {
    const mark = Math.floor(dailySoftCap() * share);
    if (sentCount >= mark && !warnedAt.has(mark)) {
      warnedAt.add(mark);
      // Текст начинается со слова ПРЕДУПРЕЖДЕНИЕ намеренно.
      //
      // Уходит это через captureException, то есть в Sentry ляжет как ошибка, а
      // утренняя сводка считает ошибками прода письма с темой «проект - ТипОшибки»
      // — значит предупреждение попадёт в счёт аварий. Выбор осознанный:
      // captureMessage(level="warning") пометил бы верно, но правила оповещений
      // срабатывают на ошибках, и письма могло не быть вовсе — тревога стала бы
      // невидимой. Видимость здесь важнее аккуратности ярлыка, а чтобы человек не
      // принял это за аварию, сообщение само говорит, чем оно является.
      const msg =
        (sentCount >= dailySoftCap()
          ? `ИСЧЕРПАНО: [почта] суточный потолок ${dailySoftCap()} писем достигнут — `
            + `подтверждения подписки дальше могут не доходить, и снаружи это выглядит `
            + `как «письма задерживаются». Отправлено ${sentCount}.`
          : "") ||
        `ПРЕДУПРЕЖДЕНИЕ (не авария): [почта] за сутки отправлено ${sentCount} писем ` +
        `из ${dailySoftCap()} — при исчерпании подтверждения подписки перестанут ` +
        `приходить всем, и снаружи это выглядит как «письма задерживаются»`;
      console.warn(msg);
      captureException(new Error(msg), { where: "brevo.dailyQuota", sent: sentCount, cap: dailySoftCap() });
    }
  }
  return { day: sentDay, count: sentCount };
}

/**
 * Сколько писем ушло СЕГОДНЯ и каков потолок.
 *
 * Нужно не только проверкам: скрипт рассылки на запуск живёт в отдельном
 * процессе и счётчик сервера не видит. Без этого числа он считал бы, что
 * сегодня не ушло ничего, и мог бы пробить суточный потолок провайдера —
 * 301-е письмо просто не уходит, и узнать об этом неоткуда.
 *
 * Отдаётся наружу через /api/health/channels, значений и адресов там нет.
 */
export function emailQuotaToday(): { day: string; count: number; cap: number } {
  return { day: sentDay, count: sentCount, cap: DAILY_SOFT_CAP };
}

/** Для проверок: текущее состояние счётчика без побочных действий. */
export function __emailCounter(): { day: string; count: number; cap: number } {
  return { day: sentDay, count: sentCount, cap: dailySoftCap() };
}

/** Для проверок: обнулить счётчик между случаями. */
export function __resetEmailCounter(): void {
  sentDay = "";
  sentCount = 0;
  warnedAt.clear();
}
