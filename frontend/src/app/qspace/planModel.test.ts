...
describe("светильники знают про помещения", () => {
  ...
  it("контроль прибора: старое поведение действительно давало дефект", () => {
    ...
    // 23.09: прежняя планировка перестала показывать дефект — не потому, что
    // светильники поумнели, а потому, что штамп стен на сетке площадей стал
    // точнее и границы комнат сдвинулись на клетку. Дефект прежнего пути от
    // этого никуда не делся, он про УЗКИЕ помещения: сетка по габариту ставит
    // светильники через 2–2.5 м и в санузел шириной 1.2 м не попадает вовсе.
    const санузел: Plan = {
      name: "с санузлом", source: "demo", openings: [],
      walls: [
        w2(0, 0, 12, 0, 0.3), w2(12, 0, 12, 3, 0.3), w2(12, 3, 0, 3, 0.3), w2(0, 3, 0, 0, 0.3),
        w2(9, 0, 9, 3), w2(10.4, 0, 10.4, 3),
      ],
    };
    const rooms = findRooms(санузел);
    expect(rooms.rooms.length, "контроль пуст: помещений меньше трёх").toBeGreaterThanOrEqual(3);
    const безУчёта = generateLights(санузел); // прежний путь: сетка по габариту
    const пустые = rooms.rooms.filter(
      (r) => !безУчёта.some((l) => rooms.roomAt(l.x, l.y) === r.index));
    expect(пустые.length, "на узких помещениях прежняя сетка была исправна — тест ничего не доказывает")
      .toBeGreaterThan(0);
  });
});
