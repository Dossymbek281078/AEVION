...
Верни СТРОГО JSON, без markdown:
{
  "filters": {
    ...
    "minSalary": "number | null (только для mode=vacancy — нижняя граница; см. правило 'от/до' ниже)",
    "maxSalary": "number | null (только для mode=vacancy — верхняя граница; см. правило 'от/до' ниже)"
  },
...
- **"от/до" для зарплаты** (частый источник ошибок в сложных запросах — проверяй его отдельно от остального текста, не давай длине запроса сбить с толку): "от N" БЕЗ "до" → ТОЛЬКО minSalary=N, maxSalary остаётся null. "до N"/"максимум N" БЕЗ "от" → ТОЛЬКО maxSalary=N, minSalary остаётся null. Оба поля заполняются ТОЛЬКО если в запросе явно назван диапазон ("от N до M", "N-M"). "от 1000$ на человека" — это исключительно minSalary=1000, никогда не maxSalary.`;
...
- **Mode-принадлежность minExp/maxExperience**: mode=talent разрешает только "minExp" (обнуляй "maxExperience" если парсер его всё же выставил, перенеся значение в "minExp"); mode=vacancy разрешает только "maxExperience" (обнуляй "minExp" если он есть, перенеся значение в "maxExperience"). Это частая ошибка парсера — специально проверяй её в каждом запросе.
- **"от/до" для зарплаты**: перечитай запрос отдельно на предмет слова "от" (без "до" рядом) → это ТОЛЬКО minSalary, maxSalary должен остаться null; "до"/"максимум" (без "от") → ТОЛЬКО maxSalary. Если парсер поставил одностороннее "от N" в maxSalary (или наоборот) — это ошибка, исправь: перенеси число в правильное поле и обнули второе. Не подтверждай слепо решение парсера здесь — перепроверь сам по исходному тексту, это отдельная частая ошибка, особенно в длинных запросах с несколькими условиями.
