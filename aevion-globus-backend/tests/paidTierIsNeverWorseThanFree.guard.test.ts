...
const KNOWN: Record<string, string> = {
  // ✅ ПУСТ с 01.09.2026 — расхождение по озвучке закрыто, а не списано.
  //
  // Было: free 100000 против pro 30000, то есть заплативший $149 получал втрое
  // меньше того, кто не платил. Ждали решения основателя о составе пакета —
  // и всё это время продолжали продавать ухудшение.
  //
  // Поставлен ПОЛ, а не ответ: pro = ровно столько же, сколько free. Настоящее
  // число остаётся решением основателя (по остальным возможностям платный
  // больше бесплатного в 17–20 раз). Но «платный не хуже бесплатного» — не
  // решение об упаковке, а нижняя граница честности, и ждать её не нужно.
  //
  // Пустой список делает правило безусловным: ЛЮБАЯ новая инверсия краснеет
  // сразу, а не пополняет перечень известных.
};
