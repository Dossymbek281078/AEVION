...
/**
 * Комментарии вырезаем ДО поиска.
 *
 * Первая редакция считала их наравне с кодом, и список получил ТРИ ложные
 * записи из тринадцати. В одной из них комментарий прямо объясняет, почему
 * адрес на мёртвом домене НЕ используется и выбран рабочий: сторож ругался на
 * разбор, который его же и предотвратил.
 *
 * Класс знакомый — свой счётчик, считающий комментарии, — и я всё равно в него
 * въехал. Значит правило стоит держать в коде, а не в памяти.
 */
function безКомментариев(текст: string): string {
  ...
  for (;;) {
    const от = s.indexOf("/" + "*");
    if (от < 0) break;
    const до = s.indexOf("*" + "/", от + 2);
    if (до < 0) break;
    s = s.slice(0, от) + s.slice(до + 2);
  }
  return s;
}
...
describe("новых мёртвых адресов не появляется", () => {
  ...
  const сАдресом = все.filter((p) =>
    СМЕРТЕЛЬНЫЙ_ДОМЕН.test(безКомментариев(readFileSync(join(КОРЕНЬ, p), "utf8"))),
  );
