...
function offenders(mod: string, src: string, libs: Set<string>): string[] {
  ...
  for (const { name, body } of chunks(lines)) {
    ...
    /*
   * ГРАНИЦА ОХВАТА, замерена 28.08.2026 — чтобы зелёный цвет не читался шире,
   * чем он значит.
   *
   * Имя проверки базы ловится ПОДСТРОКОЙ "DbReady()", поэтому все двадцать
   * вариантов (isDbReady, isQSocialDbReady, isQLearnDbReady …) видны. На этой
   * оси сторож не слеп — в отличие от соседнего, который знал одно имя.
   *
   * А вот карта памяти опознаётся по соглашению `mem` + Заглавная. Замер: в
   * маршрутах 168 объявлений `new Map`, под шаблон попадают 52 (31%).
   * Оставшиеся 116 — в подавляющем большинстве кэши, счётчики, индексы и
   * ограничители темпа (`rateLimiter`, `SDK_STATS_CACHE`, `tagCount`, `ring`),
   * которым память положена по устройству; ловить их значило бы кричать на
   * исправный код.
   *
   * Проверен самый похожий на хранилище случай — `coach.ts` (`sessions`,
   * `goals`): у модуля НОЛЬ обращений к базе вообще, то есть память там не
   * подменяет хранилище, а является им. Это другой вопрос и к другому
   * владельцу.
   *
   * Практический вывод: сторож стережёт модули, следующие соглашению `memX`.
   * Заводя хранилище под другим именем, добавьте его сюда — иначе оно окажется
   * вне охвата молча.
   */
