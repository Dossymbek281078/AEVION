...
export default function PricingPage() {
  ...
  useEffect(() => {
    ...
    // Человек пришёл ЗА КОНКРЕТНЫМ приложением — довозим его до карточки.
    //
    // Замер 22.09.2026, телефон 390 px: нажатие «Купить» на /qright приводит на
    // /pricing?app=qright#apps, страница открывается СВЕРХУ (scrollY = 0), а
    // нужная карточка стоит на y≈6900 — около восьми экранов вниз. Якорь #apps
    // не срабатывает: блок появляется после отрисовки, браузер прыгает раньше,
    // чем элемент существует. Первое, что видит пришедший за QRight, — кнопка
    // «Купить CyberChess».
    //
    // Поэтому прокручиваем сами и ПОСЛЕ появления карточки: ждём её в DOM
    // короткой проверкой, а не фиксированной паузой (пауза — ставка на скорость
    // машины). Не нашли за отведённое время — ничего не делаем: молча оставить
    // человека наверху лучше, чем дёрнуть страницу в случайный момент.
    const слаг = params.get("app");
    if (слаг) {
      let попыток = 0;
