...
describe("уже стоящий предмет выбирается без мыши", () => {
  // Сдвиг, поворот и удаление были доступны кнопками и раньше, а ВЫБОР —
  // только кликом в трёхмерном виде. Человек без мыши мог поставить предмет
  // из каталога (тот выбирается сам) и подвинуть его, но не мог вернуться к
  // нему потом. Демо приходит с 14 предметами, и ни один из них не выбирался
  // с клавиатуры вовсе.
  const client = readFileSync(path.join(__dirname, "_client.tsx"), "utf8");

  it("список предметов — это КНОПКИ, а не строка со счётчиком", () => {
    expect(client, "список предметов не отрисовывается").toMatch(/placed\.map\(/);
    const кусок = client.slice(client.indexOf('aria-label="Выбор предмета"'),
                               client.indexOf('aria-label="Выбор предмета"') + 700);
    expect(кусок, "срез не найден").toContain("setSelectedUid");
    expect(кусок, "выбор сделан не кнопкой — с клавиатуры не нажать")
      .toMatch(/<button/);
  });

  it("выбранный предмет объявлен диктору, а не только покрашен", () => {
    // цвет читалка не произносит; без aria-pressed человек не знает, что
    // выбрано, и будет двигать не тот предмет
    expect(client).toMatch(/aria-pressed=\{p\.uid === selectedUid\}/);
  });

  it("выбранное отличается НЕ ТОЛЬКО цветом", () => {
    // при слабом зрении и в чёрно-белой печати один цвет не читается
    const стиль = client.slice(client.indexOf("btnOn: {"), client.indexOf("btnOn: {") + 260);
    expect(стиль, "стиль выбранного не найден").toContain("border");
    expect(стиль, "у выбранного нет отличия, кроме цвета фона")
      .toMatch(/2px|fontWeight/);
  });

  it("подпись больше не занижает: она не утверждает, что двигать нельзя", () => {
    // прежний текст говорил «перетаскивание с клавиатуры недоступно», хотя
    // кнопки сдвига существовали — человек без мыши сдался бы раньше времени
    expect(client).not.toMatch(/перетаскивание и правка[\s\S]{0,40}с клавиатуры пока\s+недоступны/);
    expect(client, "не сказано, что без мыши доступно всё").toMatch(/без мыши доступно вс[её]/i);
  });
});
