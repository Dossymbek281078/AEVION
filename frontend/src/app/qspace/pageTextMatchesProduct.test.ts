...
describe("страница описывает продукт таким, какой он есть", () => {
  ...
  it("границы названы: масштаб спрашиваем, проёмы только из DXF", () => {
    ...
    // Формулировка упрощена тем же днём: в шапке стоял чертёжный жаргон
    // («сколько по имени блока, а сколько по слою»), нужный только тому, кто
    // грузит DXF, — а шапку читают все, включая того, кто грузит фото.
    // Подробность не потеряна: она приходит в предупреждении ПОСЛЕ загрузки,
    // то есть ровно тому человеку и в тот момент, когда она ему нужна.
    expect(/ширина берётся из чертежа/i.test(client),
      "страница молчит, что ширина берётся ИЗ ЧЕРТЕЖА").toBe(true);
    expect(/где её в файле нет[^<]{0,40}типова/i.test(client),
      "страница не признаёт, что без чертежа ширина ТИПОВАЯ").toBe(true);
    // и отдельно: про масштаб страница говорит о НАС, а не о ФОРМАТЕ.
    // Прежняя редакция утверждала «у PDF нет масштаба внутри файла» — это
    // утверждение о формате, которого мы не проверяли (у PDF есть
    // необязательный словарь /Measure с настоящим масштабом).
    expect(/Масштаб из PDF и[\s\S]{0,30}мы не читаем/i.test(client),
      "страница утверждает о ФОРМАТЕ то, чего мы не проверяли").toBe(true);
  });
