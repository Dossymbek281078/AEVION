...
describe("обещание про Kaspi следует за фактом", () => {
  ...
  // Правило про валюту живёт в отдельном модуле: страницу цен нельзя
  // смонтировать в тестах без полного снимка данных, и решение вынесли туда,
  // где его МОЖНО проверить. Читаем оттуда, а не из файла страницы.
  // Путь от ФАЙЛА ТЕСТА, а не от рабочего каталога: process.cwd() зависит от
  // того, откуда запущен прогон, и тот же тест падал под мутатором (он зовёт
  // vitest из корня репозитория с --root frontend). Соседние пути в этом файле
  // давно так и заданы — я просто не посмотрел на них.
  const правило = readFileSync(join(__dirname, "..", "..", "..", "lib", "chargeCurrencyNote.ts"), "utf8");
  ...
  test("незнание о продаваемости НЕ выключает кнопки покупки", () => {
    ...
    //
    // 14.09.2026: состояние переименовано notSellable -> sellableRefs, потому
    // что список стал ПОЛОЖИТЕЛЬНЫМ (что точно продаётся), а не отрицательным.
    // Смысл этой проверки не изменился: null по-прежнему значит «не знаем» и
    // кнопки НЕ гасит. Поведенчески то же направление держит тест
    // unsellableTierExplainsItself («нет поля продаваемости — кнопки живы»).
    expect(page).toContain("sellableRefs === null ? true");
    // и что решение вообще применяется к кнопкам. С 15.09.2026 кнопка баннера
    // диплинка покупает отдельное приложение на сроке, и её условие смотрит на
    // ссылку этой пары, а не на зашитый "lite".
    expect(page).toMatch(/disabled=\{checkingOut === ключ \|\| !продаётсяСсылка\(ссылка\)\}/);
  });
  ...
  test("«не смогли спросить» не выдаётся за «Kaspi не подключён»", () => {
    ...
    /*
     * ⚠️ 01.09.2026: правило переехало из страницы в lib/chargeCurrencyNote.ts —
     * страницу цен нельзя смонтировать в тестах без полного снимка данных, и
     * автор вынес решение туда, где его можно проверить. Сторож остался
     * смотреть в файл страницы и краснел на ВЕРНОЙ работе.
     *
     * Смотрим туда, где логика живёт СЕЙЧАС. Свойство то же и оно важнее места:
     * незнание («не спросили») не должно схлопываться в «Kaspi не подключён».
     */
    expect(правило).toContain("payboxLive === null");
    ...
    // Ветка истинности в новом модуле — возврат ключа kztNote. Якорь берём с
    // вопросительным знаком: ключ незнания называется kztUnknownNote и СОДЕРЖИТ
    // подстроку kztNote, поэтому наивный поиск нашёл бы незнание и объявил
    // порядок верным всегда. Тот же класс подстроки внутри длинного имени.
    const истинность = правило.indexOf("? \"pricing.home.heroModule.kztNote\"");
  ...
  test("обещание про Kaspi показывается только при подтверждении", () => {
    ...
    const правило = readFileSync(
      join(__dirname, "..", "..", "..", "lib", "chargeCurrencyNote.ts"),
      "utf8",
    );
    expect(
