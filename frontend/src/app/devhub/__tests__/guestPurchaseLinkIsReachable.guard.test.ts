...
// ГРАНИЦА ЭТОГО СТОРОЖА (уточнено 01.09.2026).
//
// Он читает ИСХОДНИК: проверяет, что экран существует и в нём есть нужные
// строки. Этого мало — экран может отрисовываться и при отказе сервера
// показывать «письмо отправлено». Строки при этом на месте, сторож зелёный,
// а человек, оплативший доступ, уходит ждать письма, которого не будет.
//
// ПОВЕДЕНИЕ проверяется отдельно: devhub/link/__tests__/linkScreenTellsTheTruth
// отрисовывает экран и нажимает кнопку — нейтральный ответ показывается как
// есть, отказ 503 НЕ выдаётся за отправленное письмо, негодная ссылка названа
// негодной, а годная подключает покупку (положительный контроль: без него
// первые три прошли бы и на экране, отказывающем всегда).
//
// Этот файл оставлен намеренно: он стережёт САМО НАЛИЧИЕ адреса и то, что на
// него ведёт ссылка из модуля, — чего отрисовка одной страницы не видит.
// Не дублируйте сюда поведение: два сторожа, утверждающих одно, расходятся
// молча, и тогда верят более короткому.
...
describe("подключение покупки доступно человеку", () => {
  ...
  it("экран зовёт обе ручки", () => {
    // С ЗАКРЫВАЮЩЕЙ КАВЫЧКОЙ, а не по куску имени. Мутационная проверка
    // 08.09.2026: подмена адреса на «guest/link-requestX» проходила МОЛЧА —
    // toContain совпадает внутри более длинного имени, и сломанная ручка
    // денежного пути выглядела бы целой. Тот же класс уже ловили на именах
    // юрлица и на роли диалога.
    expect(L, "нет запроса ссылки").toContain('guest/link-request"');
    expect(L, "нет подтверждения").toContain('guest/link-confirm"');
  });

  it("кнопки и переходы СВЯЗАНЫ с действиями", () => {
    // Добавлено 31.08.2026 после того, как я проверил мутацией СВОЙ ЖЕ экран.
    // Отвязал форму от запроса и подтверждение от перехода по ссылке — обе
    // мутации прошли незамеченными: сторож проверял достижимость экрана и
    // пути отказа, но не то, что нажатие вообще что-то запускает.
    //
    // Это третий класс за вечер: сторожа пишут на то, что происходит ПОСЛЕ
    // действия, и не проверяют связь самого действия с обработчиком.
    expect(
      L,
      "форма отвязана от запроса: человек введёт почту, нажмёт и ничего не произойдёт",
    ).toContain('onSubmit={request}');
    expect(
      L,
      "переход по ссылке из письма больше не вызывает подтверждение: покупка не подключится",
    ).toContain("void confirm(id, token)");
  });
