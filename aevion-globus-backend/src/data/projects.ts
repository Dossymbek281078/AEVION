...
export const projects: GlobusProject[] = [
  ...
  },
  {
    id: "qspace",
    code: "QSPACE",
    name: "QSpace — 3D Room & Interior Modeler",
    description:
      "3D-модельер помещений из 2D-плана прямо в браузере. Вход — три формата: DXF из AutoCAD (LINE/LWPOLYLINE, единицы из $INSUNITS), ВЕКТОРНЫЙ PDF из AutoCAD/Revit (разбор операторов m/l/re и распаковка FlateDecode средствами платформы, без внешних библиотек) и РАСТРОВЫЙ план — JPEG/PNG/скан, где стены находятся обходом пикселей, а результат обязательно проходит экран ручной правки: линии рисуются поверх исходной картинки, лишнее снимается кликом. Масштаб у PDF и картинки не угадывается, его называет человек — иначе вышла бы правдоподобно неверная модель. Дальше модель показывается тремя переключаемыми слоями. Черновая отделка: разводка электрики по типовым нормам (розетки 0.3 м, выключатели 0.9 м у дверей, магистраль под потолком, щиток у входа), трубы ХВС/ГВС и канализация с уклоном 2 см/м к стояку, плюс состав конструкций — пирог пола 80 мм, перегородка на выбор (газоблок 123 мм или каркас ГКЛ 103 мм с кабелями внутри), потолок 61 мм, и главное число: при высоте по умолчанию 2.70 м в бетоне остаётся 2.56 м после ремонта, а если человек назвал свою высоту — пересчитывается от неё. Чистовая: каталог 23 материалов (паркет, ламинат, плитка, бетон, шесть красок, обои, стеновая плитка), окна и двери в проёмах, светильники по помещениям — сетка с шагом 3 м, просеянная через границы комнат: ни одна комната не остаётся без света и ни один светильник не висит в стене. Декор: 40 предметов мебели и оборудования, включая кухонный гарнитур, сантехнику, сплит-систему и радиатор, с перетаскиванием мышью; пальцем предмет тянется поперёк экрана, потому что вертикальный свайп отдан прокрутке страницы, а кнопками сдвига он двигается с клавиатуры. Спецификация материалов считается ИЗ модели и меняется вместе с планом: площади, литры краски, метры кабеля и труб, счёт розеток и светильников. Честно: разводка и толщины — черновик по практике ремонта для разговора с прорабом, а не проектная документация под подпись; распознавание растра даёт предположение, которое человек обязан проверить; проёмы из DXF распознаются по блокам чертежа: вид берётся из имени блока, а если имя молчит — из слоя, и модуль говорит, сколько взято тем и другим способом; ширина проёма ИЗМЕРЯЕТСЯ по определению блока в секции BLOCKS с учётом масштаба вставки — так берётся настоящий размер и у блока в натуральную величину, и у символа шириной в одну единицу; если определения в файле нет или измеренная ширина выходит неправдоподобной, ставится типовая и модуль говорит об этом вслух, а не подставляет число молча. Стена, начерченная ДВУМЯ ЛИНИЯМИ (внутренняя и внешняя грань вместо оси — стандартная манера архитектурного чертежа), сводится в одну, и ТОЛЩИНА берётся из чертежа — расстояние между гранями и есть толщина, а не типовые 0.15 м; без сведения комната из четырёх стен приезжала восемью, а краска считалась дважды. ДВЕРЬ, нарисованная РАЗРЫВОМ стены (проём часто не рисуют вовсе), узнаётся и не сливает соседние помещения в одно — иначе разбивка по комнатам, по которой покупают плитку и обои, оказывалась неверной при верной сумме. Единицы чертежа берутся таблицей $INSUNITS — дюймы, ФУТЫ, мм, см, метры, километры, ярды; незнакомый код единицы называется вслух, а не проглатывается. Читается и POLYLINE СТАРОГО ВИДА, где координаты лежат в дочерних VERTEX: его пишут старые файлы и многие экспортёры, и без него смешанный чертёж молча терял стены, отчитываясь, что взял все. Координата с ЗАПЯТОЙ вместо точки не толкуется по догадке, а называется: «8,5» читается как 8, и модуль говорит, сколько таких координат и что потеряно. У PDF отдельно проверяется РАЗРЫВ в геометрии: если в файле не только план, но и штамп или экспликация, габарит охватывает всё сразу, и названная вами длина большей стороны растянулась бы на лишнее — модуль предупреждает и советует сохранить только лист плана. Скан и фото должны быть ВЫРОВНЕНЫ: при наклоне больше двух-трёх градусов стены разваливаются на куски, это замерено и сказано на странице прямо. ВЫСОТА ПОТОЛКА спрашивается у человека: из плоского чертежа её не узнать никаким разбором, а от неё прямо считается площадь под покраску — разница между 2.5 и 3.2 м это пятая часть материала. Из PDF и картинки проёмы по-прежнему ставит человек нажатием на стену. Расстановка проверяется сама: предметы друг в друге, мебель перед дверью, выход за план, высота выше потолка, и отдельно — не перегородила ли мебель проход, из-за чего до части комнаты не дойти от двери. Помещения выделяются автоматически, по каждому считается площадь и строка для закупки. По тем же площадям считается тёплый пол: метры трубы при выбранном шаге, число контуров (длиннее 100 м насос не продавит) и мощность. Отдельно считается вентиляция и влажность: расход воздуха по назначению помещения (жилая по площади, кухня и санузел фиксированной вытяжкой), и модуль предупреждает, если приток с вытяжкой не сходятся или санузел без окна остался без вентилятора. По тем же помещениям подбирается сплит-система: требуемая мощность из объёма и солнечности комнаты плюс тепло от людей и техники, с округлением ВВЕРХ до типоразмера и названием, которое спрашивают в магазине («девятка», «двенашка») — слабый кондиционер не экономит, он работает без остановки и всё равно не охлаждает. Проём, поставленный рукой, проверяется так же: влезает ли он в стену и не пересекается ли с соседним. Работа не пропадает: проект сохраняется в браузере сам и выгружается файлом .qspace.json для надёжного хранения и передачи подрядчику. Спецификация выгружается ТАБЛИЦЕЙ (CSV) — весь список закупки, а не только разбивка по комнатам: пол, покрытие, стены, краска, розетки, выключатели, кабель, трубы, канализация, светильники. Колонки «цена за единицу» и «сумма» ПУСТЫЕ намеренно: цены зависят от города, поставщика и дня, мы их не знаем и не придумываем — человек вписывает свои, а сумма считается формулой, которую он видит. Файл открывается в Excel без плясок: метка кодировки на месте, десятичная запятая в паре с точкой с запятой, формулы без имён функций, чтобы работали и в русской версии. Готовый план выгружается чертежом сверху в SVG — с размерами стен, площадями помещений, дугами дверей и обозначенными окнами: этот лист печатают и берут на стройку. Готовую модель можно скачать в GLB — она открывается в Blender, SketchUp и штатном просмотрщике Windows, и в файл уезжают только видимые слои. Бэкенд модулю не нужен, всё считается в браузере. MVP 2026-09-07 — страница /qspace.",
    kind: "product",
    status: "mvp",
    priority: 3,
    tags: ["interior-design", "3d", "dxf", "renovation", "furniture"],
    createdAt: now,
    updatedAt: now,
  },
  {
