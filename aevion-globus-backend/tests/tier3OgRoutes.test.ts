...
// Явный таймаут: тест не делает НИ ОДНОГО запроса — он только импортирует
// роутеры и смотрит, какие пути зарегистрированы. Всё время уходит на импорт,
// который тянет за собой дерево зависимостей маршрутов. Замер 28.07 в полном
// параллельном прогоне: 10169–10695 мс при общем лимите 10 000 — то есть тест
// краснел по таймауту, а выглядело это как «пропал публичный маршрут». В
// одиночку укладывается в доли секунды. Тот же приём и по той же причине уже
// стоит в qcoreaiQuotaVisibility.test.ts.
