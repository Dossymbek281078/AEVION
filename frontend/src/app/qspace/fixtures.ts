...
export interface Блок { x0: number; y0: number; x1: number; y1: number; n: number; lines?: Отрезок[] }
...
export function блокиИзЛиний(segs: Отрезок[], eps = 0.04): Блок[] {
  ...
  segs.forEach((s, i) => {
    ...
    if (!b) группы.set(r, { x0, y0, x1, y1, n: 1, lines: [s] });
    else { b.x0 = Math.min(b.x0, x0); b.x1 = Math.max(b.x1, x1); b.y0 = Math.min(b.y0, y0); b.y1 = Math.max(b.y1, y1); b.n++; b.lines!.push(s); }
  });
...
/** Подпись мебели внутри блока: «учебный стол» → стол, «полки» → стеллаж. Стол в кухне/гостиной — обеденный. */
const МЕБЕЛЬ_ПО_ПОДПИСИ: Array<[RegExp, string | ((room: RoomType | null) => string)]> = [
  [/(стол|desk|table)/i, (room) => (room === "kitchen" || room === "living" ? "dining" : "desk")],
  [/(полк|стеллаж|shelf)/i, "shelf"],
  [/(шкаф|гардероб|wardrobe)/i, "wardrobe"],
  [/(диван|sofa)/i, "sofa"],
  [/(кроват|bed)/i, "bed"],
  [/(тумб|nightstand)/i, "nightstand"],
  [/(кресл|armchair)/i, "armchair"],
  [/(телевиз|(^|\s)тв($|\s)|tv)/i, "tv"],
];

export function furnitureFromLabel(text: string, room: RoomType | null): string | null {
  for (const [re, id] of МЕБЕЛЬ_ПО_ПОДПИСИ) if (re.test(text)) return typeof id === "function" ? id(room) : id;
  return null;
}

export interface ПодписьМ { text: string; x: number; y: number }
 ...
 * блока совпала с длинной стороной предмета каталога. Подпись внутри блока
 * («учебный стол») сильнее габарита: по габариту 1.46×0.56 — шкаф, по подписи —
 * стол. Слипшаяся группа (душ + тумба, 2×2.7 м) режется повторной сборкой с
 * допуском вдвое меньше. Неузнанные блоки (стулья, ковры) отдаются в unknown
 * и не ставятся: лишний предмет в модели хуже пропущенного.
 */
export function fixturesFromSegments(
  ...
  const годен = (b: Блок) => b.n >= 6 && (b.x1 - b.x0) >= 0.25 && (b.y1 - b.y0) >= 0.25 && (b.x1 - b.x0) <= 4.5 && (b.y1 - b.y0) <= 4.5;
  const блоки = блокиИзЛиний(свои).filter(годен);
  ...
  const поПлощади = (a: Блок, b: Блок) => (b.x1 - b.x0) * (b.y1 - b.y0) - (a.x1 - a.x0) * (a.y1 - a.y0);
  блоки.sort(поПлощади);
  ...
  const подписьВнутри = (b: Блок, room: RoomType | null): string | null => {
    for (const l of labels) {
      if (l.x < b.x0 - 0.2 || l.x > b.x1 + 0.2 || l.y < b.y0 - 0.2 || l.y > b.y1 + 0.2) continue;
      const id = furnitureFromLabel(l.text, room);
      if (id) return id;
    }
    return null;
  };
  /** true — поставлен; false — пропущен как дубль */
  const положить = (b: Блок, id: string, room: number): boolean => {
    ...
    if (id === "fridge" && занято.some((z) => z.id === "fridge" && z.room === room)) { unknown.push(b); return false; }
    if (занято.some((z) => cx > z.x0 && cx < z.x1 && cy > z.y0 && cy < z.y1)) return false; // центр внутри уже поставленного
    // тот же предмет, перекрывающий поставленный больше чем наполовину, — его же дубль
    if (занято.some((z) => z.id === id && перекрытие(z, b) > 0.5 * bw * bd)) return false;
  ...
  for (const b of блоки) {
    ...
    const тип = types[room] ?? null;
    const id = подписьВнутри(b, тип) ?? классифицироватьБлок(b.x1 - b.x0, b.y1 - b.y0, тип);
    if (id) { положить(b, id, room); continue; }
    // слипшаяся группа: обе стороны от 1.2 м и линии есть — собрать заново с допуском вдвое меньше
    if ((b.x1 - b.x0) >= 1.2 && (b.y1 - b.y0) >= 1.2 && b.lines) {
      const дети = блокиИзЛиний(b.lines, 0.02).filter(годен).sort(поПлощади);
      let узнано = 0;
      for (const d of дети) {
        const r2 = roomAt((d.x0 + d.x1) / 2, (d.y0 + d.y1) / 2);
        if (r2 === null) continue;
        const id2 = подписьВнутри(d, types[r2] ?? null) ?? классифицироватьБлок(d.x1 - d.x0, d.y1 - d.y0, types[r2] ?? null);
        if (id2 && положить(d, id2, r2)) узнано++;
      }
      if (узнано > 0) continue;
    }
    unknown.push(b);
  }
