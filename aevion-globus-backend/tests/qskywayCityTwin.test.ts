...
describe("the committed twins are internally consistent", () => {
  it.each([["astana", CITY], ["nyc", CITY_NYC], ["tokyo", CITY_TOKYO], ["singapore", CITY_SINGAPORE], ["amsterdam", CITY_AMSTERDAM], ["berlin", CITY_BERLIN], ["vienna", CITY_VIENNA], ["zurich", CITY_ZURICH]])(
...
describe("the committed twins publish what the generator could not vouch for", () => {
  ...
  it("пустой список никогда не отгружается — поля либо нет, либо в нём есть записи", () => {
    ...
    for (const [name, city] of [["astana", CITY], ["nyc", CITY_NYC], ["tokyo", CITY_TOKYO], ["singapore", CITY_SINGAPORE], ["amsterdam", CITY_AMSTERDAM], ["berlin", CITY_BERLIN], ["vienna", CITY_VIENNA], ["zurich", CITY_ZURICH]] as const) {
