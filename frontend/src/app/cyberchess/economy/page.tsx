    ...
    <main style={{ background: C.bg, minHeight: "100vh", color: C.text, fontFamily: "system-ui, sans-serif", padding: "32px 16px" }}>
      ...
      <article style={{ maxWidth: 980, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap", marginBottom: 32 }}>
          ...
          {/* Страница обязана сказать о себе. 19.08.2026: проверено — здесь НЕТ
              ни одного обращения к серверу, все три действия (ставка, аренда,
              подписка) пишут только в localStorage браузера. То есть аукцион
              существует в одном экземпляре у каждого человека, и «ставка»
              никому не видна.

              Отдельно опасен баланс: Chessy — настоящая валюта с серверным
              кошельком CyberWallet, а здесь читается местная копия. Списание
              на этой странице настоящего баланса не меняет, и человек вправе
              решить, что потратил заработанное.

              Страницу не убираю — это продуктовое решение. Но молчать о том,
              что это витрина замысла, нельзя: тот же класс, что выдуманные
              гроссмейстеры в таблице лидеров. */}
          <div
            data-testid="economy-prototype-notice"
            style={{
              marginTop: 14,
              padding: "10px 14px",
          ...
          >
            <b>Это витрина замысла, а не работающая биржа.</b> Ставки, аренда и
            подписки сохраняются только в этом браузере: другие игроки их не
            видят, а баланс здесь — местная копия, настоящие Chessy он не
            списывает.
          </div>
          </div>
