...
export default function TeamRequestDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function apply(e: React.FormEvent) {
    ...
    try {
      ...
      setError(t("build.teamRequestDetail.applyError"));
  ...
  if (!req) return (
    <BuildShell>
      <p className="text-slate-400">{t("build.teamRequestDetail.notFound")}</p>
    </BuildShell>
  );
  ...
  return (
    <BuildShell>
      <div className="space-y-6">
        <Link href="/build/team-requests" className="text-xs text-slate-400 hover:text-white">{t("build.teamRequestDetail.backLink")}</Link>
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 p-6 space-y-4">
          ...
          <div>
            <p className="text-xs font-semibold uppercase text-slate-400 mb-2">{t("build.teamRequestDetail.rolesNeeded")}</p>
                ...
                return (
                  <div key={i} className="flex items-center justify-between rounded-lg border border-white/10 bg-white/5 px-4 py-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{t("build.teamRequestDetail.applicationsCount", { count: appCount })}</p>
                    </div>
                    {token && !alreadyApplied && (
                      ...
                      >
                        {t("build.teamRequestDetail.applyButton")}
                      </button>
                    )}
                    {alreadyApplied && (
                      <span className="text-xs text-emerald-400">{t("build.teamRequestDetail.appliedBadge")}</span>
                    )}
                  </div>
                );
              })}
        ...
        {applyRole !== null && (
          <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-5">
            <h3 className="font-semibold text-white mb-3">
              {t("build.teamRequestDetail.applyToRole", { specialty: req.roles[applyRole]?.specialty })}
            </h3>
            {applied ? (
              <p className="text-emerald-400 text-sm">{t("build.teamRequestDetail.applySuccess")}</p>
            ) : (
              <form onSubmit={(e) => void apply(e)} className="space-y-3">
                <textarea
                  value={message}
                  onChange={(e) => setMessage(e.target.value)}
                  placeholder={t("build.teamRequestDetail.messagePlaceholder")}
                  rows={3}
                  maxLength={2000}
                ...
                <div className="flex gap-3">
                  <button type="submit" disabled={busy} className="rounded-lg bg-emerald-500 px-5 py-2 text-sm font-bold text-emerald-950 hover:bg-emerald-400 disabled:opacity-50">
                    {busy ? "…" : t("build.teamRequestDetail.submitApplication")}
                  </button>
                  <button type="button" onClick={() => setApplyRole(null)} className="rounded-lg border border-white/10 px-4 py-2 text-sm text-slate-300">{t("build.teamRequestDetail.cancelButton")}</button>
                </div>
              </form>
            )}
          </div>
        ...
        {req.applications.length > 0 && user && (
          <div className="space-y-3">
            <p className="text-sm font-semibold text-slate-300">{t("build.teamRequestDetail.allApplications", { count: req.applications.length })}</p>
            {req.applications.map((app) => (
              <div key={app.id} className="rounded-xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-white">
                      {app.applicantName ?? t("build.teamRequestDetail.defaultApplicantName")} → {req.roles[app.roleIndex]?.specialty}
                    </p>
                  ...
                  >
                    {t("build.teamRequestDetail.profileLink")}
                  </Link>
                </div>
              </div>
            ))}
