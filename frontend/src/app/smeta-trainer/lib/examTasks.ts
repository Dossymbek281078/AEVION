...
/* =========================================================================
 *  ЗАДАНИЕ 22 — Завышенный коэффициент условий (coef-over-limit, batch F)
 * ========================================================================= */
const T22_OBJECT: LearningObject = {
  id: "exam-coef-over-limit",
  title: "СОШ №47, штукатурка лестничной клетки — значение коэффициента",
  type: "капремонт",
  region: REGION,
  description: "Та же лестничная клетка, штукатурка стен 95 м². Коэффициент стеснённости обоснован, но значение завышено — 1.4 вместо типового 1.15.",
  geometry: { kind: "room", length: 6.0, width: 2.5, height: 6.3, openings: [] },
  attachments: [],
};
const T22_REFERENCE = mkLsr({
  id: "exam-t22-ref", title: "Эталон — штукатурка", objectId: T22_OBJECT.id, lsrNumber: "exam-t22",
  sections: [
    {
      id: "ref-s1", title: "Раздел 1. Штукатурка", category: "отделочные",
      positions: [
...
const T22_STARTER = mkLsr({
  id: "exam-t22-student", title: "Моё решение", objectId: T22_OBJECT.id, lsrNumber: "exam-t22",
  sections: [
    {
      id: "stu-s1", title: "Раздел 1. Штукатурка", category: "отделочные",
      positions: [
        {
          id: "sp1", rateCode: "ЭСНСб15-13-01-001", volume: 0.95,
          // BUG: значение коэффициента стеснённости 1.4 завышено — типовой предел 1.15.
          // Обоснование есть, но экспертиза срежет превышение, смета завышена.
          coefficients: [{ kind: "стеснённые", value: 1.4, justification: "ППР, стеснённые условия лестничной клетки" }],
          formula: "95 / 100 = 0.95",
        },
      ],
    },
...
export const EXAM_TASKS: ExamTask[] = [
  ...
  {
    id: "stairwell-coef-over-limit",
    title: "Штукатурка лестничной клетки — значение коэффициента",
    category: "Отделка",
    difficulty: "средняя",
    durationMin: 12,
    icon: "📐",
    hiddenError: "Коэффициент стеснённости задан 1.4 вместо типового 1.15 — превышение норматива",
    object: T22_OBJECT,
    reference: T22_REFERENCE,
    starter: T22_STARTER,
    hints: [
      "Коэффициент условий начисляется в пределах норматива: для стеснённости типовое значение К≈1.15",
      "Завышенное значение экспертиза срежет до норматива — исправьте 1.4 на 1.15 (СН РК 8.02-05)",
    ],
  },
];
