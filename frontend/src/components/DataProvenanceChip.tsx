...
export function DataProvenanceChip({ dataQuality, labels, compact }: Props) {
   ...
   */
  /*
   * Серверная заметка ВЫТЕСНЯЛА переведённый текст.
   *
   * Замер 02.09.2026 по живому ответу /api/qskyway/cities: поле note приходит
   * ТОЛЬКО по-русски («обмерено властями…»), английского варианта в данных нет.
   * Так как оно подставлялось первым, вся работа со словарём обходилась: на
   * английской странице читатель получал русскую строку.
   *
   * Приём подсказало соседнее окно: сравнить живой ответ прода с фикстурой
   * сторожа. Мой сторож подавал note пустым и был слеп к этому по построению —
   * зелёный и бесполезный.
   *
   * Правило: текст на языке, которого читатель не выбирал, хуже общего
