...
interface KidsAiConceptMessage {
  id: string;
...
const KIDSAI_CONCEPT_MAX = 200;
const kidsAiConceptMessages: KidsAiConceptMessage[] = [];
...
kidsAiContentRouter.get("/status", globalLimit, (_req: Request, res: Response) => {
  res.json({
    ...
    conceptMessagesCount: kidsAiConceptMessages.length,
    timestamp: new Date().toISOString(),
  });
});

kidsAiContentRouter.get("/concept/messages", globalLimit, (req: Request, res: Response) => {
  const limit = Math.min(Math.max(parseInt(String(req.query.limit ?? "20"), 10) || 20, 1), 100);
  const items = kidsAiConceptMessages.slice(0, limit);
  res.json({ items, total: kidsAiConceptMessages.length, moduleId: "kids-ai-content", noun: "concept/messages" });
});

kidsAiContentRouter.post("/concept/messages", globalLimit, (req: Request, res: Response) => {
  try {
    ...
    const tagsRaw = Array.isArray(payload.tags) ? payload.tags : ["kids-ai"];
    const tags = tagsRaw.map((t) => String(t).trim().slice(0, 30)).filter(Boolean).slice(0, 6);
    const msg: KidsAiConceptMessage = {
      id: crypto.randomUUID(),
      payload: { idea, rationale, author },
      tags: tags.length ? tags : ["kids-ai"],
      createdAt: new Date().toISOString(),
    };
    kidsAiConceptMessages.unshift(msg);
    if (kidsAiConceptMessages.length > KIDSAI_CONCEPT_MAX) {
      kidsAiConceptMessages.length = KIDSAI_CONCEPT_MAX;
    }
    return res.status(201).json(msg);
  } catch (err: unknown) {
    console.error("[kids-ai] concept_post_failed", err instanceof Error ? err.message : err);
    return res.status(500).json({ error: "concept_post_failed" });
  }
});

kidsAiContentRouter.get("/concept-stats", globalLimit, (_req: Request, res: Response) => {
  const now = Date.now();
  const sevenDays = 7 * 86_400_000;
  const last7d = kidsAiConceptMessages.filter(
    (m) => now - new Date(m.createdAt).getTime() <= sevenDays,
  ).length;
  const tagCounts = new Map<string, number>();
  for (const m of kidsAiConceptMessages) {
  ...
  res.json({
    ...
    total: kidsAiConceptMessages.length,
    last7d,
    topTags,
  });
});
