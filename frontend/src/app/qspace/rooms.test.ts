...
describe("выделение комнат", () => {
  ...
  it("демо-квартира даёт РОВНО три комнаты и ни одного разрыва", () => {
    // Демо — это первое, что видит каждый посетитель, и план у него
    // постоянный. Значит числа известны точно, и порог «не меньше трёх» тут
    // лишняя свобода: он одинаково терпит и 3, и 5, то есть молчаливую
    // перемену первого впечатления.
    const r = findRooms(demoPlan());
    expect(r.rooms.length, "разбивка демо-квартиры изменилась").toBe(3);

    // И контроль на мою же правку 09.09: дверей-РАЗРЫВОВ в демо нет (стены
    // сплошные), поэтому сведение не должно срабатывать вовсе. Если оно
    // сработает — правило ловит лишнее, и увидит это каждый посетитель.
    expect(r.warnings.join(" "), "сведение разрывов сработало на сплошных стенах")
