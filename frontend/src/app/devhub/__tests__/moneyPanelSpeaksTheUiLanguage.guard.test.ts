 ...
 *
 * ❌ И ОТДЕЛЬНО — ОТЗЫВ ТРЕВОГИ, которую я поднял в коммите про эту починку.
 * Я написал, что сам переводчик на проде отдаёт вместо перевода посторонние
 * фразы. Это НЕВЕРНО: ручка исправна, кириллицу калечил мой же `curl -d` со
 * строкой в Git Bash. Проверено правильным и сломанным замером рядом —
 * «Уже оплатили?» → «Already paid?» при теле файлом и `--data-binary`, и
 * → «Why not subscribe?» при `-d`. Тревогу подтвердили четыре окна, и все
 * четверо мерили одним и тем же сломанным способом.
 *
 * На нужность ЭТОЙ починки отзыв не влияет: кнопка покупки не должна зависеть
 * от сетевого вызова в момент оплаты, даже когда вызов исправен.
 *
