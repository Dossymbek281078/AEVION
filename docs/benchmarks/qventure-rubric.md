...
4. **v5 changed nothing on the calibration corpus** — identical to the decimal.
   ...
   All six are fixed and pinned (`tests/qventureDisclosedCorpus.test.ts`, 285
...
20. **A milestone's words also describe a business.** Two more entries taken to
   ...
   That is seven list entries now tested against real documents — defence, PPA,
...
`tests/qventureDisclosedCorpus.test.ts`, 285):
