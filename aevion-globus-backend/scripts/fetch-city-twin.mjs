...
const CITIES = {
  astana: {
    ...
    measured: {
      ...
      marginM: 100,
      nearRadiusM: 0,
      // h_1 — высота всей 3D-модели (не крыши без мачт, как у NYC/PLATEAU):
      // тег OSM выше неё — это неверный тег, а не антенна. См. разбор ниже
      // у правила «OSM выше обмера».
      osmTallerIsTag: true,
    },
    header: [
  ...
  amsterdam: {
    ...
    vertiports: [
      ...
      // Центральная площадка стояла в (610, 510) — в 163 м от центра демо-круга
      // над станцией Зюйд (734, 405; r 220): смоук на проде 16.09 дал 30/42
      // маршрутов — все 12 непроходимых через неё. Сдвинута на (970, 610):
      // 313 м от станции и 370 м от круга у Малерплейн.
...
if (city.measured) {
  ...
  buildings.forEach((b, i) => {
    ...
    if (p.how === "contained") {
      ...
      //
      // Но это зависит от того, ЧТО меряет источник. NYC height_roof и PLATEAU
      // measuredHeight — высота КРЫШИ без мачт и антенн, и тег OSM выше них
      // законно (Эмпайр-Стейт 443 против 427 у съёмки): замер 15.09 — 1243
      // таких домов в Нью-Йорке, и переводить их в «тег» значило бы уронить
      // обмер с 96.8 % до 54.8 % и поднять просвет над всем Мидтауном без
      // причины. А 3D-модель Астаны — высота ВСЕЙ модели, выше неё только
      // неверный тег. Поэтому флаг на источнике: osmTallerIsTag — высота
      // остаётся большей (безопасная сторона), но класс — тега (hs=1), и
