      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        {!loading && list.length === 0 && (
          <div className="text-center py-16 text-slate-500">
            ...
            <p className="text-sm mb-2">{tab === "mine" ? "У вас нет плейлистов" : "Публичных плейлистов нет"}</p>
            {tab === "mine" && (
              <button onClick={() => setCreating(true)} className="mt-3 px-4 py-2 bg-violet-600 hover:bg-violet-700 rounded-lg text-xs font-semibold text-white">+ Создать первый плейлист</button>
            )}
          </div>
        )}
