...
export default function PricingAdminPage() {
  ...
  /*
   * Фактически списанные деньги по каналам — вторая колонка рядом с ожидаемой.
   *
   * Ожидаемая берётся из адреса возврата, то есть НАША сумма из каталога.
   * Фактическая — из записи подписки, куда её кладут вебхуки касс. Расхождение
   * между ними и есть сигнал: цену у Gumroad и LemonSqueezy назначает продавец,
   * и до 01.09.2026 никто не сверял, совпадает ли она с обещанной на странице.
   *
   * `null` означает «не спросили» и рисуется как НЕИЗВЕСТНО, а не как ноль:
   * ручка живёт в соседней ветке и до мержа отвечает отказом. Ноль здесь
   * прочли бы как «продаж нет», а это худшая из возможных неправд на этой
   * панели.
   */
  const [фактическая, setФактическая] = useState<{
    byChannel: Record<string, { count: number; amountUsdSum: number; withAmount: number }>;
    total: number;
    withAmount: number;
    withChannel: number;
    /*
     * Происхождение данных. Прислано соседним окном вместе с ручкой, и без
     * него панель врала бы молча: файл покупок лежит В КОНТЕЙНЕРЕ (переменная
     * пути на проде не задана, хотя том подключён), поэтому при КАЖДОЙ выкатке
  ...
  /*
   * Номер запроса: панель перезагружают при смене окна (24 ч / 7 дней / 30
   * дней), а этот запрос идёт отдельно и не дожидается остальных. Значит ответ
   * на СТАРОЕ окно может прийти последним и подменить свежие числа — панель
   * будет выглядеть исправной и показывать выручку не за тот период.
   * Применяем ответ, только если он от последнего запроса.
   */
  const номерЗапроса = useRef(0);
  ...
  async function loadAll() {
    ...
    try {
      ...
      // Отдельным запросом, а НЕ в общем Promise.all: ручка есть не во всех
      // ветках, и её отказ не должен ронять всю сводку.
      const мойНомер = (номерЗапроса.current += 1);
      void (async () => {
        try {
          const r = await fetch(
            apiUrl("/api/pricing/provisioning/subscriptions/by-channel") + `?hours=${hours}`,
            { headers },
          );
          if (мойНомер !== номерЗапроса.current) return; // пришёл ответ на старое окно
          if (r.status === 503) {
            // Осознанный отказ: админ-токен на сервере не задан. Это не поломка.
            setФактическая(null);
            setФактическаяОтвет("закрыта");
            return;
          }
          if (!r.ok) {
            setФактическая(null);
            setФактическаяОтвет("нет");
            return;
          }
          setФактическая(await r.json());
          setФактическаяОтвет("есть");
        } catch {
          if (мойНомер !== номерЗапроса.current) return;
          // Сеть не ответила — это «не знаем», а не «ноль».
          setФактическая(null);
          setФактическаяОтвет("нет");
        }
      })();
          ...
          {summary && (
            <section style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 32 }}>
              ...
              {summary.returnsByProvider && Object.keys(summary.returnsByProvider).length > 0 ? (
                <div style={{ gridColumn: "1 / -1" }}>
                  ...
                  <div style={{ marginTop: 8, fontSize: 12, color: "#78350f", background: "#fef3c7", padding: "8px 10px", borderRadius: 8 }}>
                    Отказ доходит только от кассы, которой задан адрес отмены. Это PayBox и
                    PayPal — и на проде обе НЕ настроены (проверено 03.09.2026: секреты не заданы,
                    и наша же ручка состояния каналов отвечает configured: false). У Gumroad и
                    LemonSqueezy адреса отмены нет вовсе. Значит сегодня отказ не приходит НИ ОТ
                    КОГО: пустая колонка означает «мы не узнаём», а не «никто не отваливается».
                    Настроят кассу с адресом отмены — колонка оживёт сама.
                  </div>
                </div>
              ) : null}
              ...
              <Breakdown
                ...
                accent="#166534"
              />
              {/* Тревогу о расхождении я здесь ЗАВЁЛ И УБРАЛ в тот же час —
                  оставляю разбор, чтобы её не завели снова.

                  Колонки считают РАЗНЫЕ СОВОКУПНОСТИ. Ожидаемая — по событиям,
                  которые шлёт браузер со страницы «оплата принята». Фактическая
                  — по записям подписок, которые пишет вебхук кассы. Покупка, у
                  которой человек закрыл вкладку до возврата, даёт запись БЕЗ
                  события; сбой вебхука даёт событие БЕЗ записи.

                  Значит суммы расходятся по причинам, не имеющим отношения к
                  цене, и тревога была бы вечно красной — ровно тот случай, от
                  которого мы весь день лечим панели.

                  Для дрейфа цены есть верный прибор, и он сравнивает
                  сопоставимое: frontend/scripts/check-gumroad-prices.mjs
                  (наш каталог против страницы товара у продавца). */}
              {фактическая && фактическая.storage?.onVolume === false ? (
                <div
                  style={{
                    gridColumn: "1 / -1",
                    padding: "10px 14px",
                ...
                >
                  История неполная: записи покупок живут в контейнере и обнуляются при
                  каждой выкатке. Ниже — только то, что накопилось после последней.
              ...
              {фактическая ? (
                ...
                >
                  Колонки считают разное: ожидаемая — по событиям со страницы
                  «оплата принята», фактическая — по записям подписок. Суммы могут
                  не совпадать и без всякой ошибки в цене. Сравнивать их напрямую
                  нельзя; для цены есть отдельная проверка каталога против кассы.
                </div>
              ) : null}
              {/* ИНВАРИАНТ, без которого две колонки бессмысленны: обе
                  подписаны ОДНИМИ ключами. Метка `?c=tt` разбирается в имя
                  «tiktok» ещё на витрине (channelFrom), и дальше это имя едет
                  и в событие замера, и в кассу — withChannel кладёт аргумент
                  как есть, вебхук читает custom_data.channel не меняя. Начни
                  где-нибудь ездить короткая метка — колонки перестанут
                  сопоставляться, а выглядеть будут по-прежнему.

                  Фактически списанное — рядом, а не вместо. Две колонки лучше
                  одной именно потому, что РАСХОЖДЕНИЕ между ними и есть сигнал:
                  цену у Gumroad и LemonSqueezy назначает продавец, наша сумма в
                  ссылку не передаётся. Совпали — хорошо; разошлись — повод
                  открыть кабинет кассы. */}
              {фактическая ? (
                <Breakdown
                  title={`${t("pricing.admin.breakdown.chargedRevenueByChannel")} · ${фактическая.withAmount}/${фактическая.total}`}
                  data={Object.fromEntries(
                    Object.entries(фактическая.byChannel).map(([k, v]) => [k, v.amountUsdSum]),
                  )}
                  accent="#1d4ed8"
                />
              ) : (
                ...
                >
                  {фактическаяОтвет === "закрыта"
                    ? "Фактически списанное: ручка закрыта — на сервере не задан админ-токен."
                    : "Фактически списанное: спросить не удалось. Это НЕ ноль и не «продаж нет» — сверять пока не с чем."}
                </div>
              )}
