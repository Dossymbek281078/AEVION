...
export function explainMissingCoef(lsr: Lsr, notice: AiNotice): string | null {
  ...
  lines.push(`доступ ограничен, темп ниже. По СН РК 8.02-05 применяется **К=1.15** к трудозатратам.`);
  lines.push("");
  lines.push(`Это повышает стоимость работ позиции на **15%** (умножается ФОТ и ЭМ; материалы — без коэффициента).`);
...
export function explainHeight(lsr: Lsr, notice: AiNotice): string | null {
  ...
  lines.push(`наверх, ограниченный фронт). По СН РК 8.02-05 (коэф. условий производства) применяется:`);
