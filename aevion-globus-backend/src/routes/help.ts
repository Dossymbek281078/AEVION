 ...
 *
 * ЗАЧЕМ. Форма на /help отправляет сюда с самого начала, но ручки не было ни
 * одной: прод отвечал 404. То есть основной путь не работал никогда, и КАЖДОЕ
 * обращение уходило в запасной — открыть почтовый клиент пользователя.
 *
 * Точности ради: запасной путь рабочий, он ведёт на личную почту основателя, и
 * статус на экране честный («открылся почтовый клиент, нажмите Отправить»).
 * Но он требует настроенного почтового клиента, теряет всех, у кого его нет,
 * и не оставляет у нас никакого следа обращения — ни счётчика, ни истории.
 *
 * Отдельная беда рядом: у домена aevion.app НЕТ записи MX (проверено двумя
 * резолверами, порт 25 на IP сайта молчит), поэтому адреса вида support@ и
 * billing@, напечатанные на юридических страницах, писем не принимают.
 *
