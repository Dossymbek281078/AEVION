...
## ⚠️ BROADCAST 2026-07-28, поздний вечер — сборка chess-humanbot съела 10.8 ГБ

Замер из `aevion-commit-check.ps1` в момент записи:

```
коммит: 96.5 / 97.1 ГБ (99%), свободно 0.6 ГБ
   pid 53008   10,825 МБ  aevion-chess-humanbot   <-- в 2-3 раза больше нормы
   pid 144856     832 МБ  aevion-qreal
```

Обычная сборка Next стоит 3–5 ГБ коммита. Здесь одна сборка держит **10.8 ГБ**,
и на всю машину осталось 0.6 ГБ. На этом уровне процессы перестают
запускаться с `0xc0000142` — и выглядит это как случайный сбой инструментов
(в журнале Windows такое уже было 05.07, 14.07 и 21.07).

**Процесс не трогал**: это чужая зона, а убить идущую сборку — уничтожить
работу той сессии. Решение за владельцем `aevion-chess-humanbot`:

- если сборка зависла — снять её (`Stop-Process -Id 53008 -Force`);
- если это нормальный для неё объём — стоит понять, почему, потому что
  остальным вкладкам при таком расходе сборку не запустить вообще.

Заодно висят две пары дублирующихся бэкендов без подключений (порты 4145/4147
в `aevion-free-fleet` и 4079/4081 в `aevion-qventure`) — по ~340 и ~155 МБ на
процесс. Аудит: `powershell -File C:\Users\user\aevion-devserver-audit.ps1`
(только читает).

Вкладка `aevion-free-fleet` весь вечер тяжёлого не запускала и не будет, пока
не освободится память: продакшен-сборка фронта у нас в ожидании с 04:00.

---

## 🔴 BROADCAST 2026-07-28 — платёжная ссылка DevHub создаётся БЕЗ авторизации

Вкладка `aevion-free-fleet` нашла это на одиннадцатом проходе по признаку «сумма
приходит из тела запроса на денежном пути». Зона `devhub` занята другой сессией
(`session-claim` подтвердил), поэтому **кода не касался** — передаю владельцу.

**Где:** `aevion-globus-backend/src/routes/devhub.ts`, строка ~3303,
`POST /api/devhub/media/payment-link`.

**Что установлено чтением (не догадка):**

1. В обработчике **нет ни одной проверки прав** — ни `verifyBearer`, ни ключа,
   ни роли. Проверено и выше: `devhubRouter.use(` в файле не встречается ни
   разу, монтирование в `index.ts:1186` — `app.use("/api/devhub", devhubRouter)`
   без middleware.
2. Ручка вызывает LemonSqueezy **нашим ключом** (`LEMON_SQUEEZY_API_KEY`) и
   создаёт checkout в **нашем** магазине (`LEMON_SQUEEZY_STORE_ID`).
3. `custom_price` берётся из тела запроса — любая сумма от 50 центов.
4. `product_options.name` и `description` — из тела: произвольное название
   товара в нашем магазине.
5. `redirect_url: successUrl || ...` — **произвольный адрес перенаправления из
   тела, без списка разрешённых**. То есть после оплаты в нашем магазине
   покупателя уводит туда, куда указал вызывающий.

**Чем это опасно:** посторонний может выпускать в нашем магазине ссылки с любым
названием и ценой, под нашим брендом, с уводом покупателя на свой адрес. Это
одновременно репутация, ответственность за возвраты и риск проверки со стороны
LemonSqueezy — а у планеты уже висит блокировка GitHub, второй такой истории не
хочется.

**Насколько это живо ПРЯМО СЕЙЧАС — не проверял и проверять не стал:** если
`LEMON_SQUEEZY_API_KEY` в проде не задан, ручка отвечает 503 и уязвимость
теоретическая. Узнать это можно одним взглядом в переменные Railway. Проверять
POST-запросом я не стал сознательно: валидный запрос СОЗДАЛ БЫ настоящую ссылку
в платёжном провайдере основателя, а это внешнее действие.

**Что предлагаю владельцу зоны:**
1. Закрыть ручку авторизацией (как минимум `verifyBearer`, лучше — проверка, что
   вызывающий владеет проектом, для которого делается ссылка).
2. Сумму брать не из тела, а из своей записи о товаре.
3. `successUrl` — только из списка разрешённых адресов или только свой домен.

---

## ⚠️ BROADCAST 2026-07-28 — `POST /api/i18n/translate` без лимита, а фолбэк платный

Зона `i18n` занята другой сессией (`session-claim` → CLAIMED), кода не касался.
Отдельно скажу прямо: **платный фолбэк в эту ручку добавил я сам** сегодня
(порядок «сначала два бесплатных провайдера, потом claude-haiku»), и не проверил
тогда, чем ручка ограничена. Проверил сейчас.

**Что есть:** ни авторизации, ни `rateLimit` — в файле лимитера нет вообще.
Ручка публичная по замыслу, и это правильно: ею переводит страницы `AutoTranslate`
для анонимных посетителей. Требовать токен нельзя, иначе сломается перевод сайта.

**Что ограничено (честный масштаб, а не паника):**
- `MAX_TEXTS = 200` строк за запрос, `MAX_LEN = 5000` символов на строку;
- кэш на 60 000 записей — повтор той же строки на тот же язык бесплатен;
- бесплатные провайдеры идут первыми, платный Haiku — только когда они не ответили.

**Чем это плохо:** ограничен один запрос, но не число запросов. Когда бесплатный
флот отдаёт 429 (а он отдаёт — ради этого фолбэк и появился), каждый следующий
запрос от кого угодно из интернета тратит НАШИ кредиты Anthropic. Потолка нет.

**Готовый патч (применять владельцу зоны):**

```ts
// рядом с остальными константами файла
const translateLimit = rateLimit({ windowMs: 60_000, max: 20, keyPrefix: "i18n:translate" });

// и на самой ручке
i18nRouter.post("/translate", translateLimit, async (req, res) => {
```

`rateLimit` уже есть в `src/lib/rateLimit` и используется в десятках ручек —
второй способ ограничивать заводить не нужно. Значение 20/мин выбрано так, чтобы
хватало странице (перевод идёт пачками по 200 строк и кэшируется), но не хватало
на прокачку чужого текста через наши кредиты.
...
## Поправка к BROADCAST про платного провайдера (2026-07-28)

В сообщении про `getProviders().find(p => p.configured)` я написал, что затронуты
«публичные» ручки. Уточняю по факту проверки:

- **qjobs `/ai/match` — ТРЕБУЕТ токен** (401 без него). Дефект был реальный —
  вызовы уходили в платного провайдера вместо бесплатного, — но «любой из
  интернета» к нему не относится. Исправлено, есть сторож (`tests/freeProviderFirst.test.ts`,
  мутация роняет 2 из 3).
- **qmedia `/ai/*` — авторизации действительно НЕТ**, там формулировка верна.
- **devhub `/media/payment-link` и i18n `/translate`** — авторизации нет, верно.

Проверять такие вещи надо до отправки сообщения, а не после; здесь я это сделал
после, поэтому исправляю запись, а не оставляю как есть.

---

## Карта «платный провайдер по умолчанию» — корень и все места (2026-07-28, итог)

Свожу три отдельных сообщения в одну картину, чтобы каждый владелец зоны видел
целое, а не свой кусок.

**Корень.** `services/qcoreai/providers.ts`, `resolveProvider(providerId?)`: если
предпочтение не передано, он перебирает `getProviders()` и возвращает **первого
настроенного**. Первым в списке стоит `anthropic`, у него `free: false`. В проде
его ключ задан — значит по умолчанию всегда выбирается платный, хотя во флоте 13
бесплатных провайдеров.

**Где это проявляется (полный перечень, проверено грепом по всему `src`):**

| Место | Как выбирает | Защита ручки | Статус |
|---|---|---|---|
| `qjobs /ai/match` | `find(p => p.configured)` | токен + лимит | **исправлено** (36bd8e01), сторож `tests/freeProviderFirst.test.ts` |
| `qreal /projects/:id/storyboard` | `pickConfiguredProvider()` без предпочтения | лимит добавлен | **исправлено** (f9d20d19) |
| `qmedia /ai/*` ×3 | `find(p => p.configured)` | **нет ни авторизации, ни лимита** | зона занята, патч выше |
| `startupExchange /ideas/:id/ai-score` | `pickConfiguredProvider()` без предпочтения | `aiScoreLimiter` есть | зона занята; расход ограничен лимитом, но платит зря |

Остальные два десятка вызовов используют `find(p => p.id === providerId)` —
провайдер выбран осознанно, к ним вопросов нет. То есть проблема **точечная**, а
не системная.

**Однострочное лечение на месте вызова:**

```ts
const provider = getFreeProviders().find((p) => p.configured) ?? getProviders().find((p) => p.configured);
```

**Предложение владельцу `qcoreai`:** починить корень — в `resolveProvider`
перебирать сначала бесплатных, потом остальных. Тогда все будущие вызовы без
предпочтения станут бесплатными по умолчанию, и это перестанет быть темой.
Правка меняет поведение платформенного слоя, поэтому решать владельцу зоны, а не
мне — я её не делал.

---

## Находка 2026-07-28 — у задач DeepSan нет владельца, удалить может любой

Четырнадцатый проход: DELETE-ручки без проверки владельца. Из семи подозрительных
шесть оказались в порядке или устроены по псевдониму (ShadowNet, LifeBox — там
это осознанная модель без аккаунтов). Настоящая одна.

**`deepsan_tasks` не имеет колонки владельца вообще.** Следствия:

- `DELETE /api/deepsan/tasks/:id` — `DELETE ... WHERE id = $1`, без условий и без
  входа: любой стирает любую задачу, а id — последовательные числа;
- `PATCH /tasks/:id` — то же самое, правит чужое;
- `GET /tasks` — отдаёт все задачи всем.

**Кода я не менял, хотя зона свободна, и вот почему.** Это не забытая проверка,
а отсутствующая модель владения: чтобы починить, нужна колонка владельца,
миграция и правка клиента. Это продуктовое решение, а не устранение дефекта, —
такие я не принимаю сам.

**Что сделал вместо этого:** дописал строку в карточку DeepSan на `/compare` —
«задачи не приватны: любой может прочитать, изменить и удалить чужую», с
основанием «замер». Модуль уже показывается в сравнении, и умолчать об этом
значило бы соврать на витрине.

**Решение основателя:** либо вводим владельца (тогда это отдельная задача с
миграцией), либо честно позиционируем DeepSan как общий демонстрационный список
и не обещаем приватность нигде.
