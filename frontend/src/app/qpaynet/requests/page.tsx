...
const STATUS: Record<PayRequest["status"], { key: string; chip: string }> = {
  pending:   { key: "qpaynet.requests.status.pending",   chip: "bg-amber-900 text-amber-300" },
  paid:      { key: "qpaynet.requests.status.paid",      chip: "bg-emerald-900 text-emerald-300" },
  cancelled: { key: "qpaynet.requests.status.cancelled", chip: "bg-slate-800 text-slate-500" },
  expired:   { key: "qpaynet.requests.status.expired",   chip: "bg-slate-800 text-slate-500" },
};
...
export default function MyRequestsPage() {
  ...
  useEffect(() => {
    ...
    fetch(apiUrl("/api/qpaynet/requests"), { headers: { Authorization: `Bearer ${saved}` } })
      ...
      .catch(() => setError(t("qpaynet.requests.err.load")))
      .finally(() => setLoading(false));
  }, []);

  async function cancel(id: string) {
    if (!confirm(t("qpaynet.requests.confirmCancel"))) return;
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <header className="border-b border-slate-800 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-sm font-bold">{t("qpaynet.requests.title")}</h1>
        </div>
      ...
      <div className="max-w-3xl mx-auto px-6 py-6 space-y-4">
        ...
        {reqs.length > 0 && (
          <div className="flex gap-2 flex-wrap">
            {[["all", t("qpaynet.requests.filter.all")], ["pending", t("qpaynet.requests.filter.pending")], ["paid", t("qpaynet.requests.filter.paid")], ["cancelled", t("qpaynet.requests.filter.cancelled")], ["expired", t("qpaynet.requests.filter.expired")]].map(([v, l]) => (
        ...
        {loading && <div className="text-slate-500 text-sm py-12 text-center">{t("qpaynet.requests.loading")}</div>}
        {error && <div className="text-red-400 text-sm py-4">{error}</div>}
        {!loading && !error && reqs.length === 0 && (
          <div className="text-center py-16 text-slate-600">
            <div className="text-4xl mb-4">📥</div>
            <p className="text-sm">{t("qpaynet.requests.empty")}</p>
            <Link href="/qpaynet/request" className="text-violet-400 hover:text-violet-300 text-sm underline mt-2 inline-block">
              {t("qpaynet.requests.createFirst")}
            </Link>
          </div>
        )}

        {!loading && filtered.length === 0 && reqs.length > 0 && (
          <div className="text-center py-12 text-slate-600 text-sm">{t("qpaynet.requests.emptyCategory")}</div>
        )}

        <div className="space-y-2">
          {filtered.map(r => {
            const s = STATUS[r.status];
            const statusLabel = t(s.key);
            return (
              <div key={r.id}
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <span className={`text-xs px-2.5 py-1 rounded-full font-semibold ${s.chip}`}>{statusLabel}</span>
                      <span className="text-xs text-slate-500">{fmtDate(r.created_at, lang)}</span>
                      {r.expires_at && <span className="text-xs text-slate-600">{t("qpaynet.requests.expires")} {fmtDate(r.expires_at, lang)}</span>}
                    </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button onClick={() => copyLink(r.payUrl, r.id)}
                      ...
                      {copied === r.id ? t("qpaynet.requests.copied") : t("qpaynet.requests.link")}
                    </button>
                    {r.status === "pending" && (
