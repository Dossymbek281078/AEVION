...
/**
 * What may stand between a metric and its figure.
 *
 * This was written out at twenty-four sites, each slightly different, which is
 * why the pipe of a markdown table and the em dash of a slide label were
 * readable at none of them: there was nowhere to add them once. Each site now
 * starts from one of these and appends only the words it genuinely needs
 * ("between" for a band, "were|was" for a past-tense filing line).
 *
 * LINK_NO_DASH exists for gross margin alone. Everywhere else a dash between a
 * label and a figure is punctuation; there the sign is the finding, and
 * "Gross margin — 45%" must stay refused rather than become +45.
 */
...
export function parsePlanSignals(text: string): PlanSignals {
  ...
  const ratioRange = firstMatch(t, new RegExp(String.raw`ltv[:/ ]*cac\s*(?:${LINK})?\s*${NUM}\s*(?:x|:\s*1)?\s*(?:-|–|—|to)\s*${NUM}\s*(?:x|:\s*1)?`, "i"))
    || firstMatch(t, new RegExp(String.raw`ltv[:/ ]*cac\s*(?:${LINK})?\s*between\s*${NUM}\s*(?:x|:\s*1)?\s*and\s*${NUM}\s*(?:x|:\s*1)?`, "i"));
  if (ratioRange) {
...
function detectRevenueConflict(t: string, s: PlanSignals, planCurrency: MoneyCurrency | null): void {
  ...
  const nameFirst = new RegExp(String.raw`(?:net\s*|total\s*)?(?:${REV_NOUN})\s*(?:${LINK}|were|was|${TO_LEVEL})?\s*${CUR}${NUM}\s*${UNIT}`, "gi");
