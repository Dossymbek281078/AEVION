...
// Сторож обходит дерево src и читает каждый файл. В одиночку это укладывается в
// дефолтные 5с, а при полном параллельном прогоне — нет: замер 12.08.2026 дал
// 7312 мс и КРАСНЫЙ тест при полностью исправном коде (в одиночку те же 6
// проверок зелёные). Ложная краснота у сторожа хуже его отсутствия: к ней
// привыкают и перестают смотреть. Тяжёлому файловому тесту — свой явный таймаут,
// как у PaywallModal.test.tsx; на скорость одиночного прогона это не влияет.
