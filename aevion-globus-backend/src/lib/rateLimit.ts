 ...
 *
 * Ведущая гипотеза теперь другая: `app.set("trust proxy", 1)` доверяет ОДНОМУ
 * узлу, а цепочка длиннее, и `req.ip` оказывается адресом КРАЕВОГО УЗЛА, а не
 * клиента. Тогда один клиент размазан по нескольким корзинам, а все клиенты
 * одного узла делят одну — то есть предел не «на посетителя» ни в одну сторону.
 *
 * Гипотеза НЕ доказана: чтобы доказать, нужно увидеть, каким наш адрес видит
 * сервер. Пока этого нет, здесь стоит ИЗМЕРЕНИЕ, а не объяснение. Уверенная
 * неверная причина хуже отсутствия причины: она отправляет следующего строить
 * общий счётчик в Redis, который этой поломки не вылечит.
 *
