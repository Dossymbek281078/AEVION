...
async function run() {
  ...
  assert("x-error-codes registry present", typeof spec.body["x-error-codes"] === "object");
  ...
  // 36. Wallet metadata roundtrip + 4KB cap
  console.log("\n36. Wallet metadata");
  const metaWallet = await req("POST", "/api/qpaynet/wallets",
    { name: "meta-test", metadata: { merchantOrderId: "ORD-123", customerEmail: "test@partner.com" } }, auth);
  assert("create with metadata → 201", metaWallet.status === 201);
  assert("metadata echoed in create response",
    metaWallet.body.metadata?.merchantOrderId === "ORD-123");
  const metaGet = await req("GET", `/api/qpaynet/wallets/${metaWallet.body.id}`, null, auth);
  assert("GET wallet returns metadata",
    metaGet.body.metadata?.merchantOrderId === "ORD-123");
  const metaUpdate = await req("PATCH", `/api/qpaynet/wallets/${metaWallet.body.id}`,
    { metadata: { merchantOrderId: "ORD-456" } }, auth);
  assert("PATCH metadata → 200", metaUpdate.status === 200);
  assert("metadata updated",
    metaUpdate.body.metadata?.merchantOrderId === "ORD-456");
  const tooBig = await req("POST", "/api/qpaynet/wallets",
    { name: "big-meta", metadata: { junk: "x".repeat(5000) } }, auth);
  assert("metadata > 4KB → 400 metadata_too_large",
    tooBig.status === 400 && tooBig.body.error === "metadata_too_large");

  // 37. Error code registry shape
  console.log("\n37. Error code registry");
  assert("registry covers money codes",
    typeof spec.body["x-error-codes"]?.money?.insufficient_balance === "string");
  assert("registry covers lifecycle codes",
    typeof spec.body["x-error-codes"]?.lifecycle?.tx_already_refunded === "string");
  assert("registry covers idempotency codes",
    typeof spec.body["x-error-codes"]?.idempotency?.idempotency_key_body_mismatch === "string");

  // 38. Merchant key scopes — read-only key cannot charge
  console.log("\n38. Merchant key scopes");
  const readOnlyKey = await req("POST", "/api/qpaynet/merchant/keys",
    { walletId, name: "read-only", scopes: "read" }, auth);
  if (readOnlyKey.status === 201) {
    assert("scopes returned in create response",
      Array.isArray(readOnlyKey.body.scopes) && readOnlyKey.body.scopes.includes("read"));
    assert("read-only key omits charge scope",
      !readOnlyKey.body.scopes.includes("charge"));
    // Try to charge with read-only key
    const chargeAttempt = await fetch(`${BASE}/api/qpaynet/merchant/charge`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-API-Key": readOnlyKey.body.key },
      body: JSON.stringify({ customerWalletId: walletId, amount: 1 }),
    });
    const chargeBody = await chargeAttempt.json();
    assert("read-only key charge → 403 scope_missing",
      chargeAttempt.status === 403 && chargeBody.error === "scope_missing");
    // Cleanup
