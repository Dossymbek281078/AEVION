...
async function main() {
  ...
  assert(cityIds.includes("astana") && cityIds.includes("nyc") && cityIds.includes("tokyo") && cityIds.includes("singapore") && cityIds.includes("amsterdam") && cityIds.includes("berlin") && cityIds.includes("vienna") && cityIds.includes("zurich"), "registry has astana + nyc + tokyo + singapore + amsterdam + berlin + vienna + zurich", cityIds.join(","));
  ...
  const jtamper = await jpost("/api/qskyway/route/justification/verify", {
    // 15.09.2026: было `inertPenaltySegments: 0` — а у Астаны с обмером города он и так 0,
    // подделка становилась пустой, и «подпись цела» читалось как провал. Меняем на
    // значение, которое ТОЧНО отличается от исходного.
    document: { ...jda, blindHeight: { ...jda.blindHeight, inertPenaltySegments: (jda.blindHeight.inertPenaltySegments ?? 0) + 1 } },
