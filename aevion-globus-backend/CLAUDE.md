...
## 10. Конвенция деплой-путей: deploy = uploaded + serves

Урок 2026-07-21 (PR #760/#768): у DevHub было три класса вранья в деплоях —
CF Pages «success» без сохранённых ассетов (каждая страница 500-ила месяцами),
Railway-фолбэк, симулировавший успешный билд на выдуманном URL, и «live через
5 секунд» без единой проверки. Все убиты. Чтобы не родить новые:

1. **Ни один деплой-путь не помечает deployment/project «live», пока
   задеплоенный URL реально не отдал 2xx.** Используй существующий
   `verifyDeploymentServes()` из `src/routes/devhub.ts` (5 попыток × 5с);
   не отдал — статус `failed` с явной причиной в buildLog.
2. **Никаких симуляций успеха на реальных путях.** Апстрим недоступен /
   вернул мусор → `failed` + причина. Симуляция допустима только в явном
   dev-режиме без токена (и её buildLog должен говорить, что это симуляция).
3. **2xx от API провайдера ≠ работающий деплой.** CF отвечал
   `deploy: success` на деплои без ассетов. Проверяй результат, который
   увидит человек (страницу), а не код ответа загрузки.
4. **CF Pages — только через wrangler** (`src/lib/wranglerPagesDeploy.ts`);
   сырой REST-multipart мёртв: manifest принимает, ассеты выбрасывает.
5. **Каждый новый live-модуль со страницей** — добавь её в
   `scripts/pages-live-smoke.js` (daily-набор открывает страницы, не только API).
