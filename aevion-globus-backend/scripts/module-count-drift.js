...
if (problems.length) {
  // REPORT-ONLY, пока копирайт не приведён в порядок.
  //
  // На 2026-07-27 находок 16: на витринах живут ЧЕТЫРЕ разных счётчика —
  // 27, 29, 37 и 38, — и ни один не совпадает с реестрами (43/32/42/41).
  // Часть требует решения автора текста, а не механической замены:
  // «37 модулей × 4 тарифа» на /pricing/compare заодно называет 4 тарифа
  // при шести реальных. Ставить гейтом сразу — уронить общий смоук из-за
  // копирайта; поэтому сначала отчёт, а EXIT_ON_DRIFT=1 включает гейт.
  const gate = process.env.EXIT_ON_DRIFT === "1";
  ...
  log(
    "\nЧисло модулей на продающей странице берётся из реестра, а не из памяти.\n" +
      ...
      (gate ? "" : "REPORT-ONLY: гейт включается через EXIT_ON_DRIFT=1.\n"),
  );
