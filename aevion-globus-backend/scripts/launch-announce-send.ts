 ...
 *
 *     ⚠️ ГРАНИЦА, И ОНА ВАЖНАЯ: этот список ЛОКАЛЬНЫЙ. Запуск с другой машины
 *     о нём не знает и разошлёт всё заново — по всему списку, живым людям.
 *     Поэтому отправлять нужно С ОДНОЙ машины, а путь к файлу скрипт печатает
 *     в отчёте, чтобы его было видно и можно было сохранить.
 *
...
// Путь берётся от РАСПОЛОЖЕНИЯ скрипта, а не от текущей папки.
//
// Замер 08.09.2026, вечер: копий репозитория СО СКРИПТОМ на машине
// тринадцать. Прежняя редакция брала путь от каталога скрипта — это чинило
// зависимость от текущей папки, но НЕ от того, из какой копии позвали:
// тринадцать копий вели бы тринадцать РАЗНЫХ реестров.
//
// Чем это грозит: рассылку запускают повторно ровно тогда, когда первая
// оборвалась на середине — не влезли в суточный потолок Brevo (300), упал
// интернет. Запуск из соседней папки решит, что не отправлял НИКОМУ, и
// пошлёт письмо по второму разу всем, кто уже получил. Отозвать нельзя.
//
// Поэтому реестр живёт ВНЕ рабочих копий, в домашней папке: она одна на
// машину. Заодно исчезает риск утащить адреса людей в git — раньше от него
// защищал .gitignore, теперь файла нет в дереве вовсе.
//
// Переопределяется переменной на случай другой машины или переноса.
const каталогРеестра =
  process.env.AEVION_LAUNCH_SENT_DIR?.trim() ||
  join(process.env.USERPROFILE || process.env.HOME || join(__dirname, ".."), ".aevion-launch-sent");
...
async function main(): Promise<void> {
  ...
  console.log(`След отправок:     ${файлОтправленных}`);
  if (!existsSync(файлОтправленных)) {
    // Пустой след значит одно из двух, и различить их отсюда нельзя:
    // либо не отправляли, либо отправляли С ДРУГОЙ МАШИНЫ. Второе тише и
    // дороже: рассылка уйдёт по второму разу.
    console.log("                   (следа нет: либо не отправляли, либо отправляли с другой машины)");
  }
  ...
  // ССЫЛКА ОТПИСКИ ОБЯЗАТЕЛЬНА, И ПРОВЕРЯЕТСЯ ДО ПЕРВОГО ПИСЬМА.
  //
  // Замер 08.09.2026: письмо, собранное БЕЗ ключа подписи, ставит вместо
  // ссылки личный адрес основателя — и он уходит всем подписчикам. Отправка
  // при этом идёт как ни в чём не бывало: молчаливая деградация на самом
  // видном месте. С ключом ссылка появляется, адрес исчезает — проверено
  // сборкой письма обоими способами.
  //
  // Почему ОТКАЗ, а не предупреждение: письмо можно послать через час, а
  // личный адрес из трёхсот ящиков не вернёшь. Почему ДО первого письма:
  // иначе половина списка уже уйдёт неправильной.
  //
  // Крупные почты для массовых рассылок требуют отписку В ОДИН КЛИК —
  // "напишите на адрес" ею не является, и письмо уходит в спам вернее.
  const пробаОтписки = unsubscribeUrl("probe@example.com");
  if (!пробаОтписки) {
    const беда = [
      "",
      "🔴 ССЫЛКУ ОТПИСКИ НЕЧЕМ ПОДПИСАТЬ — рассылка остановлена, письма НЕ отправлены.",
      "",
      "Без ключа письмо поставит вместо ссылки личный адрес основателя.",
      "",
      "Нужна WAITLIST_UNSUB_SECRET (или AUTH_JWT_SECRET) В ЭТОМ ОКНЕ.",
      "На проде она задана — возьмите оттуда, значение нигде не печатается:",
      "",
      "  Set-Location C:/Users/user/aevion-launch/aevion-globus-backend",
      "  $j = npx --yes @railway/cli variables --json | ConvertFrom-Json",
      "  $env:WAITLIST_UNSUB_SECRET = [string]$j.WAITLIST_UNSUB_SECRET",
      "  [bool]$env:WAITLIST_UNSUB_SECRET   # должно напечатать True",
      "",
      "⚠️ Каталог обязателен. Из непривязанной папки railway отвечает",
      "   «не задано» ПРО ВСЕ переменные разом — это почти наверняка вызов",
      "   не оттуда, а не отсутствие ключа. Признак: пустыми выглядят сразу",
      "   все переменные, а не одна.",
      "",
    ].join(String.fromCharCode(10));
    if (ЖИВОЙ) стоп(2, беда);
    console.warn(беда);
    console.warn("Это сухой прогон, поэтому продолжаю — но с таким ключом отправка будет остановлена.");
  }
