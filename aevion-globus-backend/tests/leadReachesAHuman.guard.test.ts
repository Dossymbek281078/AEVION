...
/**
 * ⏱ Про медленные прогоны — и почему здесь НЕТ своего короткого предела.
 *
 * ЗАМЕР 17.09.2026: два прогона упали медленно (32.4 с и 23 с) при обычных
 * 1–2 с; около пятнадцати остальных — зелёные, включая прогоны с чистого кэша
 * и совместные с соседним сторожем. Оба красных пришлись на минуты нагрузки
 * (второй — ровно когда перезапускались все окна).
 *
 * Сначала я поставил сюда предел 5 с, чтобы «несработавшая подмена почты
 * падала быстро». Это было НЕВЕРНО, и вот почему: vitest.config.ts этого
 * проекта держит testTimeout 30 с по ЗАМЕРУ — под нагрузкой машины
 * `vi.resetModules` + динамический импорт крупного маршрута «легко перерастает
 * десять секунд». Этот сторож делает ровно это, четыре раза заново импортируя
 * pricing.ts. Короткий предел перечёркивал измеренное решение проекта и
 * превращал медленный импорт под нагрузкой в ложный красный.
 *
 * Причина двух медленных падений всё равно НЕ УСТАНОВЛЕНА: версия «тяжёлый
 * импорт под нагрузкой» совпадает с записанным в конфиге классом, но не
 * доказана. Поэтому защита от главного риска — ухода в живую почту — сделана
 * не пределом времени, а прямым контролем ниже: первый тест спрашивает сам
 * подменённый модуль, активна ли подмена. Если нет — падение внятное, с
 * названием причины, а не таймаут, который потом нечем объяснить.
 */
