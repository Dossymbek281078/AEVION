 ...
 */
/**
 * ГРАНИЦА, названная честно (проверено мутацией 29.08.2026).
 *
 * Проверка смотрит на строку ПОКАЗА. Она НЕ видит случая, когда сырое
 * значение сервера кладут в нашу структуру раньше, а показывают потом:
 *
 *   const issue = { error: d.error };   // <- сюда сторож не смотрит
 *   ...
 *   setGithubMsg(issue.error);          // <- а здесь уже наше поле
 *
 * Ровно так прошли мимо три показа, найденные чтением. Они закрыты в ТОЧКЕ
 * ВХОДА (перевод ставится там, где значение входит в структуру), но мутация
 * этого места сторож не ловит — и об этом надо знать, а не думать, что
 * зелёный цвет закрывает весь класс.
 */
  ...
  it("каждый выброс с текстом сервера идёт через devhubServerError", () => {
    ...
    for (const f of files) {
      ...
      lines.forEach((l, i) => {
        ...
        // Список имён убран: он был угадыванием. Замер 29.08.2026 — имён
        // перед .error оказалось десять, сторож знал пять, и три показа
        // сырого текста прошли мимо (issue.error, githubIssue.error,
        // r.error в списке переводов).
        //
        // Теперь ловим любое <имя>.error, кроме пойманного исключения: у
        // `e`/`err` к моменту показа текст уже переведён в месте выброса.
        // Любое <имя>.error, кроме пойманного исключения: у `e`/`err` к
        // моменту показа текст уже переведён в месте выброса.
        //
        // Без регулярки намеренно: собранная строкой, она в этой среде
        // теряет обратные слэши и молча перестаёт находить.
        const fromServer =
          l.includes(".error") && !l.includes("e.error") && !l.includes("err.error");
