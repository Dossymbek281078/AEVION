...
describe("мусор на входе не выдаётся за модель", () => {
  // 13.09.2026: прежняя редакция разрешала «ЛИБО пустой план, ЛИБО слова».
  // Это допускало молчание: пустой план без единого предупреждения проходил
  // проверку, а на экране человек видел бы, что ничего не произошло, — и
  // решил бы, что сломан модуль, а не что файл не тот.
  //
  // Замер показал, что код УЖЕ молчит ни на одном входе: 8 случаев из 8 дают
  // предупреждение, причём двух разных видов — «не ASCII-DXF» и «нет ни
  // одного отрезка». Значит ужесточение ничего не ломает и просто закрывает
  // дверь на будущее: честный ноль сегодня не гарантирует его завтра.
  it("DXF: любой мусор ОБЯЗАН объяснить словами, а не промолчать", () => {
    for (const g of МУСОР) {
      const r = parseDxf(g);
      const пусто = r.plan === null || r.plan.walls.length === 0;
      expect(пусто, `мусор «${g.slice(0, 20)}» дал НЕпустую модель`).toBe(true);
      expect(
        r.warnings.length,
        `мусор «${g.slice(0, 20)}» отработал МОЛЧА: ни модели, ни объяснения`,
      ).toBeGreaterThan(0);
      for (const w of r.warnings) ЧЕЛОВЕЧНО(w);
    }
  });

  // Контроль прибора в обратную сторону: список мусора не должен исчерпаться
  // до нуля (тогда цикл выше не сделал бы ни одного утверждения).
  it("контроль: список мусора непустой и охватывает разные виды", () => {
    expect(МУСОР.length).toBeGreaterThan(3);
  });
