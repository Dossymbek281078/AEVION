 ...
 *
 * ⚠️ ЧЕГО ЭТОТ СТОРОЖ МОЖЕТ ЗАПРЕТИТЬ ЗРЯ. Границу ожидания требует Next для
 * СТАТИЧЕСКОЙ генерации. У страницы есть законный второй выход: объявить её
 * динамической (`export const dynamic = "force-dynamic"`) — тогда Suspense не
 * нужен, а сторож всё равно покраснеет. Если это случилось, расширять надо
 * ЕГО, а не обходить страницу: красный на правильном коде отключают в первый
 * же день, и защиты не остаётся вовсе.
 *
