...
const ALLOWED_BACKEND: Array<{ fragment: string; reason: string }> = [
  // 20.09.2026: исключение для цитаты отзыва в trust.ts снято вместе с самими
  // демо-отзывами — публиковать выдуманные отзывы под рекламу нельзя.
  // 22.09.2026: снято и это исключение. Кейсы с выдуманными клиентами
  // вынесены в cases.unpublished.ts и не публикуются, значит и отставная
  // цена $19 больше нигде не печатается. Сторож сам об этом и сказал:
  // исключение, которое ни на что не указывает, — будущая слепота.
];
