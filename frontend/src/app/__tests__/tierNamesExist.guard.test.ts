...
/**
 * ЧЕГО ЗДЕСЬ НЕТ И ПОЧЕМУ — про время ответа поддержки.
 *
 * 19.08.2026 я написал рядом второго сторожа: обещанные часы в текстах должны
 * совпадать с supportSlaHours в реестре. Сейчас они СОВПАДАЮТ (проверено:
 * Lite/Medium 24ч, Full 8ч, Universe 6ч, Enterprise 1ч), и проверка была
 * зелёной. Но на мутации — подменил в тексте «6h» на «2h» — она осталась
 * зелёной тоже, дважды: и в варианте с одной составной регуляркой, и в
 * переписанном построчном.
 *
 * Отладка показала, что внутри vitest проверка ВИДИТ и ключ, и мутированное
 * значение («2h support SLA» в захваченной строке), и правильные часы из
 * реестра (pro = 6) — а список расхождений всё равно пуст. Та же логика,
 * запущенная отдельным node-скриптом на том же файле, находит расхождение.
 * Причину за разумное время не нашёл.
 *
 * Поэтому сторожа здесь нет. Проверка, которая не краснеет на собственном
 * случае, выдаёт зелёный за работу, которой не делала, — это хуже, чем её
 * отсутствие: на неё начинают полагаться. Совпадение часов на 19.08.2026
 * зафиксировано замером, а не проверкой, и это честнее.
 */
