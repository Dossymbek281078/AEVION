...
export default function QRightWebhookDetailPage({
  params,
}: {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [whRes, dlRes] = await Promise.all([
        fetch(apiUrl("/api/qright/webhooks"), { headers: authHeaders() }),
        fetch(apiUrl(`/api/qright/webhooks/${encodeURIComponent(id)}/deliveries?limit=100`), {
          headers: authHeaders(),
        }),
      ]);
      if (whRes.ok) {
  ...
  const retry = async (deliveryId: string) => {
    ...
    try {
      const r = await fetch(
        apiUrl(`/api/qright/webhooks/${encodeURIComponent(id)}/retry/${encodeURIComponent(deliveryId)}`),
            ...
            <>
              ...
              {deliveries.length === 0 ? (
                <div style={{ ...card, textAlign: "center", color: "#94a3b8", fontSize: 12 }}>
                  No deliveries yet — fire one by revoking an object.
                </div>
              ) : (
