...
export const metadata: Metadata = {
  ...
  title: { absolute: "AEVION — что почитать и попробовать" },
  description:
...
export default async function GoPage({
  ...
  // Книга ТОЛЬКО ТЕКСТОМ, $9.99. Держим её рядом с полным пакетом, потому что
  // за всё время у нас две продажи, и обе — именно она (замер 20.09.2026).
  const bookText = productById("orcfbo");
      ...
      <div style={styles.wrap}>
        ...
        <section style={styles.section}>
          <WaitlistCapture
            ...
            // AutoTranslate переводит EN-визитёру весь текст страницы, но НЕ
            // атрибуты — человек из шапки соцсетей видел английскую страницу
            // с русским «вы@почта.рф» ровно в поле контакта. Форма идёт за
            // языком посетителя: русскому — русская, как и раньше.
        ...
        <section style={styles.section}>
          ...
          {/*
            🔴 Книга за $9.99 отдельной карточкой — 22.09.2026, по слову
            основателя «доведи до оплат».

            ЗАМЕР, из которого правка: за всё время у нас ДВЕ продажи, и обе —
            именно эта книга, самая дешёвая позиция каталога. На /go её не
            было: страница признавала «есть версии дешевле» словами, со
            ссылкой в магазин и БЕЗ ЦЕНЫ. То есть единственный товар, который
            у нас реально покупают, стоял на один переход дальше остальных и
            без числа, по которому решают.

            Позиционирование не меняется — про версию дешевле страница
            говорила и раньше. Меняется одно: у неё появились цена и кнопка.
            Цена берётся из каталога, не переписана числом.
          */}
          {bookText && (
            <LinkCard
              href={withChannel(bookText.href, channel, "go")}
              external
              product={bookText}
              channel={channel}
              kicker={bookText.format}
              title="Только текст книги"
              note="Та же книга без аудио и дополнительных материалов. Самый доступный вход."
              price={priceOf(bookText)}
            />
          )}
          <div style={styles.aside}>
            Есть версия с аудиокнигой:{" "}
