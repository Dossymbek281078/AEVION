 ...
 *
 * QVenture выходит 20.09.2026 за $39/мес. Доступ выдаётся так: planGate
...
describe("QVenture говорит, от чего зависит доступ", () => {
  it("контроль: страница прочитана и это она", () => {
    ...
    expect(SRC, "читается не та страница").toContain("инструмент ИИ-отбора");
  });
  ...
  it("сказано, что доступ привязан к почте подписки", () => {
    ...
    ).toMatch(/email on your subscription|почт[еы] подписки|адресу почты в подписке/i);
  });
...
).toMatch(/sign in with that\s+same email|войдите с той же|входите с тем же\s+адресом/i);
  });
});
