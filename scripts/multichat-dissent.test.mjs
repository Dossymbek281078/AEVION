...
// ── Служебные слова обязаны отсекаться ────────────────────────────────────
//
// Список STOP состоял почти целиком из слов короче трёх букв, а их и так убирает
// фильтр length > 2. Проверено пословно 27.07: из 28 частых русских служебных
// слов не отсекалось ни одно, из 15 английских — тоже. Цена: два ответа,
// противоположных по смыслу, но с одинаковым обрамлением, давали схожесть 0.714
// при пороге консенсуса 0.45 — карта объявляла согласие там, где идёт спор.
for (const w of ["который","также","есть","может","нужно","стоит","будет","однако","если","очень","потому","чтобы",
                 "would","because","which","however","also","very","been","should"]) {
  ok(`служебное слово отсекается: ${w}`, !m.tokens(w + " канал").includes(w), JSON.stringify(m.tokens(w + " канал")));
}

// Оценочные слова НЕ служебные: в них и живёт разногласие, отсекать их нельзя.
for (const w of ["достаточно","выгодно","неэффективно","рано","рискованно"]) {
  ok(`оценочное слово сохранено: ${w}`, m.tokens(w + " канал").includes(w));
}

// Измеримый эффект на том самом случае: было 0.714, стало ниже.
const framedDifferently = m.similarity(
  "Если честно, это будет очень неэффективно, потому что нужно сначала проверить канал, однако решение стоит принять.",
  "Если честно, это будет очень выгодно, потому что нужно просто включить оплату, однако решение стоит принять.",
);
ok("одинаковое обрамление больше не тянет схожесть к 0.7",
  framedDifferently < 0.6, String(framedDifferently));

// Обратная проверка: дословное совпадение обязано остаться полным согласием,
// иначе чистка списка сломала бы основной сигнал.
ok("дословно совпадающие ответы по-прежнему 1.000",
