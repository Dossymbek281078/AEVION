...
export const TRUST_NUMBERS: TrustNumber[] = [
  ...
  // утверждений. Ни одно не исправлено здесь намеренно — см. ниже, почему.
  //
  // 1. «27 модулей» — устарело: в реестре projects.ts сейчас 41 запись.
  //    Фронтовый сторож scaleClaims.guard ловит такие числа, но он сканирует
  //    только frontend/src и до этого файла не достаёт.
  // 2. «на Business» — тарифа Business не существует: он deprecated-алиас без
  //    объекта тарифа (см. TierId в data/pricing.ts).
  ...
  //
  // Почему не поправил: цифры 2–4 — это коммерческие обещания и заявления о
  // трафике. Подставить сюда правдоподобное число значит выдумать его, а это
  // ровно то, против чего затевался весь аудит. Нужен ответ основателя: какая
  // лестница SLA настоящая и откуда взяты счётчики.
