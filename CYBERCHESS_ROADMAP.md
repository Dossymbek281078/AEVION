...
**Осталось по обнаружимости/полировке (приоритет):**
1. Home/setup-экран: карточки-промо для Ghost Duel / Экономики / AI-коуча (как «Задача дня»).
2. AI-коуч тумблер — дать понятный лейбл + тултип (сейчас безымянная иконка).
3. Перф: page.tsx = 13.7k строк / ~140 useState — декомпозиция под мобильные.
4. Решить судьбу Multiverse (возродить или снять из киллер-листа).
5. Ecosystem-балласт (AevionMiniHub, Projects-баннер) — НЕ трогаем: осознанная кросс-промо стратегия экосистемы.
