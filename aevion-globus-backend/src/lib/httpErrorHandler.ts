 ...
 *
 * Округление ВНИЗ, и это не косметика: число здесь называет ПРЕДЕЛ. Округлив
 * вверх, мы назовём предел больше настоящего — человек пришлёт ровно столько,
 * сколько ему сказали, и получит отказ на своё же соблюдение правила. Названный
 * предел обязан быть достижимым.
 *
 * Сегодня все пределы кратны (10 МБ, 256 КБ) и округление не срабатывает вовсе;
 * защита ставится на день, когда кто-то задаст предел вроде 500 000 байт.
 */
function humanBytes(n: number): string {
  if (n >= 1024 * 1024) {
    const mb = Math.floor((n / (1024 * 1024)) * 10) / 10;
    return `${Number.isInteger(mb) ? mb : mb.toFixed(1)} МБ`;
  }
  return `${Math.floor(n / 1024)} КБ`;
}
