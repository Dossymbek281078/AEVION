...
describe("наружу можно, внутрь нельзя", () => {
  ...
  it("ручка ДЕЙСТВИТЕЛЬНО зовёт проверку, а не просто импортирует", () => {
    ...
    // Регистрация ручки, а не первое упоминание пути: с 15.09.2026 путь встречается раньше —
    // в списке суточного ограничителя загрузок (devhubRouter.use([...])), и срез оттуда пуст.
    const i = src.indexOf('devhubRouter.post("/media/upload-audio"');
