...
export function odnaFraza(s: GameSummary): string {
  ...
  if (s.zevkov === 0) {
    // Глагол согласуется с числом так же, как существительное: одна ошибка
    // стоилА, две стоилИ. Замечено на живой сборке — существительное
    // склонялось, глагол остался во множественном.
    const stoil = ccPlural(s.oshibok, "стоила", "стоили", "стоили");
    return `Грубых зевков нет, но ${s.oshibok} ${ccPlural(s.oshibok, "ошибка", "ошибки", "ошибок")} ${stoil} позиции.`;
  }
