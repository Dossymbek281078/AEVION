...
describe("непроверенная продажа Gumroad видна снаружи", () => {
  ...
  test("в тревоге назван идентификатор продажи — иначе её не с чем сверить", async () => {
    ...
    //
    // Мерж 06.09: идентификатор ищется в КОНТЕКСТЕ, а не в тексте ошибки.
    // Приклеенный к тексту saleId дробил группировку Sentry — сто выдач
    // выглядели бы как сто разных issue, и всплеск не был бы виден. Текст
    // постоянный (это охраняет сосед unverifiedGumroadSaleLeavesATrace),
    // сверяемость даёт поле контекста.
    await ping();
    const hit = captured.find((c) => c.message.includes("gumroad_sale_unverifiable_provisioned"));
    expect(
      JSON.stringify(hit?.ctx ?? {}),
      "тревога без номера продажи не сверяется ни с чем",
    ).toContain("unverified_sale_");
  });
});
