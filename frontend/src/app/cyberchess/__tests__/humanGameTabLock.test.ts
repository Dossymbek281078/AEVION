...
describe("режим стримера скрывает рейтинг независимо от порядка действий", () => {
  it("панель статистики не рисуется при включённом режиме", () => {
    /* Все входы в панель и так закрыты в этом режиме — проверено поимённо. Но панель
       можно было открыть ДО включения режима, а включить его из палитры: тогда обещание
       «скрывает рейтинг и историю» нарушалось порядком нажатий. Гарантия на отрисовке
       от порядка не зависит. */
    expect(page).toMatch(/open=\{showStatsDashboard&&!streamerMode\}/);
  });
});
