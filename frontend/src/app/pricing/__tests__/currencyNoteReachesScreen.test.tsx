...
describe("подпись о валюте на экране", () => {
  it("страница тарифов отрисовывается и несёт подпись о списании", async () => {
     ...
     */
    // Мерж 06.09: словарь распилен по языкам, старый монолит i18n-data.ts
    // стал тонким модулем без ключей. В тестовой среде страница рисуется
    // английской (умолчание I18nProvider без сохранённого выбора), поэтому
    // сверяем с en.ts — тот же язык, что видит рендер.
    const словарь = readFileSync(join(process.cwd(), "src/lib/i18n-lang/en.ts"), "utf8");
