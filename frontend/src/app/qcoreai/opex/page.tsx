  ...
  return (
    <div className="min-h-screen bg-[#0b0f1a] text-white">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="mb-6">
          <h1 className="text-xl font-bold">Расходы по провайдерам</h1>
          <p className="text-slate-400 text-sm mt-0.5">
            Платформенный учёт: все записанные вызовы, все пользователи. Стоимость считается по реальным
            токенам каждого вызова и прайс-листу провайдера. Персональная аналитика —{" "}
        ...
        {!loading && !data && (
          <div className="text-center py-16 text-slate-500 text-sm">Данные OPEX недоступны.</div>
        )}
        ...
        {!loading && data && (
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
              {[
                ["Потрачено", fmtUsd(data.totals.costUsd)],
                ["Вызовов", String(data.totals.calls)],
                ["Токены in", fmtTokens(data.totals.tokensIn)],
                ["Токены out", fmtTokens(data.totals.tokensOut)],
              ].map(([label, value]) => (
            ...
            {data.daily.length > 0 && (
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 mb-6">
                <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-3">Расход по дням · 30 дней</p>
                <div className="flex items-end gap-[2px] h-20">
                  {data.daily.map((d) => (
                    <div
                      ...
                      title={`${d.date}: ${fmtUsd(d.costUsd)} · ${d.calls} вызовов`}
                    />
                  ))}
                </div>
              </div>
            ...
            {/* Per-provider */}
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 mb-6">
              <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-3">По провайдерам</p>
              {data.byProvider.length === 0 && (
                <p className="text-sm text-slate-500">Пока нет записанных вызовов.</p>
              )}
                    ...
                    <div key={p.provider}>
                      <div className="flex items-center justify-between text-sm mb-1">
                        ...
                        <span className="text-slate-400 text-xs">
                          {fmtUsd(p.costUsd)} · {share.toFixed(1)}% · {p.calls} вызовов · {fmtTokens(p.tokensIn + p.tokensOut)} ткн
                        </span>
                      </div>
            ...
            {data.byModel.length > 0 && (
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 mb-6 overflow-x-auto">
                <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-3">Топ моделей по расходу</p>
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-[10px] text-slate-500 uppercase tracking-wider">
                      <th className="pb-2 pr-3 font-medium">Модель</th>
                      <th className="pb-2 pr-3 font-medium text-right">Вызовы</th>
                      <th className="pb-2 pr-3 font-medium text-right">Токены</th>
            ...
            <p className="text-xs text-slate-600 text-center mt-4">
              Источник: {data.source === "db" ? "Postgres (все время)" : "память процесса (с последнего рестарта)"} ·
              бесплатные fleet-модели считаются $0 по прайс-листу
            </p>
          </>
        )}
      </div>
