 ...
 *
 * Это МЕХАНИЧЕСКАЯ часть более широкого правила: форма ответа публичной ручки
 * задаётся явным перечнем полей. Правило целиком, вместе с итогами аудита 19
 * ручек 28.07 и списком уже проверенных, — `docs/PUBLIC-ENDPOINTS.md`.
 *
 ...
 * и таблица попадёт под правило сама. Это не теория: замер живого прода 28.07
 * показал в ответе 17 полей сертификата против 12 в `CREATE TABLE` — четыре
 * добавлены поздними миграциями и подхватились сами.
 *
 * ГРАНИЦЫ. Правило узкое намеренно — только колонки, которым нельзя покидать
 * процесс (секреты, хеши паролей, «приватные» payload'ы). Колонки с почтой сюда
 * НЕ включены, хотя `IPCertificate."authorEmail"` — персональные данные:
 * выбирать почту звёздочкой законно (её надо читать, чтобы письмо отправить), и
 * правило дало бы три ложных срабатывания на местах, где ответ и так собирается
 * явным объектом. Ложное срабатывание хуже отсутствия проверки — оно заставляет
 * переписывать корректный код и учит игнорировать красноту.
 *
 * Публичные ручки сертификатов проверены живым прогоном по проду 28.07 и НЕ
 * требуют правок (перепроверять не нужно): `/api/pipeline/verify/:certId` и
 * `/api/pipeline/certificate/:certId/bundle.json` — оба используют `SELECT *`,
 * но собирают ответ явным объектом, ни почты, ни ownerId, ни секретов в выдаче
 * нет. Утечки нашлись только там, где строка БД попадала в ответ целиком.
 */
