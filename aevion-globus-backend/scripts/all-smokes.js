...
const SMOKES = [
  ...
  // Сверка ЖИВОЙ витрины магазина с кодом. Каталог выше сверяет 16 позиций и
  // НИ ОДНОГО тарифа, а тариф покупают как раз те, кто упёрся в платную
  // стену. Таблица сопоставления лежала в коде с 13.08.2026 без потребителя —
  // и первый же прогон 13.09 нашёл, что «Planet — Annual» списывает
  // ЕЖЕМЕСЯЧНО, тогда как все прочие годовые — раз в год.
  //
  // Известные расхождения записаны в storefront-vs-code.baseline.json: они
  // ждут решения основателя (панель провайдера), а набор от них краснеть не
  // должен. Роняет только НОВОЕ; исчезнувшее печатается как починка.
  // Прогнан перед добавлением: 17 товаров, новых расхождений 0.
  { name: "storefront-vs-code", script: "storefront-vs-code.js", readOnly: true },
