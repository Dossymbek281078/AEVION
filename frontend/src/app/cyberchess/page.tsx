        ...
        <div style={{display:"flex",gap:16,flexWrap:"wrap",alignItems:"flex-start"}} onContextMenu={e=>{e.preventDefault();sPms([]);sPmSel(null)}}>
          <div style={{flexShrink:0}}>
            ...
            <div style={{display:"flex",gap:6,marginTop:10,flexWrap:"wrap"}}>
              ...
              {pms.length>0&&<button onClick={()=>sPms(p=>p.slice(0,-1))} style={{padding:"7px 14px",borderRadius:8,border:`1px solid rgba(96,165,250,0.3)`,background:"rgba(96,165,250,0.1)",color:S.blue,fontSize:11,fontWeight:700,cursor:"pointer"}}>↩ Undo last</button>}
              {pms.length>0&&<button onClick={()=>{sPms([]);sPmSel(null)}} style={{padding:"7px 14px",borderRadius:8,border:`1px solid rgba(248,113,113,0.2)`,background:"rgba(248,113,113,0.06)",color:S.danger,fontSize:11,fontWeight:700,cursor:"pointer"}}>✕ Clear all ({pms.length})</button>}
            </div>
            {/* Game actions: resign, draw, takeback */}
            {on&&!over&&!setup&&(
              <div style={{display:"flex",gap:6,marginTop:6,flexWrap:"wrap"}}>
                <button onClick={()=>{if(confirm("Resign this game?")){const nr=Math.max(100,rat-Math.max(5,Math.round((rat-lv.elo)*0.1+10)));sRat(nr);svR(nr);const ns={...sts,l:sts.l+1};sSts(ns);svS(ns);sPms([]);sOn(false);sOver("You resigned — AI wins");snd("resign")}}} style={{padding:"7px 14px",borderRadius:8,border:`1px solid rgba(248,113,113,0.3)`,background:"rgba(248,113,113,0.08)",color:S.danger,fontSize:11,fontWeight:700,cursor:"pointer"}}>🏳 Resign</button>
                <button onClick={()=>{if(confirm("Offer draw?")){const score=ev(game);if(Math.abs(score)<200){const ns={...sts,d:sts.d+1};sSts(ns);svS(ns);sPms([]);sOn(false);sOver("Draw by agreement");snd("draw")}else{showToast("AI declined the draw","error")}}}} style={{padding:"7px 14px",borderRadius:8,border:`1px solid ${S.border}`,background:S.surface,color:S.textDim,fontSize:11,fontWeight:700,cursor:"pointer"}}>½ Draw</button>
