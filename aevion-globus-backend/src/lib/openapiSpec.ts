/**
 * ⚠️ ЭТОТ ФАЙЛ НИКТО НЕ ИМПОРТИРУЕТ. Проверено 21.08.2026 поиском по всему
 * бэкенду: единственная ссылка на него — КОММЕНТАРИЙ в `index.ts` («See
 * lib/openapiSpec.ts»), а служится другой объект, собранный там же на месте
 * (`app.get("/api/openapi.json", …)`, ~267 путей против 126 здесь).
 *
 * Чем это опасно: правка здесь НИЧЕГО НЕ МЕНЯЕТ для потребителя, а выглядит как
 * работа. Я сам на это попался — нашёл в этом файле две несуществующие ручки,
 * записал их как «публичная спецификация обещает несуществующее», и только
 * проба живого прода показала, что файл не служится вовсе.
 *
 * Живая спецификация при этом ЧЕСТНА: из её 195 путей без параметров все,
 * объявленные как GET, отвечают (сплошная проба 21.08.2026, ноль расхождений).
 *
 * Что с этим делать — решение владельца модуля: либо подключить этот файл
 * вместо инлайна в index.ts, либо удалить его. Оставлять две спецификации,
 * одна из которых мертва, а комментарий указывает на мёртвую как на источник, —
 * ловушка для следующего, кто сюда придёт.
 */
...
export const openapiSpec = {
  ...
  paths: {
    ...
    // ВОЗВРАЩЕНО В СПЕЦИФИКАЦИЮ 21.08.2026, потому что механизм ПОСТРОЕН.
    //
    // История записи стоит того, чтобы её знать. Ручка была описана здесь
    // подробно, а маршрута не существовало — соседняя сессия справедливо
    // убрала запись: публичная спецификация не должна обещать средство
    // безопасности, которого нет, на него могут положиться.
    //
    // Их предупреждение «прежде чем реализовывать, прочтите замер от 20.08:
    // ни счётчика, ни его сверки в коде НЕТ» выполнено буквально. Сделано не
    // «дописать одну ручку», а весь механизм: `tv` кладётся в токен при
    // выпуске, сверяется в РАЗБОРЕ токена (точка одна на 41 файл), ручка
    // увеличивает счётчик. Тест проверяет смысл: тот же токен с верной
    // подписью перестаёт подходить после нажатия кнопки.
    //
    // Разбор: папка 15-Аудиты-и-сводки, файл ВЫХОД-со-всех-устройств-не-работает-20-08.md
    "/api/auth/sign-out-everywhere": {
      post: {
        summary: "Bump tokenVersion — invalidate every JWT for caller (auth)",
        description:
          "Increments AEVIONUser.tokenVersion. Every previously issued token for this user stops verifying, INCLUDING the current one — that is what the button promises. Sessions are marked revoked as well, so the device list matches reality.",
        responses: {
          "200": { description: "{ ok: true, tokenVersion: number, revokedCount: number }" },
          "401": { description: "auth required" },
          "500": { description: "sign-out-everywhere failed" },
        },
      },
    },
