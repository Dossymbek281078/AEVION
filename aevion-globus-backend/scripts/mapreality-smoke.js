...
async function run() {
  ...
  assert("data is array", Array.isArray(list.body?.signals));
  ...
  if (signalId) {
    ...
    assert("signal.id matches", single.body?.signal?.id === signalId);
  }
  ...
  if (signalId) {
    ...
    assert("supportCount incremented", (s1.body?.supportCount ?? 0) >= 1);
  ...
  const items = needs.body?.signals ?? [];
  ...
  assert("byCategory present", typeof stats.body?.byCategory === "object");
  assert("topSignals is array", Array.isArray(stats.body?.topSignals));
