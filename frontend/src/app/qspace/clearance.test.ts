...
describe("предмет внутри стены", () => {
  // Найдено на демо-расстановке замером зазоров: ванна сидела в наружной стене
  // на 25 мм при ПОЛНОМ молчании сторожа. Причина — «за пределами плана»
  // считается по осям стен, а стена имеет толщину: полтолщины (у наружной
  // 0.15 м) можно было занять безнаказанно, и в 3D предмет торчал сквозь стену.
  const room = (): Plan => ({
    name: "коробка",
    walls: [
      { x1: 0, y1: 0, x2: 6, y2: 0, thickness: 0.3, height: 2.7 },
  ...
  const item = (x: number, y: number): Placed =>
    ({ uid: 1, name: "Шкаф", x, y, rotY: 0, size: [1.0, 0.6, 2.0] });

  it("вплотную к стене — законно, замечания быть не должно", () => {
    // внутренняя грань южной стены на y = 0.15; глубина 0.6 -> центр 0.45
    expect(checkClearance(room(), [item(3, 0.45)])).toEqual([]);
  });

  it("утоплен в стену на 10 см — замечание есть", () => {
    const issues = checkClearance(room(), [item(3, 0.35)]);
    expect(issues.map((i) => i.text).join(" ")).toMatch(/заходит в стену/);
  });

  it("замечание НЕ дублируется с «вышел за пределы плана»", () => {
    // предмет целиком снаружи: одно замечание, а не два — иначе сторож шумит
    expect(checkClearance(room(), [item(3, -1.5)]).length).toBe(1);
  });

  it("утоплен на 5 см — тоже ловится: граница прощения узкая", () => {
    // Без этого случая допуск закреплён лишь между 2 см и ~20 см: мутация
    // «прощать 20 см» переживала прежние два теста. Пара «5 см ловится / 1 см
    // прощается» зажимает его с обеих сторон.
    expect(checkClearance(room(), [item(3, 0.4)]).map((i) => i.text).join(" "))
      .toMatch(/заходит в стену/);
  });

  it("прощение мелочи: 1 см захода не поднимает тревогу", () => {
    // мебель ставят вплотную, и придирка к миллиметрам сделала бы сторожа
    // шумным — а шумного отключают
