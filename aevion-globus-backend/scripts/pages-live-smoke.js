...
(async () => {
  ...
  //
  // Добавлен 02.09.2026, после находки соседнего окна: на ДИНАМИЧЕСКИХ адресах
  // выдуманный путь отвечает 200, отдаёт полную разметку и содержит слово
  // aevion — то есть проходит все три наших условия. Замер: из шести выдуманных
  // адресов пять «живы».
  //
  // Значит критерий «200 + размер + бренд» на таких маршрутах не доказывает
  // ничего. Различить curl-ом нельзя в принципе: содержимое рисует скрипт, а
  // маркер notFound лежит в каркасе и есть на ЛЮБОЙ странице, включая /pricing.
  //
  // Поэтому прибор обязан сообщать о собственной слепоте. Контроль не роняет
  // прогон: он не про продукт, а про то, что этот прогон умеет утверждать.
  const ВЫДУМАННЫЕ = [
    "/devhub/net-takogo-proekta-zzz",
    "/qskyway/net-takogo-marshruta-zzz",
    "/privacy-zzz",
  ];
  const слепые = [];
  for (const p of ВЫДУМАННЫЕ) {
    try {
      const r = await fetch(BASE + p, { redirect: "follow", headers: { Accept: "text/html" } });
      const body = await r.text();
      if (r.ok && body.length > 5000 && /aevion/i.test(body)) слепые.push(p);
    } catch { /* сеть — не про слепоту, молчим */ }
  }
  if (слепые.length > 0) {
    console.log(
      `
  ! ГРАНИЦА ПРОВЕРКИ: выдуманные адреса тоже проходят (${слепые.length} из ${ВЫДУМАННЫЕ.length}): ${слепые.join(", ")}`,
    );
    console.log(
      "    На динамических маршрутах PASS означает «сервер ответил», а НЕ «страница существует».",
    );
  } else {
    console.log("контроль: выдуманные адреса не проходят — PASS означает то, что обещает");
  }
