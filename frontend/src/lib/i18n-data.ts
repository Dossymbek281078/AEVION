...
export const LANG_KEY_COUNT: Record<Lang, number> = {
  ...
  // 16.09: СЛИТОЕ значение двух работ. Посчитано по словарям, а не выбрано стороной —
  // здесь это принципиально, и вот почему.
  //
  //   моя сторона (цены «тариф = срок»): ключи pricing.* переписаны, сняты period/annual/
  //     bundles, заведены сроки и блок приложений → давала ru 7425, en 7425, kk 7384;
  //   их сторона (qskyway): +1 ключ qskyway.city.amsterdam, пятый город → давала 7463/7463/7422,
  //     но считалась от базы БЕЗ моих снятий (37 ключей), поэтому напрямую не переносится.
  //
  // Каждое число верно относительно своей базы и НЕВЕРНО относительно слитой: словари
  // i18n-lang/*.ts слились без конфликтов и содержат теперь и мои ключи, и их. Взять любую
  // сторону — тот самый класс, когда правка верна по строке и рвёт связь между двумя числами.
  // Правильное значение = счёт по слитым словарям; судья — dictPreloadFromHead, он сравнивает
  // с Object.keys(dict).length.
  // 17.09 (общая выкатка ca5ffbde9 + 24138af68): Берлин и Вена дали +2 в ru/en/kk,
  // а казахскому словарю user-68 долил 41 недостающий ключ — паритет трёх полных
  // языков восстановлен. Число — счёт по слитым словарям, не сторона мержа.
  // 17.09: +1 в ru/en/kk — qskyway.city.zurich (восьмой город); снова счёт по слитым словарям.
  // 20.09: −26 в ru/en/kk. Убраны выдуманные свидетельства: четыре истории
  // найма QBuild с именами и цитатами (27 строк, из них часть — не ключи
  // словаря), цитаты страницы миграций с названиями компаний; добавлены две
