...
> Но рендер-ветка срабатывает, только если ОПРАШИВАЕМАЯ страницей ручка вернула 402. Разобрал
> пробу каждой страницы: **10 опрашивают `/api/<модуль>/health`**, который `isExemptPath()`
> намеренно оставляет открытым на закрытом модуле, — у них стена недостижима. **3 опрашивают
> закрытые ручки** и стену покажут: `/qcoreai/playground` (`/api/qcoreai/chat`), `/qmaskcard`
> (`/api/qmaskcard/masks`), `/qmedia` (`/api/qmedia/videos`). У `/veilnetx` проба не
> определяется статически. Проверено запуском: с `PAYWALL_MODULES=healthai` API отдаёт 402,
> а `/healthai` — 200 и обычную страницу.
>
> | Что открывает бесплатный пользователь | Что он увидит при включённом гейте |
> |---|---|
> | 10 страниц, чья проба идёт в открытый `/health` (`/healthai`, `/qcoreai`, `/qskyway`, `/smeta-trainer`, `/multichat-engine`, `/longevity`, `/qrenew` + `/report`, `/qmelanin` + `/track`) | обычную страницу |
> | `/qcoreai/playground`, `/qmaskcard`, `/qmedia` | **страничную стену** — они зовут реально закрытые ручки |
