...
const OWN_GATE = new Set([
  ...
  // 15.09.2026: DevHub ведёт доступ своим тарифом (DevHubTier, routes/devhub.ts),
  // а подписка платформы поднимает его до Pro — общий гейт ему не нужен.
  "devhub",
]);
...
describe("у каждого платного модуля есть проводка шлюза", () => {
  ...
  it("платный модуль либо в таблице шлюзов, либо в списке своих механизмов", () => {
    const ids = gatedIds();
    // Платный — тот, кто НЕ входит во free: с 15.09.2026 отдельная цена добавки
    // осталась лишь у пяти приложений, и мерить ею платность значило бы проверять
    // четыре модуля вместо сорока одного. Бесплатные гейтить незачем, и требовать
    // этого было бы вечно красным аудитом.
    const paid = MODULES_PRICING.filter((m) => !m.includedIn.includes("free"));
    expect(paid.length, "контроль: платные модули найдены").toBeGreaterThan(20);
    // Контроль в другую сторону: бесплатные в список не попали.
    expect(paid.map((m) => m.id)).not.toContain("globus");
  ...
  it("список своих механизмов не разросся молча", () => {
    ...
    expect(OWN_GATE.size).toBeLessThanOrEqual(10);
  });
});
