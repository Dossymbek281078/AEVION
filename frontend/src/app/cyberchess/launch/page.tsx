...
export default async function CyberChessLaunchPage({
  ...
  // Дата ПЕРЕНЕСЕНА основателем 29.08.2026 на 30 сентября. 30.08 страница
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", display: "flex", flexDirection: "column", gap: 28 }}>
        ...
        <p style={{ color: MUTED, fontSize: 13.5, lineHeight: 1.6, margin: 0 }}>
          {/* В ДЕНЬ запуска «не хотите ждать» становится ложью: ждать уже
              нечего. Оба текста ниже развилкой по left, чтобы 30 сентября
        ...
        {/* ЧЕСТНАЯ ПОДПИСЬ РАННЕЙ ВЕРСИИ.
            ...
            ТЕСТОВАЯ версия, полный запуск 30 сентября. 13.09.2026 основатель
            перенёс запуск ПЛАТФОРМЫ с 10 на 20 сентября; полный запуск шахмат
            остался 30 сентября, и строка ниже от переноса не меняется —
            приложение и так открыто, речь только о том, как мы его называем. Причину переноса он
            назвал прямо: нет удобного разбора партии сразу после её конца.
            Строка ниже говорит это же посетителю — иначе страница зовёт
            попробовать то, что мы сами считаем недоделанным, и молчит о том,
            чего не хватает. Механизма закрытого доступа мы намеренно НЕ
            строим: приложение уже открыто, «тестовая версия» — это то, как мы
            его называем, а не отдельная дверь. */}
        <p style={{ color: MUTED, fontSize: 15, lineHeight: 1.6, margin: "14px 0 0" }}>
          Сейчас это ранняя версия: играть, решать задачи и заниматься с коучем
          можно уже сегодня, а разбор партии сразу после её конца мы доделываем
          к полному запуску.
        </p>
