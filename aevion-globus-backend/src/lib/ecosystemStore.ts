...
export type Tournament = {
  ...
  /**
   * Образец, заведённый чтобы раздел не был пустым, — не настоящий турнир.
   *
   * Признак живёт в САМОМ ответе, а не в имени: до 27.08.2026 отличить образец
   * можно было только по префиксу `tour_demo_` в идентификаторе, то есть
   * знанием наших внутренних соглашений. Любой читатель — наша же страница
   * выигрышей, партнёр по API, страница диагностики — обязан уметь сказать,
   * настоящий турнир перед ним или показательный.
   *
   * Отсутствие поля означает «настоящий»: так старые записи в хранилище не
   * превращаются молча в образцы.
   */
  demo?: boolean;
};
