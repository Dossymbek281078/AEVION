...
describe("обещание написать в день запуска обеспечено рассылкой", () => {
  ...
  /**
   * 🔴 Добавлено 09.09.2026, за сутки до запуска. Состав списков сходился, дата
   * сходилась — а строка `qright` была СКОПИРОВАНА с соседней (bureau) и не
   * поправлена: подписчику со страницы QRight письмо называло чужой модуль
   * («AEVION IP Bureau») и вело на чужую страницу запуска, накануне запуска
   * его собственного модуля.
   *
   * Прежние две проверки этого не видели по устройству: они сверяют, КТО в
   * списках, а дефект был в том, ЧТО про него написано. Совпадение состава
   * читалось как согласие списков.
   *
   * ГРАНИЦА. Проверяется не совпадение имён (они намеренно разные: «QRight»
   * против «AEVION IP Bureau» — это разные продукты), а то, что запись НЕ
   * отправляет подписчика на страницу ЧУЖОГО модуля: адрес обязан начинаться
   * со своего префикса либо быть общей страницей платформы.
   */
  it("запись не отправляет подписчика на страницу ЧУЖОГО модуля", () => {
    const src = readFileSync(CONFIRM, "utf8");
    const i = src.indexOf("const LAUNCH_MODULES");
    const блок = src.slice(i, src.indexOf("];", i));
    const записи = [...блок.matchAll(/prefix:[ ]*"([a-z][a-z-]*)"[^}]*?page:[ ]*"([^"]+)"/g)]
      .map((m) => ({ префикс: m[1], адрес: m[2] }));

    // контроль охвата: разбор обязан увидеть все записи, иначе «чужих нет»
    // означало бы «я ничего не прочитал»
    expect(записи.length, "разбор увидел не все записи письма").toBe(обещали.length);

    const чужие = записи.filter(({ префикс, адрес }) => {
      const после = адрес.includes("://") ? адрес.split("://")[1] : адрес;
      const куски = после.split("/").filter(Boolean);
      const первый = куски.length > 1 ? куски[1] : "";  // куски[0] — домен
      if (первый === "") return false;                  // общая страница — законно
      // «multichat» → «/multichat-engine», «startup» → «/startup-exchange»:
      // свой префикс может быть НАЧАЛОМ имени страницы, но не чужим именем
      return !первый.startsWith(префикс);
    });

    expect(
      чужие.map((з) => з.префикс + " -> " + з.адрес),
      "письмо-подтверждение ведёт подписчика на страницу другого модуля: "
      + "человек, оставивший адрес на одной странице, получит письмо про другую",
    ).toEqual([]);
  });
