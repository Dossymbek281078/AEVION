...
describe("масштаб по размерным числам", () => {
  ...
  it("допуск группы 1.5 %: цепочка на 2.5 % крупнее в согласные НЕ входит", () => {
    // 4 пары в масштабе 40 и 3 пары в масштабе 41 (на 2.5 % дальше). При допуске
    // 3 % они слились бы в одну группу из 7 — и чужой ряд голосовал бы за ответ.
    const r = масштабПоРазмерам([
      ...цепочка([1400, 2000, 750, 600, 1800], 40, "h", 0),
      ...цепочка([1500, 4200, 1550, 2090], 41, "h", 50),
    ]);
    expect(r!.mmPerPt).toBeCloseTo(40, 6);
    expect(r!.agree).toBe(4);
    expect(r!.pairs).toBe(7);
  });
...
describe("масштаб по осям — чертёж, растянутый при печати", () => {
  const h40 = цепочка([1400, 2000, 750, 600, 1800], 40, "h", 10);
  const v = (k: number): СловоНаЛисте[] => цепочка([3350, 4470, 3110, 1500], k, "v", 300);
  it("оси сходятся (40 и 40.3, расхождение < 2 %) — предупреждения нет, обе оси названы", () => {
    const r = масштабПоРазмерам([...h40, ...v(40.3)]);
    expect(r!.byAxis!.h).toBeCloseTo(40, 6);
    expect(r!.byAxis!.v).toBeCloseTo(40.3, 6);
    expect(r!.axisMismatchPct).toBeCloseTo(0.75, 2);
    expect(предупреждениеОбОсях(r!)).toEqual([]);
  });

  it("растянуто по вертикали на 5 % — предупреждение с обоими числами", () => {
    const r = масштабПоРазмерам([...h40, ...v(42)]);
    expect(r!.mmPerPt).toBeCloseTo(40, 6); // общий ответ — по большей группе
    expect(r!.axisMismatchPct).toBeCloseTo(5, 2);
    const w = предупреждениеОбОсях(r!);
    expect(w).toHaveLength(1);
    expect(w[0]).toMatch(/40\.0 мм\/пт/);
    expect(w[0]).toMatch(/42\.0/);
    expect(w[0]).toMatch(/5\.0 %/);
    expect(w[0]).toMatch(/растянут/);
  });

  it("порог 2 %: 2.1 % — предупреждение есть, 1.9 % — нет", () => {
    expect(предупреждениеОбОсях(масштабПоРазмерам([...h40, ...v(40.84)])!)).toHaveLength(1);
    expect(предупреждениеОбОсях(масштабПоРазмерам([...h40, ...v(40.76)])!)).toEqual([]);
  });

  it("по оси меньше трёх согласных пар — ось не считается (пары «через стену» не дают ложной тревоги)", () => {
    // две вертикальные пары сходятся между собой на 38.3, как на LA VIE
    const r = масштабПоРазмерам([...h40, ...цепочка([3350, 4470, 3110], 38.3, "v", 300)]);
    expect(r!.byAxis).toEqual({ h: 40 });
    expect(предупреждениеОбОсях(r!)).toEqual([]);
  });

  it("одна ось — сравнивать не с чем: расхождения нет, предупреждения нет", () => {
    const r = масштабПоРазмерам(h40);
    expect(r!.byAxis).toEqual({ h: 40 });
...
describe.skipIf(!existsSync(LAVIE))("LA VIE.pdf: масштаб без ввода", () => {
  it("pdf.js находит размерные цепочки, масштаб 42.59 мм/пт", async () => {
    ...
    // настоящий чертёж напечатан 1:1 — ложной тревоги о растяжении быть не должно
    expect(предупреждениеОбОсях(r!)).toEqual([]);
  }, 60_000);
