...
import {
  адресатПодтвержденияЗаявки,
  строкиПодтвержденияЗаявки,
} from "../src/lib/constitutionBrevo";
...
/**
 * Подтверждение отправителю (20.09.2026).
 *
 * Повод: страница пишет «Заявка отправлена», а в почте у человека пусто —
 * единственное письмо уходило нам. Для холодного посетителя это неотличимо от
 * «отправил в никуда».
 *
 * Проверяем не отправку (она за сетью), а два решения, которые можно испортить
 * молча: КОМУ писать и ЧТО обещать.
 */
describe("подтверждение заявки отправителю", () => {
  it("почта — пишем, телефон и мусор — нет", () => {
    expect(адресатПодтвержденияЗаявки("elena@grandec.kz")).toBe("elena@grandec.kz");
    expect(адресатПодтвержденияЗаявки("+7 777 123-45-67")).toBe(null);
    expect(адресатПодтвержденияЗаявки("позвоните мне")).toBe(null);
    expect(адресатПодтвержденияЗаявки("")).toBe(null);
  });

  it("обещает ровно то, что уже стоит на странице", () => {
    const т = строкиПодтвержденияЗаявки({}).join(" ");
    expect(т).toContain("в течение рабочего дня");
    expect(т).toContain("Бесплатно");
  });

  it("КОНТРОЛЬ: не обещает того, чего продукт не делает", () => {
    const т = строкиПодтвержденияЗаявки({}).join(" ").toLowerCase();
    for (const запрещено of ["гарантируем", "круглосуточно", "мгновенно", "без участия человека"]) {
      expect(т).not.toContain(запрещено);
    }
  });

  it("КОНТРОЛЬ: ни одна строка не повторяется дважды", () => {
    // 22.09.2026 первая строка ушла клиенту ДВАЖДЫ: «заявка получена… заявка
    // получена…». Тесты были зелёные — они проверяли наличие строк, а не то,
    // что их ровно столько, сколько задумано. Нашлось только чтением письма.
    for (const lead of [{}, { name: "Елена" }]) {
      const строки = строкиПодтвержденияЗаявки(lead).filter((s) => s.trim());
      expect(new Set(строки).size).toBe(строки.length);
    }
  });

  it("имя подставляется, если его прислали, и без него текст остаётся целым", () => {
    expect(строкиПодтвержденияЗаявки({ name: "Елена" })[0]).toContain("Елена, заявка получена");
    expect(строкиПодтвержденияЗаявки({})[0]).toContain("заявка получена");
  });
});
