...
export interface PaymentIntentInput {
  ...
  /**
   * Цена ЭТОЙ покупки в центах, когда она не равна цене товара в кассе.
   *
   * Зачем. У отдельных модулей своего товара в LemonSqueezy может не быть
   * (замер 21.09.2026: qright, qsign, startup_exchange, qskyway — цена на
   * витрине назначена, касса отвечала 503). Завести 155 товаров руками
   * нельзя: API кассы на создание товаров отвечает 405. Зато цену можно
   * назначить поверх существующего варианта — проверено настоящей кассой
   * (POST /v1/checkouts с custom_price → 201).
   *
   * Провайдер, который такого не умеет, поле игнорирует — тогда покупатель
   * заплатит цену товара. Поэтому ставить его можно только там, где обе
   * цены заведомо совпадают, либо у провайдера, который его читает.
   */
  customPriceCents?: number;
  /**
   * Язык страницы оплаты — код языка той страницы, С КОТОРОЙ пришёл покупатель.
   *
   * Зачем: замер 09.09.2026 показал, что наша страница оплаты открывалась
   * ПО-БОЛГАРСКИ («Плащане с карта») — покупатель уходил с русской витрины и
   * попадал на последний экран перед вводом карты на чужом языке. Мы не
   * передавали язык вовсе, а у кассы порядок такой: наш явный код → язык
   * магазина → язык браузера. Пустое место в первом звене и давало что попало.
   *
   * Необязательное: провайдер, который язык не поддерживает, поле игнорирует,
