...
export function CompetitorMatrix({ set }: { set: CompetitorSet }) {
  ...
  return (
    // lang="ru" — не косметика, а честная метка языка поддерева.
    //
    // Замер 27.08.2026 на живой странице /qskyway: документ объявлен lang="en",
    // шапка и описание английские, а эта матрица — 53 строки русского текста
    // против 32 латинских. Данные в lib/competitors.ts русские по построению:
    // языковых ключей в них НОЛЬ, и компонент в словарь не ходит.
    //
    // Пока перевод не сделан (это контент и позиционирование, не код), браузер
    // обязан хотя бы знать правду: без метки он считает блок английским и
    // предлагает машинно перевести ВСЮ страницу, ломая и английскую половину.
    // С меткой предложение касается только этого поддерева, а экранный диктор
    // читает его по-русски, а не по буквам.
    <section lang="ru" style={{ background: "#0d1420", border: "1px solid #1e2836", borderRadius: 10, overflow: "hidden" }}>
      <button
        onClick={() => setOpen((v) => !v)}
        style={{
