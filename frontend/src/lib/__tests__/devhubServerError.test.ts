...
describe("текст серверной ошибки — на языке человека", () => {
  ...
  test("месячная квота — самое важное сообщение денежного пути", () => {
    ...
    // 09.09.2026: имя нормы теперь ЧЕЛОВЕЧЕСКОЕ. Намерение проверки прежнее —
    // сказать, ЧТО исчерпалось, — но закреплять его английским словом значило
    // требовать жаргон на русском экране: человек читал «исчерпана: deploy».
    expect(s, "не сказано, ЧТО исчерпалось").toContain("публикации");
  ...
  test("правило работает для всех возможностей, а не только для выкаток", () => {
    ...
    // Пары «имя от сервера → что читает человек». Проверяем СЛЕДСТВИЕ:
    // норма названа и названа по-русски.
    for (const [cap, chelovecheski] of [
      ["video", "видео"],
      ["image", "картинки"],
      ["music", "музыка"],
      ["tts", "озвучка"],
      ...
      expect(s, `${cap}: имя нормы осталось машинным`).toContain(chelovecheski);
    }
  });
